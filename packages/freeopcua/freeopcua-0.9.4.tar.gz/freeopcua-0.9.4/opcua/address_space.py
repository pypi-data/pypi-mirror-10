...
class AttributeService(object):
    ...
    def write(self, params, user):
        self.logger.debug("write %s", params)
        res = []
        for writevalue in params.NodesToWrite:
            if user != User.Admin:
                al = res.append(self._aspace.get_attribute_value(writevalue.NodeId, ua.AttributeId.AccessLevel))
                ual = res.append(self._aspace.get_attribute_value(writevalue.NodeId, ua.AttributeId.UserAccessLevel))
                print("VAL IS ", val)
                if al == 2 and ual == 2 
                val = self._aspace.get
...
class NodeManagementService(object):
    ...
    def add_nodes(self, addnodeitems, user):
        results = []
