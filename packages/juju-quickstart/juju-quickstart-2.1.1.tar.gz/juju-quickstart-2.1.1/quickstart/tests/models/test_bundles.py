...
class TestBundle(helpers.BundleFileTestsMixin, unittest.TestCase):
    ...
    def test_get_changeset(self):
        # The collection of changes corresponding to the bundle is correctly
        # returned.
        expected_changeset = (
            {'id': 'addCharm-0',
             'method': 'addCharm',
             'args': ['cs:vivid/mysql-47'],
             'requires': []},
            {'id': 'addService-1',
             'method': 'deploy',
             'args': ['cs:vivid/mysql-47', 'mysql', {}],
             'requires': ['addCharm-0']},
            {'id': 'addCharm-2',
             'method': 'addCharm',
             'args': ['cs:trusty/wordpress-42'],
             'requires': []},
            {'id': 'addService-3',
             'method': 'deploy',
             'args': ['cs:trusty/wordpress-42', 'wordpress', {}],
             'requires': ['addCharm-2']},
            {'id': 'addUnit-4',
             'method': 'addUnit',
        ...
        bundle = helpers.make_ordered_bundle(self.bundle)
        self.assertEqual(expected_changeset, tuple(bundle.get_changeset()))
    ...
    def test_serialization(self):
        ...
        expected_content = (
            'services:\n'
            '  mysql:\n'
            '    charm: cs:vivid/mysql-47\n'
            '    num_units: 0\n'
            '  wordpress:\n'
            '    charm: cs:trusty/wordpress-42\n'
            '    num_units: 1\n')
        self.assertEqual(expected_content, self.bundle.serialize())
    ...
    def test_services(self):
        # Bundle services and their number of units can be easily retrieved.
    ...
    def test_services_subordinate_units(self):
        # The number of units, if not defined, is None.
...
class TestFromSource(
    ...
    def test_local_file_legacy_bundle(self):
        ...
        legacy_bundle_data = {
            'bundle': {
                'services': {
                    'wordpress': {'charm': 'wordpress', 'num_units': 1},
                    'mysql': {'charm': 'mysql', 'num_units': 1},
                }
            },
        }
...
class TestParseYAML(
    ...
    def test_yaml_gui_in_services(self):
        ...
        contents = yaml.safe_dump({
            'services': {
                settings.JUJU_GUI_SERVICE_NAME: {
                    'charm': 'juju-gui',
                    'num_units': 1,
                },
            },
...
class TestValidate(
    ...
    def test_yaml_gui_in_services(self):
        ...
        with self.assert_value_error(expected_error):
            bundles.validate(data)

    def test_multiple_errors(self):
        # A ValueError with multiple bundle errors is properly reported.
        expected_error = (
            'invalid bundle data:\n'
