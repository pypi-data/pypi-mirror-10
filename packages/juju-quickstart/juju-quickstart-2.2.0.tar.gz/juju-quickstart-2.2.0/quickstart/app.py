...
def check_environment(
    ...
    charm_ref = jujugui.parse_charm_url(charm_url)
...
def deploy_gui(
        env, service_name, charm_url, machine, options,
    ...
    Receive the following arguments:
        ...
        - machine: the machine where to deploy to (e.g. "0") or None if a new
          machine must be created;
        - options: GUI charm options used when the Juju GUI service is not
          present in the environment, or None if no customized options are
          required;
        - service_data: the service info as returned by the mega-watcher for
          services, or None if the service is not present in the environment;
        - unit_data: the unit info as returned by the mega-watcher for units,
    ...
    if options:
        option_str = '\n'.join(
            '  {}="{}"'.format(k, v) for k, v in sorted(options.items()))
    if service_data is None:
        # The service is not in the environment: deploy it without units.
        print('requesting {} deployment'.format(service_name))
        if option_str:
            print('using customized {} options:\n{}'.format(
                service_name, option_str))
        try:
            env.deploy(service_name, charm_url, num_units=0, config=options)
        except jujuclient.EnvError as err:
            raise ProgramExit('bad API response: {}'.format(err.message))
        ...
        if option_str:
            logging.warn('ignoring customized {} options:\n{}'.format(
                service_name, option_str))
...
def get_service_config(env, service_name):
    """Return the configuration options for the given service name.

    The options are returned as a dict mapping option names to values.
    If an option is unset, its value is None.
    ...
    return dict((k, v.get('value')) for k, v in config.items())


def deploy_bundle(env, bundle, uncommitted, charm_ref):
    """Deploy the given bundle connecting to the given environment.

    Receive:
        - env: the environment connection to use for deploying the bundle;
        - bundle: the bundle to be deployed, as an instance of
          "quickstart.models.bundles.Bundle";
        - uncommitted: a flag indicating whether the given bundle must be
          deployed in uncommitted mode;
        - charm_ref: the Juju GUI charm reference, used in the case uncommitted
          is True in order to verify that the GUI service supports uncommitted
          bundle deployments.

    If uncommitted is True, do not actually deploy the workload, but instead
    register it as a set of changes on the GUI server, and return a token
    identifying the change set, to be reused later by the Juju GUI.
    If uncommitted is False, start the bundle deployment and return None.

    Raise a ProgramExit if any of the API server calls return an error.
    """
    services = ', '.join(bundle.services().keys())
    ref = bundle.reference
    details = ''
    if ref is not None:
        details = (
            '\nmore details about this bundle can be found at\n'
            '  {}'.format(ref.jujucharms_url()))

    if uncommitted:
        # Ensure the currently deployed Juju GUI service supports change sets.
        series = charm_ref.series
        revision = settings.MINIMUM_REVISIONS_FOR_UNCOMMITTED_BUNDLES[series]
        if jujugui.is_promulgated(charm_ref) and charm_ref.revision < revision:
            logging.warn(
                'uncommitted bundles loading is not supported by currently '
                'deployed Juju GUI charm ({}): requested bundle deployment '
                'will be ignored'.format(charm_ref))
            logging.warn(
                'run "juju upgrade-charm {}" to enable uncommitted bundles '
                'support'.format(settings.JUJU_GUI_SERVICE_NAME))
            return None
        # Register the changes in the GUI server.
        print('requesting uncommitted bundle loading of {} with the following '
              'services:\n  {}'.format(bundle, services))
        try:
            response = env.set_changes(bundle.serialize())
        except jujuclient.EnvError as err:
            raise ProgramExit(
                'bad API server response: {}'.format(err.message))
        errors = response.get('Errors')
        if errors:
            # This is unexpected, the bundle at this point is already validated
            # using juju bundle lib.
            msg = ', '.join(errors)
            raise ProgramExit('cannot generate the change set: {}'.format(msg))
        print('the bundle is ready to be deployed\n'
              'use the GUI to configure the bundle and to start the '
              'deployment process{}'.format(details))
    ...
    print('bundle deployment request accepted\nuse the GUI to check the '
          'bundle deployment progress{}'.format(details))
    return None
