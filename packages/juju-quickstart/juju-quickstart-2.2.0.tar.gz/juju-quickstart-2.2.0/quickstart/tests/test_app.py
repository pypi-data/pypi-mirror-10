...
from quickstart.models import bundles
...
class TestDeployGui(
    ...
    def test_deployment_with_options(self, mock_print):
        # The function correctly deploys the service with the given
        # configuration options in the case the service is not present in the
        # environment.
        env = self.make_env(unit_name='my-gui/42')
        service_data = unit_data = None
        options = {'port': 4242, 'sandbox': True}
        app.deploy_gui(
            env, 'my-gui', self.charm_url, '0', options,
            service_data, unit_data)
        env.deploy.assert_called_once_with(
            'my-gui', self.charm_url, num_units=0, config=options)
        self.assertEqual(6, mock_print.call_count)
        mock_print.assert_has_calls([
            mock.call('requesting my-gui deployment'),
            mock.call('using customized my-gui options:\n'
                      '  port="4242"\n  sandbox="True"'),
            mock.call('my-gui deployment request accepted'),
            mock.call('exposing service my-gui'),
            mock.call('requesting new unit deployment'),
    ...
    def test_existing_service_with_options(self, mock_print):
        # The deployment is executed reusing an already deployed service.
        # In this case, the provided Juju GUI options are ignored.
        env = self.make_env(unit_name='my-gui/42')
        service_data = self.make_service_data()
        unit_data = None
        options = {'port': 4242, 'sandbox': True}
    ...
    def test_existing_service_unexposed(self, mock_print):
        ...
        unit_data = None
        unit_name = app.deploy_gui(
            env, 'my-gui', self.charm_url, '1', self.options,
            service_data, unit_data)
        self.assertEqual('my-gui/42', unit_name)
        env.assert_has_calls([
            # The service has been exposed.
    ...
    def test_new_machine(self, mock_print):
        ...
        service_data = unit_data = None
        unit_name = app.deploy_gui(
            env, 'my-gui', self.charm_url, None, self.options,
            service_data, unit_data)
        self.assertEqual('my-gui/42', unit_name)
        env.assert_has_calls([
            # The service has been deployed.
    ...
    def test_deploy_error(self, mock_print):
        ...
        with self.assert_program_exit('bad API response: bad wolf'):
            app.deploy_gui(
                env, 'another-gui', self.charm_url, '0', self.options,
                service_data, unit_data)
        env.deploy.assert_called_once_with(
            'another-gui', self.charm_url, num_units=0, config=None)
    ...
    def test_other_errors(self, mock_print):
        ...
        with self.assertRaises(ValueError) as context_manager:
            app.deploy_gui(
                env, 'juju-gui', self.charm_url, '0', self.options,
                service_data, unit_data)
        env.deploy.assert_called_once_with(
            'juju-gui', self.charm_url, num_units=0, config=None)
...
class TestGetServiceConfig(ProgramExitTestsMixin, unittest.TestCase):

    def test_success(self):
        # The configuration options for the service are correctly returned.
        env = mock.Mock()
        env.get_config.return_value = {
            'port': {
                'description': 'Supply a different port',
                'type': 'int',
                'value': 8080,
            },
            'login-help': {
        ...
        expected_config = {'port': 8080, 'login-help': None, 'secure': False}
        config = app.get_service_config(env, 'django')
        self.assertEqual(expected_config, config)
        env.get_config.assert_called_once_with('django')

    def test_api_error(self):
        # A ProgramExit is raised if an error occurs while interacting with the
        # Juju API.
        env = mock.Mock()
        env.get_config.side_effect = self.make_env_error('bad wolf')
        with self.assert_program_exit('bad API server response: bad wolf'):
            app.get_service_config(env, 'my-service')


@helpers.mock_print
class TestDeployBundle(ProgramExitTestsMixin, unittest.TestCase):

    bundle_data = {'services': {'django': {}, 'haproxy': {}}}
    bundle = bundles.Bundle(bundle_data)
    charm = references.Reference.from_string('cs:trusty/juju-gui-42')

    def test_bundle_deployment(self, mock_print):
        # A bundle is successfully deployed.
        env, uncommitted = mock.Mock(), False
        token = app.deploy_bundle(env, self.bundle, uncommitted, self.charm)
        ...
        self.assertFalse(env.set_changes.called)
        self.assertFalse(env.close.called)
        self.assertEqual(2, mock_print.call_count)
        mock_print.assert_has_calls([
            mock.call(
                'requesting a deployment of bundle with the following '
                'services:\n  django, haproxy'),
            mock.call(
    ...
    def test_bundle_deployment_with_id(self, mock_print):
        ...
        app.deploy_bundle(env, bundle, uncommitted, self.charm)
        env.deploy_bundle.assert_called_once_with(
            self.bundle.serialize_legacy(), 3, bundle_id='django/single')

    def test_api_error(self, mock_print):
        # A ProgramExit is raised if an error occurs while deploying the
        # workload.
        env, uncommitted = mock.Mock(), False
        ...
        with self.assert_program_exit(expected_error):
            app.deploy_bundle(env, self.bundle, uncommitted, self.charm)

    def test_other_errors(self, mock_print):
        # Any other errors occurred during the bundle deployment process are
        # not trapped.
        env, uncommitted = mock.Mock(), False
        error = ValueError('explode!')
    ...
    def test_uncommitted_bundle_deployment(self, mock_print):
        # A bundle is successfully registered as an uncommitted set of changes
        # on the GUI server.
        env, uncommitted = mock.Mock(), True
        env.set_changes.return_value = {'Token': 'mytoken'}
        token = app.deploy_bundle(env, self.bundle, uncommitted, self.charm)
        self.assertEqual('mytoken', token)
        expected_yaml = yaml.safe_dump(self.bundle_data)
        env.set_changes.assert_called_once_with(expected_yaml)
        self.assertFalse(env.deploy_bundle.called)
        self.assertEqual(2, mock_print.call_count)
        mock_print.assert_has_calls([
            mock.call(
                'requesting uncommitted bundle loading of bundle with the '
                'following services:\n  django, haproxy'),
            mock.call(
                'the bundle is ready to be deployed\n'
                'use the GUI to configure the bundle and to start the '
                'deployment process'),
        ])

    def test_uncommitted_api_error(self, mock_print):
        # A ProgramExit is raised if an error occurs while registering the
        # change set to the GUI server.
        env, uncommitted = mock.Mock(), True
        env.set_changes.side_effect = self.make_env_error('bad wolf')
        with self.assert_program_exit('bad API server response: bad wolf'):
            app.deploy_bundle(env, self.bundle, uncommitted, self.charm)

    def test_uncommitted_bundle_validation_error(self, mock_print):
        # A ProgramExit is raised if the GUI server returns bundle validation
        # errors in its response. This is really unlikely to happen.
        env, uncommitted = mock.Mock(), True
        env.set_changes.return_value = {'Errors': ['error 1', 'error2']}
        expected_error = 'cannot generate the change set: error 1, error2'
        with self.assert_program_exit(expected_error):
            app.deploy_bundle(env, self.bundle, uncommitted, self.charm)

    def test_uncommitted_other_errors(self, mock_print):
        # Any other errors occurred during the change set process are not
        # trapped.
        env, uncommitted = mock.Mock(), True
        error = ValueError('explode!')
        env.set_changes.side_effect = error
        with self.assertRaises(ValueError) as context_manager:
            app.deploy_bundle(env, self.bundle, uncommitted, self.charm)
        self.assertIs(error, context_manager.exception)

    def test_uncommitted_not_supported(self, mock_print):
        # The uncommitted set of changes cannot be registered if the Juju GUI
        # service deployed in the environment does not support it.
        env, uncommitted = mock.Mock(), True
        charm = references.Reference.from_string('cs:trusty/juju-gui-0')
        expected_logs = [
            'uncommitted bundles loading is not supported by currently '
            'deployed Juju GUI charm (cs:trusty/juju-gui-0): requested bundle '
            'deployment will be ignored',
            'run "juju upgrade-charm juju-gui" to enable uncommitted bundles '
            'support',
        ]
        with helpers.assert_logs(expected_logs, level='warn'):
            token = app.deploy_bundle(env, self.bundle, uncommitted, charm)
        self.assertIsNone(token)
        # No calls to deploy the bundle have been made.
        self.assertFalse(env.deploy_bundle.called)
        self.assertFalse(env.set_changes.called)
        self.assertFalse(mock_print.called)
