...
# TODO Specific version of pyzmq because the latest does not play well with
# aiozmq 0.6.0. Fixed in aiozmq
# <https://github.com/aio-libs/aiozmq/commit/dcdf4c8> so just wait for a new
# aiozmq release.
...
# TODO Specific versions of aiopg, msgpack-python & psycopg2 because the latest
# fail when the broker wants to know whether a list is empty; it receives the
# '{None}' string instead of an empty list.
# Code ref: xbus/broker/core/back/rpc.py:298.
# As the problem has not yet been identified, the 3 packages are frozen.
