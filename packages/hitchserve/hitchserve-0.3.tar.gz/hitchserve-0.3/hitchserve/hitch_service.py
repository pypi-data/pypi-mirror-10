...
class Service(object):
    ...
    def __init__(self, command=None, log_line_ready_checker=None, directory=None, no_libfaketime=False, env_vars=None, needs=None):
        ...
        if needs is not None:
            if type(needs) != list:
                raise HitchServiceException(
                    "Service was initialized with a needs property that"
                    "was not None or a list of service objects."
                )
            for need in needs:
                if type(need) != Service:
                    raise HitchServiceException(
                        "Service was initialized with a needs list that"
                        "does not contain Service objects."
                    )
