...
class Mail(object):
    ...
    def send(self, message, endpoint=None):
        ...
        endpoint = endpoint or self.config.MAIL_DEFAULT_ENDPOINT

        try:
            self.bus.push(self.config.MAIL_QUEUE_NAME, self.__to_dict(message), endpoint=endpoint)
    ...
    def __to_dict(message):
        ...
        if message.body:
            d['body'] = message.body
            d['is_body_html'] = message.is_body_html
