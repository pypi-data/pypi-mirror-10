...
class PylotAuthomatic(object):
    ...
    def init_app(self, app):
        ...
        Pylot.set_context__(AUTH_PROVIDERS=auth_providers)
...
auth = PylotAuthomatic()
Pylot.bind(auth.init_app)
...
def user_account(model, **kwargs):
    ...
    def wrapper(view):
        ...
        Pylot.set_context__(COMPONENT_LOGIN=True)
        ...
        template_dir = kwargs["template_dir"] if "template_dir" \
                                                 in kwargs else "Pylot/UserAccount"
        template_page = template_dir + "/%s.html"

        login_manager = LoginManager()
        login_manager.login_view = login_view
        login_manager.login_message_category = "error"
        Pylot.bind(login_manager.init_app)
