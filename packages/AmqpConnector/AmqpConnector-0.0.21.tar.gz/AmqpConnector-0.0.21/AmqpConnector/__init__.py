...
class Connector:
	...
	def _setupQueues(self):
		...
		if not self.master:
			...
			self.channel.queue_bind(self.consumer_q, exchange=self.task_exchange, routing_key=self.consumer_q.split(".")[0])
	...
	def _publishOutgoing(self):
			...
			while 1:
				try:
					...
					self.channel.basic_publish(message, exchange=self.response_exchange, routing_key=self.response_q.split(".")[0])
					self.active -= 1
