...
class Interface(parabam.core.Interface):
	...
	def run(self,input_paths,out_path,total_procs,
			task_size,keep,reader_n,verbose=False,announce=False):
		...
		out_files = telbam_interface.run(input_paths=input_paths,
							...
							reader_n=reader_n,
							verbose=verbose,
							keep_in_temp=(not keep))

		length_verbose = False
		if not verbose == 0:
			length_verbose = True
	...
	def get_parser(self):
		...
		parser.add_argument('--out',metavar='PATH',type=str,nargs='?',default="",
			help='The desired output path. Automatically generated if left blank')
