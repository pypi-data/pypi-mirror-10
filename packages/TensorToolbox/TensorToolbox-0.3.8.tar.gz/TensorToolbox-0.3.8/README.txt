...
TensorToolbox now stores data in both `cPickle <https://docs.python.org/2/library/pickle.html>`_ files and `hd5 <http://www.hdfgroup.org/>`_ through the python package `h5py <http://www.h5py.org/>`_. The package depends on `Cython <https://pypi.python.org/pypi/Cython/>`_ and requires to link to an mpi backend, and find the file `mpi.h`. In order to manually solve the dependencies do:
