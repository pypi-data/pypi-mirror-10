...
std::vector<double> stetmesh::Tetmesh::getBatchTetBarycentres(std::vector<uint> const & tets) const
{
    ...
    std::vector<double> data(ntets * 3, 0.0);
    ...
    for (uint t = 0; t < ntets; t++) {
        ...
        uint output_pos = t*3;
        uint source_pos = tidx*3;
        data[output_pos] = pTet_barycentres[source_pos];
        data[output_pos + 1] = pTet_barycentres[source_pos+1];
        data[output_pos + 2] = pTet_barycentres[source_pos+2];
    }
...
void stetmesh::Tetmesh::getBatchTetBarycentresNP(unsigned int* indices, int input_size, double* centres, int output_size) const
{
    assert(pSetupDone == true);
    if (input_size * 3 != output_size) {
        std::ostringstream os;
        os << "Error: Length of output array should be 3 * leng of input array.\n";
        throw steps::ArgErr(os.str());
    }
    ...
    for (int t = 0; t < input_size; t++) {
        ...
        uint output_pos = t*3;
        uint source_pos = tidx*3;
        centres[output_pos] = pTet_barycentres[source_pos];
        centres[output_pos + 1] = pTet_barycentres[source_pos+1];
        centres[output_pos + 2] = pTet_barycentres[source_pos+2];
    }
...
std::vector<double> stetmesh::Tetmesh::getBatchTriBarycentres(std::vector<uint> const & tris) const
{
    ...
    std::vector<double> data(ntris * 3, 0.0);
    ...
    for (uint t = 0; t < ntris; t++) {
        ...
        uint output_pos = t*3;
        uint source_pos = tidx*3;
        data[output_pos] = pTri_barycs[source_pos];
        data[output_pos + 1] = pTri_barycs[source_pos+1];
        data[output_pos + 2] = pTri_barycs[source_pos+2];
    }
...
void stetmesh::Tetmesh::getBatchTriBarycentresNP(unsigned int* indices, int input_size, double* centres, int output_size) const
{
    assert(pSetupDone == true);
    if (input_size * 3 != output_size) {
        std::ostringstream os;
    ...
    for (int t = 0; t < input_size; t++) {
        ...
        uint output_pos = t*3;
        uint source_pos = tidx*3;
        centres[output_pos] = pTri_barycs[source_pos];
        centres[output_pos + 1] = pTri_barycs[source_pos+1];
        centres[output_pos + 2] = pTri_barycs[source_pos+2];
    }
...
std::vector<double> stetmesh::Tetmesh::getBatchVertices(std::vector<uint> const & verts) const
{
    ...
    std::vector<double> data(nverts * 3, 0.0);
    ...
    for (uint v = 0; v < nverts; v++) {
        ...
        {
            std::ostringstream os;
            os << "Error (Index Overbound): There is no vertex with index " << vidx << ".\n";
            throw steps::ArgErr(os.str());
        }
        
        uint output_pos = v*3;
        uint source_pos = vidx*3;
        data[output_pos] = pVerts[source_pos];
        data[output_pos + 1] = pVerts[source_pos+1];
        data[output_pos + 2] = pVerts[source_pos+2];
    }
...
void stetmesh::Tetmesh::getBatchVerticesNP(unsigned int* indices, int input_size, double* coordinates, int output_size) const
{
    assert(pSetupDone == true);
    
    if (input_size * 3 != output_size) {
        std::ostringstream os;
        os << "Error: Length of output array (coordinates) should be 3 * leng of input array (indices).\n";
        throw steps::ArgErr(os.str());
    }
    
    for (int v = 0; v < input_size; v++) {
        uint vidx = indices[v];
        ...
        uint output_pos = v*3;
        uint source_pos = vidx*3;
        coordinates[output_pos] = pVerts[source_pos];
        coordinates[output_pos + 1] = pVerts[source_pos+1];
        coordinates[output_pos + 2] = pVerts[source_pos+2];
    }
...
std::vector<uint> stetmesh::Tetmesh::getBatchTris(std::vector<uint> const & tris) const
{
    ...
    for (uint t = 0; t < ntris; t++) {
        ...
        uint output_pos = t*3;
        uint source_pos = tidx*3;
        data[output_pos] = pTris[source_pos];
        data[output_pos + 1] = pTris[source_pos+1];
        data[output_pos + 2] = pTris[source_pos+2];
    }
...
void stetmesh::Tetmesh::getBatchTrisNP(unsigned int* t_indices, int input_size, unsigned int* v_indices, int output_size) const
{
    assert(pSetupDone == true);
    ...
    for (int t = 0; t < input_size; t++) {
        ...
        uint output_pos = t*3;
        uint source_pos = tidx*3;
        v_indices[output_pos] = pTris[source_pos];
        v_indices[output_pos + 1] = pTris[source_pos+1];
        v_indices[output_pos + 2] = pTris[source_pos+2];
    }
}
...
std::vector<uint> stetmesh::Tetmesh::getBatchTets(std::vector<uint> const & tets) const
{
    ...
    for (uint t = 0; t < ntets; t++) {
        ...
        uint output_pos = t*4;
        uint source_pos = tidx*4;
        data[output_pos] = pTets[source_pos];
        data[output_pos + 1] = pTets[source_pos+1];
        data[output_pos + 2] = pTets[source_pos+2];
        data[output_pos + 3] = pTets[source_pos+3];
    }
...
void stetmesh::Tetmesh::getBatchTetsNP(unsigned int* t_indices, int input_size, unsigned int* v_indices, int output_size) const
{
    assert(pSetupDone == true);
    
    if (input_size * 4 != output_size) {
        std::ostringstream os;
        os << "Error: Length of output array should be 4 * leng of input array.\n";
        throw steps::ArgErr(os.str());
    }
    ...
    for (int t = 0; t < input_size; t++) {
        ...
        uint output_pos = t*4;
        uint source_pos = tidx*4;
        v_indices[output_pos] = pTets[source_pos];
        v_indices[output_pos + 1] = pTets[source_pos+1];
        v_indices[output_pos + 2] = pTets[source_pos+2];
        v_indices[output_pos + 3] = pTets[source_pos+3];
    }
}

////////////////////////////////////////////////////////////////////////////////

uint stetmesh::Tetmesh::getTriVerticesSetSizeNP(unsigned int* t_indices, int input_size) const
{
    assert(pSetupDone == true);
    ...
    for (int t = 0; t < input_size; t++) {
        ...
        std::vector<uint> indices = getTri(tidx);
...
uint stetmesh::Tetmesh::getTetVerticesSetSizeNP(unsigned int* t_indices, int input_size) const
{
    assert(pSetupDone == true);
    ...
    for (int t = 0; t < input_size; t++) {
        ...
        std::vector<uint> indices = getTet(tidx);
        unique_indices.insert(indices.begin(), indices.end());
    }
...
void stetmesh::Tetmesh::getTriVerticesMappingSetNP(unsigned int* t_indices, int input_size, unsigned int* t_vertices, int t_vertices_size, unsigned int* v_set, int v_set_size) const
{
    assert(pSetupDone == true);
    
    if (input_size * 3 != t_vertices_size) {
        std::ostringstream os;
        os << "Error: Length of t_vertices array should be 3 * leng of input array.\n";
        throw steps::ArgErr(os.str());
    }
    
    std::map<uint, uint> v_set_map;
    uint current_vset_index = 0;
    uint current_t_vertices_index = 0;
    ...
    for (int t = 0; t < input_size; t++) {
        ...
        for (uint v = 0; v < 3; v++) {
            uint vidx = indices[v];
            if (v_set_map.find(vidx) != v_set_map.end()) {
                t_vertices[current_t_vertices_index] = v_set_map[vidx];
            }
            else {
                v_set_map[vidx] = current_vset_index;
                v_set[current_vset_index] = vidx;
                t_vertices[current_t_vertices_index] = current_vset_index;
                current_vset_index++;
            }
            current_t_vertices_index++;
        }
    }
    
    assert(current_vset_index == v_set_size);
    assert(current_t_vertices_index == t_vertices_size);
}

////////////////////////////////////////////////////////////////////////////////

void stetmesh::Tetmesh::getTetVerticesMappingSetNP(unsigned int* t_indices, int input_size, unsigned int* t_vertices, int t_vertices_size, unsigned int* v_set, int v_set_size) const
{
    assert(pSetupDone == true);
    
    if (input_size * 4 != t_vertices_size) {
        std::ostringstream os;
        os << "Error: Length of t_vertices array should be 4 * leng of input array.\n";
        throw steps::ArgErr(os.str());
    }
...
void stetmesh::Tetmesh::genPointsInTet(unsigned tidx, unsigned npnts, double* cords, int cord_size) const
{
    if (npnts * 3 != cord_size) {
        std::ostringstream os;
        os << "Error: Coordinate array size should be 3 * npnts.\n";
        throw steps::ArgErr(os.str());
    }
    if (tidx >= pTetsN) {
        std::ostringstream os;
    ...
    for (uint p = 0; p < npnts; p++) {
        ...
        for (uint c = 0; c < 3; c++) {
            ...
            cords[p * 3 + c] = v0[c] * a + v1[c] * s + v2[c] * t + v3[c] * u;
        }
    }
}
...
void stetmesh::Tetmesh::genPointsInTri(unsigned tidx, unsigned npnts, double* cords, int cord_size) const
{
    ...
    for (uint p = 0; p < npnts; p++) {
        ...
        for (uint c = 0; c < 3; c++) {
            ...
            cords[p * 3 + c] = (1 - sqrt_s) * v0[c] + (sqrt_s * (1-t)) * v1[c] + (t * sqrt_s) * v2[c];
        }
    }
}
...
void stetmesh::Tetmesh::genTetVisualPointsNP(unsigned int* indices, int index_size, unsigned int* point_counts, int count_size, double* cords, int cord_size) const
{
    if (index_size != count_size) {
        std::ostringstream os;
        os << "Error: Index array should be the same size as Count array.\n";
        throw steps::ArgErr(os.str());
    }
    ...
    for (uint i = 0; i < index_size; i++) {
        ...
        for (uint p = 0; p < npnts; p++) {
            ...
            for (uint c = 0; c < 3; c++) {
                ...
                cords[current_pos + c] = v0[c] * a + v1[c] * s + v2[c] * t + v3[c] * u;
            }
            current_pos += 3;
        }
    ...
    assert(current_pos == cord_size);
}

////////////////////////////////////////////////////////////////////////////////

void stetmesh::Tetmesh::genTriVisualPointsNP(unsigned int* indices, int index_size, unsigned int* point_counts, int count_size, double* cords, int cord_size) const
{
    if (index_size != count_size) {
        std::ostringstream os;
        os << "Error: Index array should be the same size as Count array.\n";
    ...
    for (uint i = 0; i < index_size; i++) {
        ...
        for (uint p = 0; p < npnts; p++) {
            ...
            for (uint c = 0; c < 3; c++) {
                cords[current_pos + c] = (1 - sqrt_s) * v0[c] + (sqrt_s * (1-t)) * v1[c] + (t * sqrt_s) * v2[c];
            }
...
void stetmesh::Tetmesh::getBatchTetVolsNP(unsigned int* indices, int index_size, double* volumes, int volume_size) const
{
    if (index_size != volume_size) {
        std::ostringstream os;
        os << "Error: Index array should be the same size as Volume array.\n";
        throw steps::ArgErr(os.str());
    }
...
void stetmesh::Tetmesh::getBatchTriAreasNP(unsigned int* indices, int index_size, double* areas, int area_size) const
{
    if (index_size != area_size) {
        std::ostringstream os;
        os << "Error: Index array should be the same size as Area array.\n";
        throw steps::ArgErr(os.str());
    }
    ...
    for (uint i = 0; i < index_size; i++) {
        ...
        areas[i] = pTri_areas[tidx];
    }
}

////////////////////////////////////////////////////////////////////////////////

void stetmesh::Tetmesh::reduceBatchTetPointCountsNP(unsigned int* indices, int index_size, unsigned int* point_counts, int count_size, double max_density)
{
    if (index_size != count_size) {
        std::ostringstream os;
        os << "Error: Index array should be the same size as Count array.\n";
    ...
    for (uint i = 0; i < index_size; i++) {
        ...
        uint npoints = max_density * pTet_vols[tidx];
        if (point_counts[i] > npoints) {
            point_counts[i] = npoints;
        }
    }
}
...
void stetmesh::Tetmesh::reduceBatchTriPointCountsNP(unsigned int* indices, int index_size, unsigned int* point_counts, int count_size, double max_density)
{
    if (index_size != count_size) {
        std::ostringstream os;
        os << "Error: Index array should be the same size as Count array.\n";
    ...
    for (uint i = 0; i < index_size; i++) {
        ...
        uint npoints = max_density * pTri_areas[tidx];
        if (point_counts[i] > npoints) {
            point_counts[i] = npoints;
        }
    }
...
////////////////////////////////////////////////////////////////////////////////
//
//std::vector<int> stetmesh::Tetmesh::getTetsTetNeighbSet(std::vector<uint> const & t_indices) const
//{
//    assert(pSetupDone == true);
//    std::set<int> neighbor_list;
//    neighbor_list.insert(-1);
//    
//    uint n_indices = t_indices.size();
//    
//    for (uint n = 0; n < n_indices; n++) {
//        int tidx = t_indices[n];
//        if (tidx >= pTetsN)
//        {
//            std::ostringstream os;
//            os << "Tetrahedron index out of range.\n";
//            throw steps::ArgErr(os.str());
//        }
//        neighbor_list.insert(pTet_tet_neighbours[tidx*4]);
//        neighbor_list.insert(pTet_tet_neighbours[(tidx*4)+1]);
//        neighbor_list.insert(pTet_tet_neighbours[(tidx*4)+2]);
//        neighbor_list.insert(pTet_tet_neighbours[(tidx*4)+3]);
//    }
//    neighbor_list.erase(-1);
//    std::vector<int> output(neighbor_list.begin(), neighbor_list.end());
//    return output;
//}
...
void stetmesh::Tetmesh::genROITetVisualPointsNP(std::string ROI_id, unsigned int* point_counts, int count_size, double* cords, int cord_size) const
{
    ...
    genTetVisualPointsNP(indices, inputsize, point_counts, count_size, cords, cord_size);
}

////////////////////////////////////////////////////////////////////////////////

void stetmesh::Tetmesh::genROITriVisualPointsNP(std::string ROI_id, unsigned int* point_counts, int count_size, double* cords, int cord_size) const
{
    ...
    genTriVisualPointsNP(indices, inputsize, point_counts, count_size, cords, cord_size);
}
