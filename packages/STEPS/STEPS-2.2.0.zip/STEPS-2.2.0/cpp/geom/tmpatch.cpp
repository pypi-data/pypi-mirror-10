...

{
	...
	uint * tri = pTetmesh->_getTri(pTri_indices[0]);
	// initialise min and max x coordinates with first x coordinate of first tet
	pXmin = pTetmesh->_getVertex(tri[0])[0];
	pXmax = pTetmesh->_getVertex(tri[0])[0];
	// initialise min and max y coordinates with first y coordinate of first tet
	pYmax = pTetmesh->_getVertex(tri[0])[1];
	pYmin = pTetmesh->_getVertex(tri[0])[1];
	// initialise min and max z coordinates with first z coordinate of first tet
	pZmin = pTetmesh->_getVertex(tri[0])[2];
	pZmax = pTetmesh->_getVertex(tri[0])[2];
	for (uint i=0; i< pTrisN; ++i)
	{
		uint * tri = pTetmesh->_getTri(pTri_indices[i]);
		// compare each vertex to current values
		for (uint j=0; j<3; ++j)
		{
			double xtemp = pTetmesh->_getVertex(tri[j])[0];
			if (xtemp < pXmin) pXmin = xtemp;
			if (xtemp > pXmax) pXmax = xtemp;
			double ytemp = pTetmesh->_getVertex(tri[j])[1];
			if (ytemp < pYmin) pYmin = ytemp;
			if (ytemp > pYmax) pYmax  = ytemp;
			double ztemp = pTetmesh->_getVertex(tri[j])[2];
