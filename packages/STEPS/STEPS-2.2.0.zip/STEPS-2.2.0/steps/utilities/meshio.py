...
#  Last Changed Rev:  $Rev: 529 $
#  Last Changed Date: $Date: 2014-04-16 11:53:31 +0900 (Wed, 16 Apr 2014) $
...
def loadMesh(pathname):
    ...
    * patches
    ...
    if info.find("ROI") != -1:
        ROIinfo = info
        assert(ROIinfo.__len__() > 23)
        assert(ROIinfo[-2:] == '">')
        nROI = int(ROIinfo[21:-2])
        
        for r in range(nROI):
            ROItemp = xmlfile.readline().strip()
            assert(ROItemp.find("ROI") != -1)
            assert(ROItemp.find("id =") != -1)
            assert(ROItemp.find("type =") != -1)
            id = ROItemp.split('"')[1]
            type = int(ROItemp[ROItemp.rfind("=") + 1: ROItemp.rfind(">")])
            idxtemp = xmlfile.readline().strip()
