...
(sqrts). The Process interface supplies a convenient method for filtering the
...
>>> d.xsections.get_xsecs(sqrts=13000., kappa_r=2., code='Prospino')
...
class XSec(object):
    ...
    def __cmp__(self, other):
        # TODO: Is there a more compact/automatic version way to do cmp on all attrs?
        veca = [self.sqrts, self.scale_scheme, self.qcd_order, self.ew_order, self.kappa_f, self.kappa_r, self.pdf_id, self.value, self.code]
        vecb = [other.sqrts, other.scale_scheme, other.qcd_order, other.ew_order, other.kappa_f, other.kappa_r, other.pdf_id, other.value, other.code]
        return cmp(veca, vecb)
...
class Process(object):
    ...
    @property
    def sqrtses(self):
        # TODO: could just use an inline sorted set literal, but that would require a high Python 2.x version
        rtn = []
        for x in self.xsecs:
            if x.sqrts in rtn:
                continue
    ...
    def __cmp__(self, other):
        if self.pidsinitial != other.pidsinitial:
            return cmp(self.pidsinitial, other.pidsinitial)
        if self.pidsfinal != other.pidsfinal:
            return cmp(self.pidsfinal, other.pidsfinal)
        return cmp(self.xsecs, other.xsecs)
    ...
    for line in spcstr.splitlines():
        ...
        if not re_indented.match(line):
            ...
            ## Handle XSECTION start lines
                ...
                xsections.setdefault(_autotuple(currentproc), Process([pdgid1,pdgid2], fspdgids)) #< There might be more than one energy so we need to combine
...
def writeSLHAXSections(xsections, precision=8):
    ...
    sep = 3 * " "
    blockstrs = []
    for _, proc in xsections.iteritems():
