...
def getArgs(argv=None):
	...
	parser.add_argument('--no_lefse', dest="nolefse", action="store_false", help='If you only want subsetted tables and no formatted or lefse-run data, add this option to the command.')
...
# Main function
def convert2Lefse(input, output, map, level, subjectID, classID, subclassID, compare, split_by, nolefse, anova_cutoff, lda_cutoff, strictness):
	...
	for split in split_by_values:
		...
		# Only Run lefse if the option no_lefse was not chosen
		if nolefse==True:
			...
			run_file_out = run_lefse_out + os.path.basename(format_file_out).replace('_format.txt', '_run_output.txt')
...
if __name__ == "__main__":
	...
	# Run!
	convert2Lefse(args.input, args.output, args.mapping, str(args.level), str(args.subjectID), str(args.classID), str(args.subclassID), str(args.compare), str(args.splitby), args.nolefse, float(args.anova_cutoff), float(args.lda_cutoff), int(args.strictness))
