...
class ItemState(object):
    ...
    def restore(self, restore_method, method_argument, maintain_validity=False):
        ...
        if self.should_change_state():
            ...
            if maintain_validity:
                ...
                item_metadata = self.item.rest_interface.generic_get_meta(restore_path)

                self.item._initialize_self(item_metadata)
    ...
    def __str__(self):
        states = [
            (self.share_key,'Share'),
            (self.in_trash,'Trash'),
            (self.old_version,'Old'),
            (self.dead,'Dead')
        ]
        return '[{}]'.format('|'.join([s[1] for s in states if s[0]]))
