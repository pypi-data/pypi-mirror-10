...
class File(Item):
    ...
    # keys used in change_attributes
    mime_key = 'mime'
    ...
    def move(self, dest, exists=ExistValues.rename, debug=False):
        ...
        self._initialize_self(result['meta'])
    ...
    def copy(self, dest, name=None, exists=ExistValues.rename, debug=False):
        ...
        :param name:        Name for copy. Defaults to the name of the item being copied.
        :param exists:      How to handle if an item of the same name exists in the destination folder. Defaults to rename.
        :param debug:       If true, will print the the request and response to stdout.
        ...
        set_debug(self, debug)
        result = self.rest_interface.copy_file(self.path, dest, name, exists)
    ...
    def mime(self, new_mime):
        self._set(self.mime_key, new_mime)
