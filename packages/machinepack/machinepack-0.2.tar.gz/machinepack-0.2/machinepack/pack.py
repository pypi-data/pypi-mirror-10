...
class MachinePackManager(object):
    def __init__(self, directory=None):
        ...
        if not self.machinepack_name.startswith('machinepack_'):
            return

        try:
            sys.path.insert(0, os.getcwd())
            self.package_config = importlib.import_module('machinepack_config').CONFIG

            self.machinepack_config = {}
            self.machine_class = self.machinepack_name.title()

            self.modules_directory = os.path.join(os.getcwd(), self.machinepack_name)
    ...
    def __getattr__(self, item):
        ...
        machine_name = item
        imported_module = importlib.import_module('%s.%s' % (self.machinepack_name, machine_name))
    ...
    def get_machine_meta(self, machine):
        if machine not in self.machines_list:
            return None
        return importlib.import_module('%s.%s' % (self.machinepack_name, machine)).meta
