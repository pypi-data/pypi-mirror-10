...
from hgsvn.common import (
    ...
    , get_hg_cset, strip_hg_rev, strip_hg_revid
...
def hg_push_branch(options, hg_parent, use_branch = None, svn_base = None):
            ...
            if use_branch is None:
                branchname, BranchSVNRev = get_branchinfo_from_hg(from_hgrev)
...
def real_main(options, args, clean = True):
        ...
        if options.prefere2hg: # and not options.tip_rev
            ...
            if hg_is_clean(orig_branch) or svn_is_clean(wc_base):
                ...
                if orig_branch != svn_branch:
                    ...
                    if IsDebug():
                        ui.status('differ branch from hg:% svn:%s@%s'%(orig_branch, svn_branch, str(svn_syncrev)), level=ui.DEBUG)
                ...
                if not (hg_syncrev is None):
                        from_hgrev = strip_hg_rev(hg_syncrev)
                        if int(from_hgrev) > 0:
                            res = hg_push_branch(options, hg_syncrev)
                            if res != 0:
                                return res
                            continue
                        else:
    ...
    if not options.collapse:
            hg_revs,hg_tree = get_hg_revs_with_graph(hg_start_cset, svn_branch, last_rev=options.tip_rev, opts = options)
