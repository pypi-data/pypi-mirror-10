...
def svn_switch_to(svn_base, rev_number = None, clean = False, ignore_ancetry=False, propagate_rev = False):
    ...
    try:
        ...
        if not (delpath is None):
            ...
            (rev_base, rev_origin, rev_head) = svn_branch_revset(svn_base)
            if (rev_base == rev_head) and (rev_origin == rev_number):
                basehgid = get_hg_cset_of_svn(svnrev)
                if (len(basehgid) == 0):
...
def svn_branch_revset(svn_base):
    base   = get_first_svn_log_entry('.', 0, 'HEAD')
    head   = get_last_svn_log_entry('.', 0, 'HEAD')
    ...
    return baserev, source, head['revision']

def svn_branch_empty(svn_base):
    entryes = run_svn_log('.', 0, 'HEAD', 2, stop_on_copy=True)
    if len(entryes) > 1:
        return False
    base = entryes[0]
