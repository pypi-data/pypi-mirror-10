...
# Default text styles added by pod in content.xml
podFolder = os.path.dirname(appy.pod.__file__)
f = file('%s/content.xmlt' % podFolder)
...
f = file('%s/styles.xmlt' % podFolder)
...
class Renderer:
    ...
    def finalize(self):
        ...
        f = file(contentXml)
        # Insert dynamic styles. Default styles for bulleted and numbered lists
        # are included.
        ds = self.dynamicStyles
        env = self.stylesParser.env
        ns = {'text': env.ns(env.NS_TEXT), 'style': env.ns(env.NS_STYLE)}
        ds.insert(0, NumberedProperties().dumpStyle('podNumberedList', ns))
        ds.insert(0, BulletedProperties().dumpStyle('podBulletedList', ns))
