...
class ToolMixin(BaseMixin):
    ...
    def getErrorPage(self, options):
        ...
        if errorType == 'NotFound':
            ...
            # Return the standard error page containing the traceback (if the
            # user is an admin).
    ...
    def getPxTarget(self, zobj, px):
        ...
        if len(px) == 3:
            className, name, pxName = px
        elif len(px) == 2:
            name, pxName = px
            className = None
        else: return # Only the PX name: the PX is not field or search-related
        # "name" can refer to a field or search. If no class name is specified,
        # it can only be a field.
        if not className: return zobj.getAppyType(name)
        # Get the search or field
        return zobj.getAppyType(name, className) or \
               self.getSearch(className, name, ui=True)
    ...
    def performLogin(self):
        ...
        if jsEnabled and not cookiesEnabled:
            ...
            return self.goto(rq['HTTP_REFERER'], msg)
        ...
        if not success:
            backUrl = self.getSiteUrl()
            if gotoUrl: backUrl += '?goto=%s' % urllib.quote(gotoUrl)
        else:
            backUrl = gotoUrl or self.getSiteUrl()
            if gotoUrl: msg = None # No need to get the welcome message here
