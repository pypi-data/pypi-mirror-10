...
class Ref(Field):
    ...
    pxObjectTitle = Px('''
     <x var="navInfo=field.getNavInfo(zobj, startNumber + currentNumber, \
             ...
             titleMode=selector and 'select' or 'link';
    ...
    pxLink = Px('''
     <a target="appyIFrame"
        ...
        href=":field.getPopupLink(obj, tiedClassName, popupMode)">
      <input type="button"
    ...
    def __init__(self, klass=None, attribute=None, validator=None,
                 ...
                 checkboxes=True, checkboxesDefault=False, sdefault='',
    ...
    def getPopupLink(self, obj, tiedClassName, popupMode):
        ...
        if self.link == 'popup':
            ...
            res = '%s/query?className=%s&search=%s*%s*%s&popup=1' % \
                  (obj.tool.url, tiedClassName, obj.id, self.name, popupMode)
        elif self.link == 'popupRef':
            # Go to the page that displays a single field
            popupObj, fieldName = self.select(obj)
            res = '%s/field?name=%s&navStrip=0&popup=1&selector=%s*%s*%s' % \
                  (popupObj.url, fieldName, obj.id, self.name, popupMode)
        return res
    ...
    def getPopupObjects(self, obj, rq, requestValue):
        ...
        if rq['semantics'] == 'checked':
            ...
            if self.link == 'popup':
                ...
                initiatorObj, name = self.select(obj)
                linkUids = initiatorObj.ids(name)
            for uid in linkUids:
                if uid not in uids:
    ...
    def onSelectFromPopup(self, obj):
        '''This method is called on Ref fields with link=popup or popupRef, when
