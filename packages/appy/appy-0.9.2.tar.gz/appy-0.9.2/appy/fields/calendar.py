...
class Action:
    '''An action represents a custom method that can be executed, based on
       ...
       cells will become selectable: the selected cells wil be available to the
       action.
...
class Calendar(Field):
    ...
    pxActions = Px('''
     ...
     <input for="action in actions" type="button" value=":_(action.label)"
            var2="id='%s_%s' % (field.name, action.name)" id=":id"/>
     ''')
    ...
    pxView = pxCell = Px('''
     <div var="defaultDate=field.getDefaultDate(zobj);
               ...
               actions=field.getVisibleActions(obj)"
          id=":ajaxHookId">
    ...
    def __init__(self, eventTypes=None, eventNameMethod=None,
                 ...
                 selectableEmptyCells=False, view=None, xml=None, delete=True):
    ...
    def getVisibleActions(self, obj):
        '''Return the visible actions among self.actions'''
        return [action for action in self.actions if action.show]
# ------------------------------------------------------------------------------
