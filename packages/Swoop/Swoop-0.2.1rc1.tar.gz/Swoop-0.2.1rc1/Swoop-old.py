...
class Library (EagleFilePart):
    ...
    def remove_deviceset(self, deviceset):
        if type(deviceset) == str:
            ...
            raise SwoopError("Wrong type of argument to remove_deviceset()")

        if name not in self.devicesets:
            raise SwoopError("Deviceset '" + name +"' is not in library '" + self.name + "'")
    ...
    def remove_symbol(self, symbol):
        if type(symbol) == str:
            ...
            raise SwoopError("Wrong type of argument to remove_symbol()")

        if name not in self.symbols:
            raise SwoopError("Symbol '" + name +"' is not in library '" + self.name + "'")
    ...
    def remove_package(self, package):
        if type(package) == str:
            ...
            raise SwoopError("Wrong type of argument to remove_package()")

        if name not in self.packages:
            raise SwoopError("Package '" + name +"' is not in library '" + self.name + "'")
...
class Part (EagleFilePart):
    ...
    def check_sanity(self):
        ...
        try:
            ...
            raise SwoopError("DeviceSet '" + self.get_library().name + ":" + self.deviceset + "' missing for " + str(self.name))

        try:
            assert self.get_device() is not None
        except Exception as e:
            raise SwoopError("Device '" + self.get_library().name + ":" + self.get_deviceset().name + ":" + self.device + "' missing for " + str(self.name))
        ...
        for i in root.findall("./attribute"):
            if i.get("name") in part.attributes:
                if part.attributes[i.get("name")].constant:
                    raise SwoopError("Tried to set constant attribute '" 
                                         + i.name + "' on part '" 
                                         + part.name + "'.")
    ...
    def check_sanity(self):
        ...
        for g in self.gates.values():
            if g.symbol not in self.parent.symbols:
                raise SwoopError("Symbol '" + g.symbol + "' missing in '" + self.parent.name + ":" + self.name + "'")
