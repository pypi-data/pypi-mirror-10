...
class Command(BaseCommand):
    ...
    */30 * * * * cd /path/to/rgallery-project/ ; source env/bin/activate ; \
    ...
    # If there is a RGALLERY_HOMETAG, set is as the default tag importing photos
    try:
    ...
    option_list = BaseCommand.option_list + (
        ...
        make_option(
            '--source',
            dest='source',
            default=conf.RGALLERY_MEDIASYNCDIR,
        ...
        make_option(
            ...
            dest='thumbs',
            default=False,
            help='Regenerate thumbnails for all the stored photos'
        ),
    )
    ...
    def handle(self, *app_labels, **options):
        ...
        print "-"*80
        if options['storage']:
                print "[WWW] Photos are being imported from [%s] storage..." % \
                    options['storage']
        if options['source']:
                print "[WWW] Photos are being imported from [%s] source..." % \
                    options['source']
        if options['tags']:
