...
# -----------------------------------------------------------------------------
def keep_leaf(graph, leaf, include_items):
    try:
        return graph(leaf, "Asset") in include_items
...
class SubPortfolio(UfoBase):
    """
    Based on a reference portfolio (RefPortfolio), this class represents a
    sub-portfolio comprised of all leaves whose asset is present in a
    reference category object (RefCategory).
    Typical use is in the calculation of P&L attributions.
    """
    ...
    def Leaves(self, graph):
        include_items = graph(graph(self, "RefCategory"), "Items")
        leaves = Structure()
        ...
        return Structure({leaf: qty for leaf, qty in leaves.items()
                          if keep_leaf(graph, leaf, include_items)})
