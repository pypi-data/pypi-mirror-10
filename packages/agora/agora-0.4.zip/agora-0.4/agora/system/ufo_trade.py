...
from onyx.core import(Structure, DelObj, CreateInMemory, ObjDbTransaction,
                      ObjDbQuery, GraphNodeVt, GetVal, UfoBase, ReferenceField,
                      IntField, FloatField, DateField, SelectField)
...
TRANSACTIONS_STORED = """
SELECT Event, Transaction FROM (
    SELECT Event, Transaction, COUNT(Event) AS num from PosEffects
    WHERE Book=%s AND Trade=%s AND TradeDate BETWEEN %s AND %s
    GROUP BY Event, Transaction)
AS grouped WHERE MOD(grouped.num, 2)=1;
"""
...
class Trade(UfoBase):
    ...
    def TradeInfo(self, graph):
        ...
        attributes that can be altered by a trade amendment.
        """
        attrs = self._json_fields.copy()
        attrs = attrs.difference({"TradeDate"})
        return dict(zip(attrs, [graph(self, attr) for attr in sorted(attrs)]))
    ...
    # -------------------------------------------------------------------------
    @GraphNodeVt("Property")
    def TransactionsStored(self, graph):
        book = graph(self, "Party")
        trade = graph(self, "Name")
        trade_date = graph(self, "TradeDate")
        pos_date = graph("Database", "PositionsDate")
        parms = (book, trade, trade_date, pos_date)
        rows = ObjDbQuery(TRANSACTIONS_STORED, parms, attr="fetchall")
    ...
    def PositionEffects(self, graph):
        pos_effects = []
        for transaction in graph(self, "TransactionsStored").values():
    ...
    def delete(self):
        if GetVal("Database", "TradesDeletable"):
            transactions = TransactionsBy(trade=self.Name)
