...
from onyx.core import(ObjDbQuery, GraphNodeVt, GetObj, GetVal, UfoBase,
                      ReferenceField, FloatField, StringField, SelectField)
...
class Transaction(UfoBase):
    ...
    Event = StringField()
    Marker = SelectField(options=["HEAD", "AMENDED",
    ...
    # -------------------------------------------------------------------------
    @GraphNodeVt("Property")
    def TradeDate(self, graph):
        return graph(graph(self, "Trade"), "TradeDate")
    ...
    def TransactionInfo(self, graph):
        ...
        attrs = attrs.difference({"Trade", "Event", "Marker"})
    ...
    # -------------------------------------------------------------------------
    @GraphNodeVt("Property")
    def SecTradedInstance(self, graph):
        sec_name = graph(self, "SecurityTraded")
        sec = AddByInference(GetObj(sec_name), in_memory=True)
        if sec.Name == sec_name:
            return sec
        else:
            raise TransactionError("Inferred name for SecurityTraded "
                                   "{0:s} doesn't match the stored "
                                   "value {1:s}".format(sec.Name, sec_name))

    # -------------------------------------------------------------------------
    @GraphNodeVt("Property")
    def PositionEffects(self, graph):
        sec = AddByInference(graph(self, "SecTradedInstance"), in_memory=False)
        qty = graph(self, "Quantity")
        date = graph(self, "TradeDate")
        ...
        pos_effects = [
            Position(date, cpty1, qty,
                     sec.Name, sec.ObjType, name, event, trade),
    ...
    def delete(self):
        if GetVal("Database", "TradesDeletable"):
            ObjDbQuery("DELETE FROM PosEffects "
                       "WHERE Transaction=%s", parms=(self.Name,))
        else:
