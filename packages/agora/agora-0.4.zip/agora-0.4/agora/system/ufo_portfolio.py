...
from onyx.core import (Structure, GraphNodeVt, UfoBase, IsInstance,
...
class Portfolio(UfoBase):
    ...
    def RecursiveBooks(self, graph):
        """
        Return the list of of all books that are children of this portfolio.
        """
        books = set()
        for kid in graph(self, "Children"):
            if IsInstance(kid, "Book"):
                books.add(kid)
            elif IsInstance(kid, "Portfolio"):
                books.update(graph(kid, "RecursiveBooks"))
            else:
                raise ValueError("How did a {0:s} ended up in a "
                                 "{1:s}?".format(graph(kid, "ObjType"),
                                                 graph(self, "ObjType")))
        return books
