...
from agora.system.ufo_transaction import TransactionError
...
def amend_transaction(head, info):
    ...
    for attr, value in info.items():
        ...
        if not same:
            break
    if same:
        raise ValueError("amend_transaction, found nothing to amend.")
    ...
    if head.TimeCreated >= Date.today():
        ...
        new_trans = head.clone()
        new_trans.Name = head.format_name(Date.today(), unique_id(8))
        new_trans.Marker = "HEAD"
        new_trans.PrevTransaction = backout.Name
        ...
        with ObjDbTransaction("Amend", "SERIALIZABLE"):
            ...
            for pos in GetVal(new_trans, "PositionEffects"):
                ObjDbQuery(QUERY_INSERT_POSEFF,
                           parms=(pos.TradeDate, new_trans.TimeCreated,
                                  pos.Book, pos.Qty, pos.Unit, pos.UnitType,
                                  pos.Name, pos.Event, pos.Trade))
...
def AgeTransaction(head):
    ...
    for sec in securities:
        new_transactions.append(Transaction(
            SecurityTraded=sec["Security"],
            Quantity=head.Quantity*sec["Quantity"],
        ...
        #     transactionsdoesn't matter as they would have to be all rolled
        #     back together)
        prev_trans = None

    with ObjDbTransaction("Ageging Event", "SERIALIZABLE"):
        AddObj(backout)
        for pos in GetVal(backout, "PositionEffects"):
            ObjDbQuery(QUERY_INSERT_POSEFF,
                       parms=(pos.TradeDate, backout.TimeCreated,
...
def TransactionsBy(book=None, trade=None,
    ...
    if len(criteria):
        query = """SELECT Transaction FROM PosEffects WHERE {0:s}
                   GROUP BY Transaction
                   ORDER BY Transaction;""".format(" AND ".join(extquery))

        rows = ObjDbQuery(query, parms=criteria, attr="fetchall")

    else:
        query = """SELECT Transaction FROM PosEffects
                   GROUP BY Transaction
