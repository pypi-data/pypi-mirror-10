...
from onyx.core import (Structure, Curve, CurveIntersect,
                       GraphNodeVt, ChildrenSet, DiddleScope)
...
def WithRiskValueTypes(cls):
    ...
    def Deltas(self, graph):
        ...
        for kid in ChildrenSet(self, "MktValUSD", "Spot", "Asset", graph):
            ...
            fx = graph(cross, "Spot")
            shift = 0.01*spot
            with DiddleScope() as ds:
                ds.set_diddle(kid, "Spot", spot+shift)
    ...
    def FxExposures(self, graph):
        exposures = Structure()
        for kid in ChildrenSet(self, "MktVal", "Spot", "CurrencyCross", graph):
            spot = graph(kid, "Spot")
            shift = 0.01*spot
            with DiddleScope() as ds:
                ds.set_diddle(kid, "Spot", spot+shift)
    ...
    def Betas(self, graph):
        ...
        betas = Structure()
        index = get_curve_usd(graph("Risk", "RefIndex"), start, end)
        for kid in ChildrenSet(self, "MktValUSD", "Spot", "Asset", graph):
            kid_prcs = graph(kid, "PricesForRisk", start, end)
            idx_prcs, kid_prcs = CurveIntersect([index, kid_prcs])
            kid_rets = np.diff(np.log(kid_prcs.values))
            idx_rets = np.diff(np.log(idx_prcs.values))
            A = np.vstack([idx_rets, np.ones_like(idx_rets)]).T
            beta, alpha = np.linalg.lstsq(A, kid_rets)[0]
            betas[kid] = beta
        return betas
...
def worst_loss(mkt_values, ndays=1):
    ...
    if len(mkt_values) == 0:
        ...
        max_loss_idx, = np.nonzero(np.ravel(diffs == max_loss))
