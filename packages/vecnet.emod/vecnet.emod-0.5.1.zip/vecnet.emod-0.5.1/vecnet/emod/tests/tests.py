...
class Tests(unittest.TestCase):
    ...
    def testLoad(self):
        ...
        self.assertRaises(ValueError, climateDataFile.load, "badjson.json", "shortvalid.bin")
        # Check to see if all the required json elements are there, extra elements are ignored
        self.assertRaises(KeyError, climateDataFile.load, "missingattributes.json", "shortvalid.bin")
        # Check to see if bin file has enough numbers
        self.assertRaises(ValueError, climateDataFile.load, "shortvalid.json", "missingdata.bin")
        # Check bin file for valid numbers
        self.assertRaises(ValueError, climateDataFile.load, "shortvalid.json", "baddata.bin")
        # Check json file and bin file
        jsonFile = open("shortvalid.json", 'r')
        binFile = open("shortvalid.bin", 'rb')
        climateDataFile.load(jsonFile, binFile)
        # Check json string
        climateDataFile.load("{    \"Metadata\": {        \"Author\": \"Me\",        \"DataProvenance\": \"No Idea\",        \"DatavalueCount\": 4,        \"DateCreated\": \"1-1-1999\",        \"IdReference\": \"NA\",        \"NodeCount\": 3,        \"OriginalDataYears\": \"BeginningOfTime-Yesterday\",        \"StartDayOfYear\": \"January 1\",        \"Tool\": \"Awesome Tool\",        \"UpdateResolution\": \"NA\"    },    \"NodeOffsets\": \"0AAAAAAE000000001AAAAAAE000000102AAAAAAE00000020\"}", "shortvalid.bin")
        # Check binary as bytes
        binFile = open("shortvalid.bin", 'rb')
        data = binFile.read()
        climateDataFile.load("shortvalid.json", data)
    ...
    def testSave(self):
        ...
        jsonFile = open("shortvalid.json", 'r')
        binFile = open("shortvalid.json", 'rb')
        climateDataFile.load("shortvalid.json", "shortvalid.bin")
        # Check to parameters
        ...
        self.assertRaises(ValueError, climateDataFile2.save, "result.json", "result.bin")
    ...
    def testFileEquivalence(self):
        ...
        climateDataFile2 = ClimateDataFile()
        climateDataFile.load("shortvalid.json", "shortvalid.bin")
        climateDataFile.save("result1.json", "result1.bin")
        climateDataFile2.load("result1.json", "result1.bin")
    ...
    def testExtraJson(self):
        ...
        climateDataFile = ClimateDataFile()
        climateDataFile.load("extrajson.json", "shortvalid.bin")
        climateDataFile.save("result2.json", "result2.bin")
    ...
    def testEVERYTHING(self):
        ...
        climateDataFile.save("result3.json", "result3.bin")
