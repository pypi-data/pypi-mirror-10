...
class ParticleDataTable(object):
    ...
    By default, the input database file will be the mass_width_2006.csv file
    ...
    def __init__(self, dbpath=None):
        ...
        if dbpath is None:
            ...
            if not dbpath or not os.path.exists(dbpath):
                ...
                dbpath = os.path.join(prefix, "share", "pypdt", "mass_width_2006.csv")
        self.clear()
        self.read_db(dbpath)
    ...
    def read_db(self, dbpath):
        ...
        """
        if dbpath.endswith(".tbl"):
            self.read_pdttbl(dbpath)
        # elif dbpath.endswith(".mc"):
        #     self.read_pdgmc(dbpath)
    ...
    def read_pdgmc(self, dbpath):
        """
        Read a PDG .mc particle data database file, updating the currently
        ...
        """
        raise Exception(".mc database format reading not implemented")
        #
        # self.dbfiles.append(dbpath)
        # dbf = open(dbpath)
        # for line in dbf:
        ...
        for line in dbf:
            ...
            try:
                ...
                if chargestr == "0":
                    ...
                    exec("threecharge = 3 * float(%s)" % chargestr)
                name = data[16]
