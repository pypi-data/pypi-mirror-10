...
def get_credentials_from_headers(request, content_md5):
    ...
    try:
        ...
        log.warning("SIGNED REQUEST: couldn't determine method from Authorization header")
        return
    if method != 'Cubicweb':
        log.warning('SIGNED REQUEST: method is not Cubicweb')
        return
    ...
    try:
        ...
        log.warning('SIGNED REQUEST: wrong date format')
        raise AuthenticationError()
    delta = datetime.utcnow() - date
    if delta < timedelta(0) or delta > timedelta(0, 300):
        log.warning('SIGNED REQUEST: date delta error')
        raise AuthenticationError()
    try:
        id, signature = credentials.split(':', 1)
        log.warning('SIGNED REQUEST: encoding info for %s' % id)
        return credentials
    except ValueError:
