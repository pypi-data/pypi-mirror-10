    ...
    with open(sname) as fin:
        ...
        for line in infile:
            ...
            r2_reg = (
                ...
                line['RNA2 strand'])
            counts[reg1] += int(line['interactions'])
            counts[reg2] += int(line['interactions'])
    return counts
...
def plot_scatter(chimera, singles, chisum, lorder, figname):
    ...
    """
    def scatit(dname, dname2, axarr, i, j, l1, l2):
        """
        ...
        axarr[i, j].scatter(xvec, yvec)
        axarr[i, j].set_xlabel(l1)
        axarr[i, j].set_ylabel(l2)
        axarr[i, j].set_yscale('log')
        axarr[i, j].set_xscale('log')
        

    lln = len(lorder)
    f, axarr = subplots(lln, lln)
    for i, l1 in enumerate(lorder):
        for j, l2 in enumerate(lorder):
            if i>j: # Print singles
                scatit(singles, singles, axarr, i, j, l1, l2)
            elif i==j:
                scatit(singles, chisum, axarr, i, j, l1, l2)
            else:
                scatit(chimera, chimera, axarr, i, j, l1, l2)
