    ...
    def get_tags(self,data=[],n=50):
        if not data:
            ...
            for fname in os.listdir('.'):
                ...
                m = re.match(fileExtension,'clsfr')
                if not m:
                    continue
...
#             featuresets = [(entry['features'], tag in entry['tags']) for entry in data if entry['tags']]
            print('preparing featureset')
            try:
                featuresets = [(entry['features'], tag in entry['tags']) for entry in data]
                print('prepared featureset')
            except:
                error_msg = 'could not prepare featureset'
                raise Exception(error_msg)                                
            print('classifying tag')
            try:
                classifier = nltk.NaiveBayesClassifier.train(featuresets)
                print('classified tag')
            except:
                error_msg = 'could not classify'
                raise Exception(error_msg)                
            print('writing to file')
            fname = unicode(tag)+'.clsfr'
            try:
                with open(fname,'w') as classifier_file:
                    pickle.dump(classifier,classifier_file)
                print('wrote classifier file')
            except:
