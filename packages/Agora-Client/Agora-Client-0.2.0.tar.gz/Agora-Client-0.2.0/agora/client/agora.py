            ...
            for n in nxt:
                ...
                if next_seeds.values():
                    ...
                    for pattern in node_patterns:
                        ...
                        if pattern_space not in node_seeds:
                            continue
                    ...
                    if filter(lambda x: len(next_seeds[x]), next_seeds.keys()):
                        # TODO: Create small enough chunks before continuing!!!
                        for space in next_seeds.keys():
                            chs = chunks(list(next_seeds[space]), min(len(next_seeds[space]), 1))
                            for chunk in chs:
                                for yt in __follow_node(n, tree_graph, {space: chunk}):
                                    yield yt
