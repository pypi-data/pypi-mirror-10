...
#                                print_to_file=print_to_file, print_filename=print_filename)
    ...
    def blot_averaged_profiles(self, ts_from=1, ts_to=-1):
        """
	...
	s3.y_range = Range1d(start=Te[np.nanargmin(Te[1:-1])+1]*0.97,end=Te[np.nanargmax(Te[1:-1])+1]*1.03)
	s3.line(r, Te[1:-1], line_width=1, color="blue")
	s3.extra_y_ranges = {"vre": Range1d(start=vre[np.nanargmin(vre[1:-1])+1]*0.97,end=vre[np.nanargmax(vre[1:-1])+1]*1.03)}
	s3.line(r, vre[1:-1], line_width=1, color="purple", y_range_name="vre")
	...
	s4.y_range = Range1d(start=Ti[np.nanargmin(Ti[1:-1])+1]*0.97,end=Ti[np.nanargmax(Ti[1:-1])+1]*1.03)
	s4.line(r, Ti[1:-1], line_width=1, color="blue")
	s4.extra_y_ranges = {"vri": Range1d(start=vri[np.nanargmin(vri[1:-1])+1]*0.97,end=vri[np.nanargmax(vri[1:-1])+1]*1.03)}
	s4.line(r, vri[1:-1], line_width=1, color="purple", y_range_name="vri")
