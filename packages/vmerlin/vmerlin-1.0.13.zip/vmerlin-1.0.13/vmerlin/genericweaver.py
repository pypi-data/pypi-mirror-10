...
class BaseWeaver(LoggingClass):
    ...
    @staticmethod
    def fillEndpointMethodDetails(props, endpointMethod):
        if endpointMethod is None:
            return props
        
        if props is None:
            props = { }

        if inspect.ismethod(endpointMethod):
            props['objecttype'] = endpointMethod.im_class.__name__
        elif inspect.isfunction(endpointMethod):
            props['objecttype'] = 'function'
        else:
            return props
        props['function'] = Helpers.toSafeString(endpointMethod.__name__)

        if not 'signature' in props:
            argspec = inspect.getargspec(endpointMethod)
            props['signature'] = inspect.formatargspec(*argspec)[1:-1]  # strip enclosing ()

        return props

    def dispatchEndpoint(self, startTime, endTime, props):
        if not 'flavor' in props:
            props['flavor'] = 'endpoint'

        objType = props.get('objecttype', 'Object')
        funcName = props.get('function', 'function')
        signature = props.get('signature', '')
        if not 'label' in props:
            props['label'] = '%s#%s' % (objType, funcName)
        if not 'entrypoint' in props:
            props['entrypoint'] = '%s#%s' % (objType, funcName)
        if not 'identifier' in props:
            props['identifier'] = '%s#%s(%s)' % (objType, funcName, signature)

        return self.dispatch(startTime, endTime, props)
