//////////////////////////////////////////////////////////////////////////////
//
//  $Id: Buffer.cc 880 2015-01-01 16:10:37Z weegreenblobbie $
//
...
__repr__() const
{
    ...
    ss
        << "Nsound::Buffer(vector at 0x"
        << std::hex
        << reinterpret_cast<uint64>(&data_)
        << ", data at 0x"
        << reinterpret_cast<uint64>(&data_[0])
        << ", length="
        << std::dec
        << getLength()
        << ")";
...
operator=(const Buffer & rhs_buffer)
{
    ...
    data_ = FloatVector(rhs_buffer.data_);
...
Buffer &
Buffer::
vectorOperator(const Buffer & rhs_buffer, MathOperator op)
{
...
// scalarOperator()
//
// Perform scalar operators on the Buffer object.
//
// Perform scalar operators on the Buffer object.
//
// If reverse is false, the buffer object is the left hand side for
// the expresion:
//   buffer + 5
//   buffer - 5
//   buffer * 5
...
Buffer &
Buffer::
scalarOperator(float64 d, MathOperator op)
{
...
void
Buffer::
set_at_index(int32 index, float64 d)
{
    int32 len = getLength();
