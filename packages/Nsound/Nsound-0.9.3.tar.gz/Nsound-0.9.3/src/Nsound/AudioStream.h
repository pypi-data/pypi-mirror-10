//-----------------------------------------------------------------------------
//
//  $Id: AudioStream.h 880 2015-01-01 16:10:37Z weegreenblobbie $
//
...
class AudioStream
{
    ...
    {
        return substream(static_cast<uint32>(start_index),
                         static_cast<uint32>(n_samples));
    };

    // DOXME
    AudioStream
    substream(float32 start_time, float32 duration = 0) const;

    AudioStream
    substream(const double & start_time, const double & duration = 0) const
    {
        return substream(static_cast<float32>(start_time),
                         static_cast<float32>(duration));
    }
    ...
    const Buffer &
    get_at_index(uint32 index) const;

    //! SWIG helper function for operator[]
    void
    set_at_index(uint32 index, const Buffer & b);
    ...
    // not using std::vector<Buffer *> because that would introduce a lot of
    // noisy syntax in the .cc file dereferencing buffer pointers.
...
inline AudioStream operator+(float64 d, const AudioStream & lhs)
{
    AudioStream temp(lhs);
    return temp += d;
}
...
AudioStream operator/(float64 d, const AudioStream & lhs);
