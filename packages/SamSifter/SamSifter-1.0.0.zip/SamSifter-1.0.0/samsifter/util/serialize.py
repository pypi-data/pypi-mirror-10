...
class WorkflowSerializer():
    ...
    @classmethod
    def workflow_from_xml(cls, tree, workflow):
        ...
        for filepath in root.iter(tag='filepath'):
            if filepath.attrib['type'] == 'input':
                workflow.set_in_filename(filepath.text)
            if filepath.attrib['type'] == 'output':
                workflow.set_out_filename(filepath.text)
        ...
        for pipeline in root.iter(tag='pipeline'):
            for tool in pipeline.iter(tag='tool'):
                ...
                for param in tool.iter(tag='parameter'):
                    ...
                    if param_type == 'threshold':
                        ...
                        for sffx in param.iter('suffix'):
                            extensions.append(
                                cls.singletons(sffx.get('extension')))
                        if len(extensions) <= 0:
                            extensions = None
                ...
                workflow.get_model().insertItem(item)
        return workflow
