...
#        from pprint import pprint
    ...
    if args.read_lengths:
        read_lengths = {}           # dict of dict of ints
        read_lengths_total = {}      # dict of ints
    line_count = 0
        ...
        for line_nr, line in enumerate(handle, 0):
            ...
            if line.startswith("@"):
                ...
                try:
                    ...
                    log.error("no ZL field in line %s", line_nr + 1)
                    exit(1)
                ...
                if args.read_lengths:
                    ...
                    if read_length in read_lengths_total:
                        read_lengths_total[read_length] += 1
                    else:
                        read_lengths_total[read_length] = 1

                    if accession in read_lengths:
                        if read_length in read_lengths[accession]:
                            read_lengths[accession][read_length] += 1
                        else:
                            read_lengths[accession][read_length] = 1
                    else:
                        read_lengths[accession] = {read_length: 1}
    ...
    log.info("%i reads are aligned to %i references", line_count, len(depths))

    # get overall read length statistics
    if args.read_lengths:
        max_length_total = max(read_lengths_total.keys())
        min_length_total = min(read_lengths_total.keys())
        length_dist_total = np.zeros(
            (max_length_total - min_length_total + 1, ), dtype=np.int)
        for (length, count) in read_lengths_total.items():
...
#                avg_depth_covered_scaled = avg_depth_covered * 1.0 / max_depth
            ...
            if args.read_lengths:
                max_length = max(read_lengths[accession].keys())
                min_length = min(read_lengths[accession].keys())
                avg_length = np.mean(np.fromiter(
                    read_lengths[accession].keys(), dtype=np.int,
                    count=len(read_lengths[accession])))
                max_lengths.append(max_length)

                # determine length distribution
                length_dist = np.zeros((max_length + 1,), dtype=np.int)
                for (length, count) in read_lengths[accession].items():
...
#                plot_lorenz(ax_lorenz, x, y)
            ...
            if args.read_lengths:
                log.info("accession %s: read length ranges from %i bp to %i "
                         "bp, average is %.3f bp",
                         accession, min_length, max_length, avg_length)
