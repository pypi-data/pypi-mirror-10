	...
	def coopyhx(self,io):
		...
		if (cmd1 == "diff"):
			...
			if (not ((color or no_color))):
				if ((output == "-") and ((self.output_format == "copy"))):
					if io.isTtyKnown():
						use_color = io.isTty()
			if use_color:
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 912, 'className': "Coopy", 'methodName': "show"}))
