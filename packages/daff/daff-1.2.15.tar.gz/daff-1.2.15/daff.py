...
Alignment = _hx_classes.registerClass("Alignment", fields=["map_a2b","map_b2a","ha","hb","ta","tb","ia","ib","map_count","order_cache","order_cache_has_reference","index_columns","reference","meta"], methods=["range","tables","headers","setRowlike","link","addIndexColumns","getIndexColumns","a2b","b2a","count","toString","toOrder","addToOrder","getSource","getTarget","getSourceHeader","getTargetHeader","toOrder3"])(Alignment)
...
CellInfo = _hx_classes.registerClass("CellInfo", fields=["raw","value","pretty_value","category","category_given_tr","separator","pretty_separator","updated","conflicted","pvalue","lvalue","rvalue"], methods=["toString"])(CellInfo)
...
CompareFlags = _hx_classes.registerClass("CompareFlags", fields=["ordered","show_unchanged","unchanged_context","always_show_order","never_show_order","show_unchanged_columns","unchanged_column_context","always_show_header","acts","ids","columns_to_ignore","allow_nested_cells","warnings"], methods=["filter","allowUpdate","allowInsert","allowDelete","getIgnoredColumns","addPrimaryKey","ignoreColumn","addWarning","getWarning"])(CompareFlags)
...
CompareTable = _hx_classes.registerClass("CompareTable", fields=["comp","indexes"], methods=["run","align","getComparisonState","alignCore","alignCore2","alignColumns","testHasSameColumns","hasSameColumns2","testIsEqual","isEqual2","compareCore","storeIndexes","getIndexes"])(CompareTable)
...
class Coopy(object):
	...
	def saveTable(self,name,t):
		...
		if (self.format_preference == "csv"):
			...
			txt = Ndjson(t).render()
		elif (self.format_preference == "html"):
			return self.renderTable(name,t)
	...
	def coopyhx(self,io):
		...
		if (cmd1 == "git"):
			...
			if (pos20 < 0):
				pos20 = 0
			res20 = args[pos20:(pos20 + len)]
			del args[pos20:(pos20 + len)]
			types = res20
		...
		if git:
			...
			if (pos21 < 0):
				pos21 = 0
			res21 = args[pos21:(pos21 + ct)]
			del args[pos21:(pos21 + ct)]
			git_args = res21
			...
			if (pos22 < 0):
				pos22 = 0
			res22 = args[pos22:(pos22 + len1)]
			del args[pos22:(pos22 + len1)]
			res22
			offset = 0
		...
		if (cmd1 == "diff"):
			...
			td.hilite(o)
			use_color = color
			if (not ((color or no_color))):
				if ((output == "-") and ((self.output_format == ""))):
					if io.isTtyKnown():
						use_color = io.isTty()
			if use_color:
	...
	def diffAsHtml(local,remote,flags = None):
		o = Coopy.diff(local,remote,flags)
		render = DiffRender()
	...
	def patch(local,patch,flags = None):
		patcher = HighlightPatch(local, patch)
	...
	def show(t):
		...
		while ((_g < h)):
			...
			while ((_g1 < w)):
				...
				txt = (HxOverrides.stringOrNull(txt) + " ")
			txt = (HxOverrides.stringOrNull(txt) + "\n")
		haxe_Log.trace(txt,_hx_AnonObject({'fileName': "Coopy.hx", 'lineNumber': 907, 'className': "Coopy", 'methodName': "show"}))
...
Coopy = _hx_classes.registerClass("Coopy", fields=["format_preference","delim_preference","extern_preference","output_format","output_format_set","nested_output","order_set","order_preference","io","pretty","css_output","fragment","mv","status","daff_cmd"], methods=["init","checkFormat","setFormat","renderTable","saveTable","saveText","loadTable","command","installGitDriver","coopyhx"], statics=["VERSION","diffAsHtml","diffAsAnsi","diff","patch","compareTables","compareTables3","keepAround","cellFor","jsonToTable","main","show","jsonify"])(Coopy)
...
DiffRender = _hx_classes.registerClass("DiffRender", fields=["text_to_insert","td_open","td_close","open","pretty_arrows","section"], methods=["usePrettyArrows","insert","beginTable","setSection","beginRow","insertCell","endRow","endTable","html","toString","render","sampleCss","completeHtml"], statics=["examineCell","markSpaces","renderCell"])(DiffRender)
...
FlatCellBuilder = _hx_classes.registerClass("FlatCellBuilder", fields=["view","separator","conflict_separator"], methods=["needSeparator","setSeparator","setConflictSeparator","setView","update","conflict","marker","links"], statics=["quoteForDiff"], interfaces=[CellBuilder])(FlatCellBuilder)
...
class HighlightPatch(object):
	...
	def finishRows(self):
		...
		while ((_g3 < python_lib_Builtin.len(_g12))):
			cmod = (_g12[_g3] if _g3 >= 0 and _g3 < python_lib_Builtin.len(_g12) else None)
			_g3 = (_g3 + 1)
			if (not cmod.rem):
				if cmod.add:
					_g21 = 0
					...
					while ((_g21 < python_lib_Builtin.len(_g31))):
						mod2 = (_g31[_g21] if _g21 >= 0 and _g21 < python_lib_Builtin.len(_g31) else None)
						_g21 = (_g21 + 1)
						if ((mod2.patchRow != -1) and ((mod2.destRow != -1))):
							d1 = self.patch.getCell(cmod.patchRow,mod2.patchRow)
							self.source.setCell(cmod.destRow,mod2.destRow,d1)
					hdr = self.header.h.get(cmod.patchRow,None)
					self.source.setCell(cmod.destRow,0,self.view.toDatum(hdr))
		_g13 = 0
		_g4 = self.source.get_width()
		while ((_g13 < _g4)):
			i = _g13
			_g13 = (_g13 + 1)
	...
	def finishColumns(self):
		...
		inc = _hx_local_1
		_g13 = 0
		_g4 = python_lib_Builtin.len(fate)
		while ((_g13 < _g4)):
			i2 = _g13
			_g13 = (_g13 + 1)
...
HighlightPatch = _hx_classes.registerClass("HighlightPatch", fields=["source","patch","view","sourceView","csv","header","headerPre","headerPost","headerRename","headerMove","modifier","currentRow","payloadCol","payloadTop","mods","cmods","rowInfo","cellInfo","rcOffset","indexes","sourceInPatchCol","patchInSourceCol","destInPatchCol","patchInDestCol","patchInSourceRow","lastSourceRow","actions","rowPermutation","rowPermutationRev","colPermutation","colPermutationRev","haveDroppedColumns","headerRow","preambleRow","flags","meta_change","process_meta","prev_meta","next_meta"], methods=["reset","processMeta","apply","needSourceColumns","needDestColumns","needSourceIndex","setMetaProp","applyMetaRow","applyRow","getDatum","getString","applyMeta","applyHeader","lookUp","applyAction","checkAct","getPreString","getRowString","isPreamble","sortMods","processMods","computeOrdering","permuteRows","finishRows","permuteColumns","finishColumns"], interfaces=[Row])(HighlightPatch)
...
Index = _hx_classes.registerClass("Index", fields=["items","keys","top_freq","height","cols","v","indexed_table","hdr"], methods=["addColumn","indexTable","toKey","toKeyByContent","getTable"])(Index)
...
IndexPair = _hx_classes.registerClass("IndexPair", fields=["ia","ib","hdr","quality"], methods=["addColumns","indexTables","queryByKey","queryByContent","queryLocal","localKey","remoteKey","getTopFreq","getQuality"])(IndexPair)
...
class Meta(object):
	pass
Meta = _hx_classes.registerClass("Meta", methods=["alterColumns","asTable","clone"])(Meta)
...
Ordering = _hx_classes.registerClass("Ordering", fields=["order","ignore_parent"], methods=["add","getList","toString","ignoreParent"])(Ordering)
...
Reflect = _hx_classes.registerClass("Reflect", statics=["field","setField","isFunction"])(Reflect)

class Table(object):
	pass
Table = _hx_classes.registerClass("Table", methods=["getCell","setCell","getCellView","isResizable","resize","clear","insertOrDeleteRows","insertOrDeleteColumns","trimBlank","get_width","get_height","getData","clone","getMeta"])(Table)
...
class SimpleTable(object):
	...
	def clone(self):
		...
		if (self.meta is not None):
			result.meta = self.meta.clone(result)
		return result
...
SimpleTable = _hx_classes.registerClass("SimpleTable", fields=["data","w","h","meta"], methods=["getTable","get_width","get_height","getCell","setCell","toString","getCellView","isResizable","resize","clear","insertOrDeleteRows","insertOrDeleteColumns","trimBlank","getData","clone","setMeta","getMeta"], statics=["tableToString","tableIsSimilar"], interfaces=[Table])(SimpleTable)

class View(object):
	pass
View = _hx_classes.registerClass("View", methods=["toString","equals","toDatum","makeHash","hashSet","isHash","hashExists","hashGet"])(View)
...
SimpleView = _hx_classes.registerClass("SimpleView", methods=["toString","equals","toDatum","makeHash","hashSet","hashExists","hashGet","isHash"], interfaces=[View])(SimpleView)
...
SqlColumn = _hx_classes.registerClass("SqlColumn", fields=["name","primary"], methods=["getName","isPrimaryKey","toString"], statics=["byNameAndPrimaryKey"])(SqlColumn)
...
class SqlCompare(object):
	...
	def apply(self):
		...
		while ((_g13 < _g4)):
			...
			_g13 = (_g13 + 1)
			if (i3 > 0):
				sql_key_match = (HxOverrides.stringOrNull(sql_key_match) + " AND ")
			n = self.local.getQuotedColumnName((key_cols[i3] if i3 >= 0 and i3 < python_lib_Builtin.len(key_cols) else None))
			sql_key_match = (HxOverrides.stringOrNull(sql_key_match) + HxOverrides.stringOrNull((((((((HxOverrides.stringOrNull(sql_table1) + ".") + HxOverrides.stringOrNull(n)) + " IS ") + HxOverrides.stringOrNull(sql_table2)) + ".") + HxOverrides.stringOrNull(n)))))
		sql_data_mismatch = ""
		_g14 = 0
		...
		while ((_g14 < _g5)):
			...
			sql_data_mismatch = (HxOverrides.stringOrNull(sql_data_mismatch) + HxOverrides.stringOrNull((((((((HxOverrides.stringOrNull(sql_table1) + ".") + HxOverrides.stringOrNull(n1)) + " IS NOT ") + HxOverrides.stringOrNull(sql_table2)) + ".") + HxOverrides.stringOrNull(n1)))))
		sql_dbl_cols = ""
		dbl_cols = []
		_g15 = 0
		...
		while ((_g15 < _g6)):
			...
			sql_dbl_cols = (HxOverrides.stringOrNull(sql_dbl_cols) + HxOverrides.stringOrNull((((((HxOverrides.stringOrNull(sql_table1) + ".") + HxOverrides.stringOrNull(n2)) + " AS ") + HxOverrides.stringOrNull(buf)))))
			dbl_cols.append(buf)
			python_lib_Builtin.len(dbl_cols)
			sql_dbl_cols = (HxOverrides.stringOrNull(sql_dbl_cols) + ",")
			sql_dbl_cols = (HxOverrides.stringOrNull(sql_dbl_cols) + HxOverrides.stringOrNull(((((((HxOverrides.stringOrNull(sql_table2) + ".") + HxOverrides.stringOrNull(n2)) + " AS ") + HxOverrides.stringOrNull(buf)) + "b"))))
			dbl_cols.append((HxOverrides.stringOrNull(buf) + "b"))
			python_lib_Builtin.len(dbl_cols)
		sql_order = ""
		_g16 = 0
		...
		while ((_g17 < _g8)):
			...
			_g17 = (_g17 + 1)
			if (i7 > 0):
				sql_dbl_order = (HxOverrides.stringOrNull(sql_dbl_order) + ",")
			n4 = self.local.getQuotedColumnName((key_cols[i7] if i7 >= 0 and i7 < python_lib_Builtin.len(key_cols) else None))
			sql_dbl_order = (HxOverrides.stringOrNull(sql_dbl_order) + HxOverrides.stringOrNull((((HxOverrides.stringOrNull(sql_table1) + ".") + HxOverrides.stringOrNull(n4)))))
		rowid = "-3"
		rowid1 = "-3"
		rowid2 = "-3"
		...
		sql_inserts = (((((((((("SELECT DISTINCT NULL, " + HxOverrides.stringOrNull(rowid)) + " AS rowid, ") + HxOverrides.stringOrNull(sql_all_cols)) + " FROM ") + HxOverrides.stringOrNull(sql_table2)) + " WHERE NOT EXISTS (SELECT 1 FROM ") + HxOverrides.stringOrNull(sql_table1)) + " WHERE ") + HxOverrides.stringOrNull(sql_key_match)) + ")")
		sql_inserts_order = (["NULL", "rowid"] + all_cols)
		sql_updates = ((((((((((((("SELECT DISTINCT " + HxOverrides.stringOrNull(rowid1)) + " AS __coopy_rowid0, ") + HxOverrides.stringOrNull(rowid2)) + " AS __coopy_rowid1, ") + HxOverrides.stringOrNull(sql_dbl_cols)) + " FROM ") + HxOverrides.stringOrNull(sql_table1)) + " INNER JOIN ") + HxOverrides.stringOrNull(sql_table2)) + " ON ") + HxOverrides.stringOrNull(sql_key_match)) + " WHERE ") + HxOverrides.stringOrNull(sql_data_mismatch))
		sql_updates_order = (["__coopy_rowid0", "__coopy_rowid1"] + dbl_cols)
		sql_deletes = (((((((((("SELECT DISTINCT " + HxOverrides.stringOrNull(rowid)) + " AS rowid, NULL, ") + HxOverrides.stringOrNull(sql_all_cols)) + " FROM ") + HxOverrides.stringOrNull(sql_table1)) + " WHERE NOT EXISTS (SELECT 1 FROM ") + HxOverrides.stringOrNull(sql_table2)) + " WHERE ") + HxOverrides.stringOrNull(sql_key_match)) + ")")
...
SqlCompare = _hx_classes.registerClass("SqlCompare", fields=["db","parent","local","remote","at0","at1","align"], methods=["equalArray","validateSchema","denull","link","linkQuery","apply"])(SqlCompare)

class SqlDatabase(object):
	pass
SqlDatabase = _hx_classes.registerClass("SqlDatabase", methods=["getColumns","getQuotedTableName","getQuotedColumnName","begin","beginRow","read","get","end","width","rowid"])(SqlDatabase)

class SqlHelper(object):
	pass
SqlHelper = _hx_classes.registerClass("SqlHelper", methods=["getTableNames","countRows","getRowIDs"])(SqlHelper)
...
class SqlTable(object):
	...
	def setCell(self,x,y,c):
		haxe_Log.trace("SqlTable cannot set cells yet",_hx_AnonObject({'fileName': "SqlTable.hx", 'lineNumber': 112, 'className': "SqlTable", 'methodName': "setCell"}))
...
class SqliteHelper(object):
	...
	@staticmethod
	def _hx_empty_init(_hx_o):		pass
SqliteHelper = _hx_classes.registerClass("SqliteHelper", methods=["getTableNames","countRows","getRowIDs"], interfaces=[SqlHelper])(SqliteHelper)
...
StringTools = _hx_classes.registerClass("StringTools", statics=["lpad","replace"])(StringTools)
...
Sys = _hx_classes.registerClass("Sys", statics=["args","command","stdout","stderr"])(Sys)
...
TableComparisonState = _hx_classes.registerClass("TableComparisonState", fields=["p","a","b","completed","run_to_completion","is_equal","is_equal_known","has_same_columns","has_same_columns_known","compare_flags"], methods=["reset"])(TableComparisonState)
		...
		if (self.show_rc_numbers and (not self.flags.never_show_order)):
			...
			while ((_g11 < _g2)):
				...
				if (unit is None):
					output.setCell(0,i1,"")
					continue
				output.setCell(0,i1,self.builder.links(unit))
			target = list()
			_g12 = 0
			...
			while ((_g13 < _g4)):
				...
				if (unit1 is None):
					output.setCell(i3,0,"")
					continue
				output.setCell(i3,0,self.builder.links(unit1))
	...
	def scanRow(self,unit,output,at,i):
		...
		while ((_g1 < _g)):
			...
			if have_pp:
				if (not have_rr):
					dd = pp
				elif self.v.equals(pp,rr):
					dd = pp
				else:
					dd = pp
					dd_to = rr
					have_dd_to = True
					if (not self.v.equals(pp,ll)):
						if (not self.v.equals(pp,rr)):
							dd_to_alt = ll
							have_dd_to_alt = True
			elif have_ll:
				if (not have_rr):
					dd = ll
				elif self.v.equals(ll,rr):
					dd = ll
				else:
					dd = ll
					dd_to = rr
			...
			if (have_dd_to and self.allow_update):
				...
				is_conflict = False
				if have_dd_to_alt:
					if (not self.v.equals(dd_to,dd_to_alt)):
						is_conflict = True
				if (not is_conflict):
...
class TableIO(object):
	...
	@staticmethod
	def _hx_empty_init(_hx_o):		pass
TableIO = _hx_classes.registerClass("TableIO", methods=["getContent","saveContent","args","writeStdout","writeStderr","command","async","exists","isTtyKnown","isTty","openSqliteDatabase"])(TableIO)
...
TerminalDiffRender = _hx_classes.registerClass("TerminalDiffRender", fields=["codes","t","csv","v","align_columns"], methods=["alignColumns","render","getText","pickSizes"])(TerminalDiffRender)
...
Unit = _hx_classes.registerClass("Unit", fields=["l","r","p"], methods=["lp","toString","fromString"], statics=["describe"])(Unit)
