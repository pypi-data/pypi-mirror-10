...
#                p.x0 = dict([(key, p.x0[key]) for key in D2])
    ...
    f_elems = [p.f] if isinstance(p.f, oofun) else p.f.view(ndarray) if isinstance(p.f, ooarray) else p.f 

    f_order = PythonMax(elem.getOrder(p.freeVarsSet, p.fixedVarsSet, fixedVarsScheduleID = p._FDVarsID) for elem in f_elems)
