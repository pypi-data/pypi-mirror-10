...
ones, array_equal, array, asfarray, ravel
...
def fdPrepare(p):
    ...
    if p.probType in ['SLE', 'NLSP', 'SNLE', 'LLSP']:
        ...
        if p.probType in ('SLE', 'LLSP'): p.C = EQs
...
#                p.x0 = dict([(key, p.x0[key]) for key in D2])
    ...
    for key, val in p.x0.items():
        ...
        if type(val) in (str, string_):
            ...
            key.formAuxDomain()
            p.x0[key] = key.aux_domain[val]#searchsorted(key.aux_domain, val, 'left')
        ...
        and key.domain is not int and key.domain is not 'int' and val not in key.domain:
            p.x0[key] = key.domain[0]
    
    p.x0 = oopoint(p.x0)
    p.x0.maxDistributionSize = p.maxDistributionSize

    setStartVectorAndTranslators(p)
    
    if p.probType in ['LP', 'MILP', 'SOCP'] and p.f.getOrder(p.freeVarsSet, p.fixedVarsSet, fixedVarsScheduleID = p._FDVarsID) > 1:
...
#                    if v.aux_domain
    ...
    p.dictOfFixedFuncs = {}
    p._dictOfRedirectedFuncs = {}
    hasFixedVariables = len(p.fixedVarsSet)
    
    from FuncDesigner import broadcast
    Iterator = eq if p.probType in ['SLE', 'NLSP', 'SNLE', 'LLSP'] else [p.f]

    for elem in Iterator:
        if p._isStochastic:
            broadcast(formDictOfRedirectedFuncs, Iterator, p.useAttachedConstraints, p)
        if hasFixedVariables:
            broadcast(formDictOfFixedFuncs, Iterator, p.useAttachedConstraints, p.dictOfFixedFuncs, areFixed, p._x0)
    ...
    if inplaceLinearRender and hasattr(p, 'f'):
        ...
        if type(p.f) in [list, tuple, set]:
            ...
            if p.f.getOrder(p.freeVarsSet, p.fixedVarsSet, fixedVarsScheduleID = p._FDVarsID) < 2:
                D = p.f.D(Z, **D_kwargs2)
...
#    p._dictOfRedirectedFuncs.update(p.dictOfFixedFuncs)
    
    
    if len(b) != 0:
        p.A, p.b = Vstack(A), Hstack([asfarray(elem).flatten() for elem in b])#Vstack(b).flatten()
        if hasattr(p.b, 'toarray'): p.b = p.b.toarray()
    if len(beq) != 0:
        p.Aeq, p.beq = Vstack(Aeq), Hstack([ravel(elem) for elem in beq])#Vstack(beq).flatten()
