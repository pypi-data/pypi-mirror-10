...
#                p.x0 = dict([(key, p.x0[key]) for key in D2])
    ...
    Iterator = [p.f] if isinstance(p.f, (oofun, ooarray)) else p.f 

    f_order = PythonMax(elem.getOrder(p.freeVarsSet, p.fixedVarsSet, fixedVarsScheduleID = p._FDVarsID) for elem in Iterator)
...
#        for j in range(1):
        if hasFixedVariables:
            #TODO: reduce arguments number
#           broadcast(formDictOfFixedFuncs, Iterator, p.useAttachedConstraints, p)
            for item in Iterator:
#                tmp = item()
#                areFixed_ = lambda dep: PythonAll(d is None or d in p.fixedVarsSet or (isinstance(d, Stochastic) and ))
