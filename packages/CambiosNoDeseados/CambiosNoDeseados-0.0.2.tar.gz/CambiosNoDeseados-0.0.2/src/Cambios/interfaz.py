...
class Interfaz(object):
    ...
    def click(self, evento):
        ...
        pos = evento.pos
        if boton == 1:
            if self.personajeSeleccionado and self.MostradorPersonaje.Area.collidepoint(*pos):
                e = event.Event(
                    COMANDOSELECCIONADO,
                    comando=self.seleccionarAccion(pos),
                    personaje=self.personajeSeleccionado,)
                event.post(e)
            elif self.jugador and self.ZonaCartas.Area.collidepoint(*pos):
                c, p = self.seleccionarCarta(pos)
                e = event.Event(CARTASELECCIONADA, carta=c, pos=p)
                event.post(e)
            elif self.juego and self.Campo.Area.collidepoint(*pos):
                p = self.seleccionarPos(pos)
...
class Campo(object):
    ...
    def buscarCuadro(self, x, y):
        for pos, objetos in self.juego.mapa.lista():
            c = Rect(
                pos.x*self.CUADRO_ANCHO,
                (pos.y*self.PISO_CUERPO) + self.PISO_TOPE,
                self.CUADRO_ANCHO,
                self.PISO_CUERPO,
                )
