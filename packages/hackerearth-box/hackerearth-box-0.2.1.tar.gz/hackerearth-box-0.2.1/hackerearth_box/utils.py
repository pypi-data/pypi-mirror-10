...
def print_packages(machines):
    ...
    for endpoint, details in machines.iteritems():
        print 'Operating on {endpoint}'.format(endpoint=endpoint)
...
def manage_dependencies_on_machine(machine, packages):
    ...
    Takes machine object and list of packages
    """
    for package in packages:
        manage_package(package, machine)

def assign_destinations(destinations, custom_machine):
    """if custom_machine is not None,
    it returns a list containing the callable"""
    if not custom_machine:
        return destinations

    return [{'server': custom_machine, 'enabled': True},]
