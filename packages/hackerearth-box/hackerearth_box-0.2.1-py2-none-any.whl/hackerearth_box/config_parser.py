...
from .utils import INSTALL, assign_destinations

def custom_dict():
    return {'machine': '', 'packages': []}

def config_parser(dependencies_config, action, custom_machine):
    ...
    for dep in dependencies_config:
        ...
        # if a specific server is passed,
        # replace the destination with it
        destinations = assign_destinations(dep.get('destinations'),
                custom_machine)
