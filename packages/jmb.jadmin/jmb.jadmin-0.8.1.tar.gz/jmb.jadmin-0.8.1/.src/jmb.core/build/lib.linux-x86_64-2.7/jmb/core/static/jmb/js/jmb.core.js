...
// e per aggiungi con il + in popup
var iframe_callback_bk; var last_add_click; var last_added_pk;
jmb.autocomplete_filterby_patch = function () {
	jQuery(document).ready(function($) {
		iframe_callback_bk = iframe_callback
		$(".autocomplete-add-another").each(function(){
			$(this).insertAfter($(this).parent())
		})	
		$(".autocomplete-add-another").on("click",function(){
			last_add_click = this
			href = $(this).attr("href") + "&_hjson=1"
			if(last_added_pk) { href = href.replace("/add","/" + last_added_pk);last_added_pk = undefined }
			iframe_callback = function(action, model, pk, content, message, method){
				iframe_callback = iframe_callback_bk;
				jmb.add_message(model + " " + gettext("aggiunto"),'info')
				//autoselezione
				$(last_add_click).parent().find("select.value-select").html('<option value="'+pk+'" selected="selected">'+pk+'</option>')
				$(last_add_click).parent().find(".deck.div").html("<span class='div hilight' data-value='"+pk+"'><span style='display: inline;' class='remove div'>X</span>"+content+"</span>")
				$(last_add_click).parent().find("input.autocomplete").hide()

				if (method == "_json_continue"){
					$(dialog).dialog("close");
					last_added_pk = pk
					$(last_add_click).click()
					return false;
			    }
				if (method == "_json"){
					$(dialog).dialog("close");
			...
			jmb.show_in_popup(href, function(){
			})
			return false;
		})
	})
