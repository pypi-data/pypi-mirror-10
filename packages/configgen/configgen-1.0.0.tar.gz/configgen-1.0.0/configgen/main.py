...
def parse_args(argv):
	...
	parser.add_argument('--option', '-o', action='append', help='an option string that will be used to choose which value to output for a given field name (may be passed multiple times)')
	...
	parser.add_argument('output', nargs='?', help='optional, file to which output is written, no output by default (see --printconfig)')
...
def main():
	...
	if args.config is None:
		...
		if numKeys == 0:
			...
			# must be exactly one Config instance, pop it and discard its key
			_, configInstance = configInstances.popitem()
	else:
		try:
