...
if sys.version_info[0] >= 3:
	basestring = str
	unicode = str
	# next is defined in python 3
	iteritems = lambda d: d.items()
else:
	# basestring is defined in python 2
	# unicode is defined in python 2
...
class MultiValue(object):
	...
	@classmethod
	def _initAllowableTypes(cls):
		cls.allowableTypes = frozenset([str, unicode, int, float, bool, type(None), dict, list, tuple, KeyValue])
	...
	def __init__(self, key, values, inheritedValue):
		...
		if isinstance(inheritedValue, MultiValue):
			...
			for valueOptions, value in iteritems(inheritedValue.values):
	...
	def selectValue(self, options):
		...
		foundTie = None
		# scan all subsets
		for valueOptions, value in iteritems(self.values):
	...
	def resolveName(self, initialKey, otherKeys, options=None):
		...
		try:
			...
			while not foundKeyValue:
				...
				try:
					isAttribute, nextFieldSelector = next(otherKeys)
...
# semi-hacky - set allowableTypes at the end of the file to allow KeyValue to be in the set, too
MultiValue._initAllowableTypes()
