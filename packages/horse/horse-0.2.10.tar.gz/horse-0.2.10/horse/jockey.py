...
import horse.config
import horse.slack
import horse.sevvers.http
import horse.sevvers.socket
...
class Jockey(object):
    ...
    def __init__(self):
        ...
        self.socket = horse.sevvers.socket.SocketServer(self)
        self.http = horse.sevvers.http.HTTPServer(self)
    ...
    def load_bridles(self):
        ...
        for bridle_path in horse.config.BRIDLES:
            ...
            if isinstance(bridle_path, basestring):
                if bridle_path.startswith('horse.ext'):
