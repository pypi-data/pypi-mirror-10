...
from config import g_CONFIG, RAINBOW_INVALID_CODE_MSG
import settings
...
def set_msg_hdl(
    ...
    message_id_dict = g_channel_msgid_hdl.get(channel, {})
    if not message_id_dict:
        g_channel_msgid_hdl[channel] = message_id_dict
    message_id_dict[message_id_channel] = {}
    message_id_dict[message_id_channel][
        'web_handle_response'] = web_handle_response
    # 客户端回复 ack 或 rec 表示收到
    message_id_dict[message_id_channel]['finish_list'] = {}
    message_id_dict[message_id_channel]['error_list'] = {}
    message_id_dict[message_id_channel]['client_count'] = client_count
    message_id_dict[message_id_channel]['rsp_data'] = ''


def clear_msg_hdl(channel, message_id_channel):
    if g_channel_msgid_hdl.get(channel) is None:
        return
    if g_channel_msgid_hdl[channel].get(message_id_channel) is None:
        return

    del g_channel_msgid_hdl[channel][message_id_channel]

    if g_channel_msgid_hdl[channel] == {}:
        del g_channel_msgid_hdl[channel]
...
class WebSocketHandler(Handler):
    ...
    def open(self):
        ...
        """
        if getattr(self, 'valid_request_rsp', None):
            self.rainbow_handle_header(self.valid_request_rsp.headers)
            self.valid_request_rsp = None

        set_identity_hdl(self.identity, self)
        log.info(
                ('open for identity = {identity}  '
                 'channels = {channels}  '
                 'self = {hdl}'.format(
    ...
    def on_close(self):
        ...
        try:
            log.info(
                ('on_close for identity = {identity}  '
                 'channels = {channels}   '
                 'self = {hdl}'.format(
                     identity=self.identity,
                     channels=self.channels,
    ...
    def close_funcs(self):
        ...
        for channel in self.channels:
            ...
            if not handlers:
                log.info('++ got WebSocketHandler.socket_handlers2[channel]')
                log.info('clear_channel_msg_data')
                # 如果这个 channel 的没有客户端
                clear_channel_msg_data(channel)
            else:
                log.info('++ no no WebSocketHandler.socket_handlers2[channel]')
    ...
    def packet_send_business_server(self, packet):
        ...
        if g_CONFIG['forward_url'].find(r) == -1:
            params = {'message_type': str(packet.msgtype)}
        else:
            params = {}

        url = g_CONFIG['forward_url'].replace(r, str(packet.msgtype))

        req = self.make_request(url, 'POST', params=params, body=packet.data)
    ...
    def channel_remove(self, channel):
        ...
        log.debug('channel_remove WebSocketHandler.socket_handlers2 = %s' %
                  WebSocketHandler.socket_handlers2)
        ...
        try:
            ...
            log.info('do not have channel = %s' % channel)
        ...
        try:
            ...
            if req:
                try:
                    ...
                    log.info('rsp.code = {code}'.format(code=rsp.code))
                    log.info('rsp.body = {body}'.format(body=rsp.body))
                    log.info('headers = {headers}'.format(headers=rsp.headers))
                    body = json.loads(rsp.body)
                    if body['status'] == 'success':
                        connect_flag = True
                        # self.rainbow_handle_header(rsp.headers)
                        # 在open的函数处理 headers
                        self.valid_request_rsp = rsp
    ...
    def get_valid_req_params(self):
        log.info('self.request.arguments = %s' % self.request.arguments)
        params = {}
        for param_key, param_val in self.request.arguments.iteritems():
            params[param_key] = param_val[0]
        log.debug('params = %s' % params)

        log.info('self.request.headers = %s' % self.request.headers)
        ...
        log.info(
            'identity = {identity}, identity_raw = {identity_raw}'.format(
                identity=self.identity, identity_raw=identity_raw))
        ...
        req = self.make_request(
            g_CONFIG['connect_url'], 'GET', params=params, headers=req_headers)
    ...
    def make_request(
        ...
        req = HTTPRequest(
            ...
            connect_timeout=15, request_timeout=15)
        log.debug('\n')
    ...
    def rainbow_handle_header(self, headers):
        ...
        h_channel1 = 'RAINBOWCLIENTCHANNELUNSUB'
        h_channel2 = 'Rainbowclientchannelunsub'
        channel = headers.get(h_channel1) or headers.get(h_channel2)
        log.debug('RAINBOWCLIENTCHANNELUNSUB = %s' % channel)
        if channel:
        ...
        channel = headers.get(h_channel1) or headers.get(h_channel2)
        log.info('RAINBOWCLIENTCHANNEL = %s identity = %s' % (
        ...
        h_invalid1 = 'RAINBOWREQUESTINVALID'
        h_invalid2 = 'Rainbowrequestinvalid'
        invalid_code = headers.get(h_invalid1) or headers.get(h_invalid2)
        if invalid_code:
            msg = RAINBOW_INVALID_CODE_MSG.get(invalid_code)
            if not msg:
                log.error('invalid_code = %s, but find no message, '
                          'upgrade your server' % invalid_code)
            else:
                log.error(msg)
...
def sub(identity, channel, occupy=False):
    ...
    """
    log.info(
        'identity = {identity} channel = {channel}'.format(
            identity=identity, channel=channel))
    if occupy:
        WebSocketHandler.channel_clean(channel)
...
def serverinfo():
    log.info('g_channel_msgid = %s' % g_channel_msgid)
    log.info('g_channel_msgid_hdl = %s' % g_channel_msgid_hdl)
    log.info('g_identity_wshandler = %s' % g_identity_wshandler)
    log.info('socket_handlers2 = %s' % WebSocketHandler.socket_handlers2)
    ...
    data['channels'] = []
    for channel, handlers in WebSocketHandler.socket_handlers2.iteritems():
        data['channels'].append((channel, len(handlers)))
