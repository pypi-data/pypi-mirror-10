...
class PrimaryColors(object):
    ...
    def __init__(self, image, max_colors=5):
        ...
        self.__colors = [''.join([hex(int(x))[2:] for x in rgb]) for rgb in self.__codes]
        total = s(counts)
        color_dist = dict(zip(self.__colors, [count/float(total) for count in counts]))
        self.__color_sorted = sorted(six.iteritems(color_dist), key=operator.itemgetter(1), reverse=True)
