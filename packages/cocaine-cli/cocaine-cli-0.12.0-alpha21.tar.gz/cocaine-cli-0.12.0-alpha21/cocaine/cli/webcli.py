    ...
    try:
        ...
        for k,v in routing.items():
            if not isinstance(k, str):
                raise TypeError("routing key '%s' should be str")
            if not isinstance(v,int):
                try:
                    routing[k] = int(v)
                except ValueError as err:
                    raise TypeError("routing value '%s' should be int"%v)
...
@show.command(name="crashlog")
@click.option("--list", "do_list", is_flag=True)
@click.option("--timestamp")
@click.argument("target")
def show_crashlog(do_list, timestamp, target):
    ...
    if do_list:
        ...
        r = request.get(normalize_url(BASE+"/project/%s/version/%s/cluster/%s/crashlog/%s"%(project_id, version_tag, target, timestamp)))
