...
from atxt.formats._utils import save_raw_data
from atxt.infofile import InfoFile
from atxt.utils import make_dir
from linkGrabber.linkGrabber import Links
...
tablereplace = string.maketrans('/.?&:=%', '_' * 7)
...
def scraper(url, level_target=0, to=None, sameroot=False, overwrite=False):
    ...
    while level + 1 <= level_target:
        ...
        archivo = archivo.translate(tablereplace)
    ...
    log.info('Number of finished urls: %s' % finished)
