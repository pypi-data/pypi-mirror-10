...
LOGGING = {
    ...
    'handlers': {
        ...
        'default': {
            ...
            'filename': os.path.join(BASE_DIR, 'logs', 'info.log'),  # 或者直接写路径：'c://logs/all.log',
        ...
        'exception': {
            'level': 'ERROR',
            'class': 'logging.handlers.RotatingFileHandler',
            'filename': os.path.join(BASE_DIR, 'logs', 'exception.log'),  # 或者直接写路径：'c://logs/all.log',
            'maxBytes': 1024 * 1024 * 10,  # 5 MB
            'backupCount': 5,
            'formatter': 'plain',
        },
...
if not os.path.exists(os.path.join(BASE_DIR, 'logs')):
    os.system('mkdir -p %s' %  os.path.join(BASE_DIR, 'logs'))

# 根据环境不同引用不同的扩展settings
server_env = None
if os.path.exists(os.path.join(BASE_DIR, 'server_tag_local')): 
    server_env = 'local'
    from settings_local import *
if os.path.exists(os.path.join(BASE_DIR, 'server_tag_online')): 
    server_env = 'online'
    from settings_online import *
if os.path.exists(os.path.join(BASE_DIR, 'server_tag_dev')): 
    server_env = 'dev'
    from settings_dev import *
if os.path.exists(os.path.join(BASE_DIR, 'server_tag_release')):
    server_env = 'release'
