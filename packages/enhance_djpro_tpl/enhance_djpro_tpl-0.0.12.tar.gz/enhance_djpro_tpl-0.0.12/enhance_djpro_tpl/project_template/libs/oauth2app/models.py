...
import datetime
#from base.models import get_date_time


def get_date_time(oint=None):
    '''
    将时间戳转换为对应的时间对象
    '''
    if oint is None:
        return datetime.datetime.now()
    if isinstance(oint, int) or isinstance(oint, long):
        return datetime.datetime.fromtimestamp(oint)
    return oint
...
class Client(models.Model):
    ...
    * *redirect_uri:* A string representing the client redirect_uri.
    ...
    def __unicode__(self):
        return u'%s-%s' % (self.key, self.secret)
...
class AccessToken(models.Model):
    ...
    * *refreshable:* A boolean that indicates whether this access token is
    ...
    def get_expire_time(self):
        return get_date_time(self.expire)

    get_expire_time.allow_tags = True
    get_expire_time.short_description = u'过期时间'
