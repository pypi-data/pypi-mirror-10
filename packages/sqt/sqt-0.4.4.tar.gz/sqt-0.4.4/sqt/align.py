...
def multialign(sequences, program='mafft'):
	"""
	...
	* MUSCLE (http://www.drive5.com/muscle/)
	...
	program -- must be 'clustalo', 'mafft', 'muscle', 'muscle-fast'. The latter
		calls MUSCLE with parameters that make it run faster (but less accurate).
	"""
	if program == 'mafft':
		args = ['mafft', '--quiet', '-']
	elif program == 'clustalo':
		args = ['clustalo', '--infile=-']
	elif program == 'muscle':
...
def consensus(aligned, threshold=0.7, ambiguous='N', keep_gaps=False):
	...
	n = len(aligned)
	ambiguous = 'N'
	for chars in zip(*aligned.values()):
