...
class SmbAcl(Acl):
    ...
    def __repr__(self):
        return '{SMB ACL: read: %s, write: %s, valid users: %s, invalid users: %s}' %(
...
class SmbShareService(ShareService):
    def __init__(self):
        ...
        self.config = RawConfigParser(delimiters=('='))
    ...
    def writeShare(self, share):
        ...
        if name in self.config.sections():
            ...
            if len(share.acl.write) >= 1:
                self.config[name]['write list'] = ' '.join(share.acl.write)
            if len(share.acl.read) >= 1:
                self.config[name]['read list'] = ' '.join(share.acl.read)
