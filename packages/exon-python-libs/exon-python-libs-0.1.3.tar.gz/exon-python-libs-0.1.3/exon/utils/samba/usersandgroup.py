...
class UsersAndGroup(object):
    ...
    _NON_EDIT_PARAMETERS = ('primaryGroupID')
    ...
    def _modifyUser(self, userData = {}, unix = None):
        ...
        for key in userData:
            if key == 'unix_user':
                ...
                if key in adData:
                    if userData[key] != adData[key] and key not in self._NON_EDIT_PARAMETERS:
    ...
    def _modifyGroup(self, name, unixUser, members):
        ...
        if unixAdd:
            ...
            ldif.append('-')
            ldif.append('replace: msSFU30Name')
        ...
        if len(ldif) > 2:
            ret = modifyLdap(self._ldbSamFile, ldif)
            if type(ret) == dict:
                raise SAMBAUserException('can not modify group in ldap. STDOUT: %s STDERR: %s LDIFF: %s' % (ret['stdout'], ret['stderr'], ret['ldif']))
        return self.getGroupByAccount(name)
