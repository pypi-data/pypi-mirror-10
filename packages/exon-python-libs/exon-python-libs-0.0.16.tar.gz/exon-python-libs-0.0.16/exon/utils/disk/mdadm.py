                ...
                if line.startswith("Raid Level"):
                    ...
                    if self.state != "ok":
                        ...
                        failRegex = re.compile(
                            '.+?recovery =\s+?(\d+?\.\d+?%)\s+?\((\d+?\/\d+?)\)\s+?finish=(\d+?\.\d+?.+?)\s+?speed=(.+)')
...
class Mdadm(object):
    ...
    def delete(self, device=None, zero=False):
        ...
        if outDel.rc != 0:
            raise MdadmCreateRaidException("Cannot stop Raid. STDOUT: %s STDERR: %s" % (outDel.stdout, outDel.stderr))
        if zero:
            outZero = exececute("mdadm --zero-superblock %s" % " ".join(raidDevs))
            if outZero.rc != 0:
                raise MdadmCreateRaidException("Cannot zero superbolocks. STDOUT: %s STDERR: %s" % (outZero.stdout, outZero.stderr))
...
if __name__ == "__main__":
    mdadm = Mdadm()
    m = mdadm.createRaid(name="Test Raid", devices=["sdb1", "sdc1", "sdd1"], level=5, part=True)
    print(m)
    print(type(m))
