...
class Filesystems(object):
    ...
    def addToFstab(self, device = None, mountpoint = None, type=None, options = None, dump = None, fsck = None, useUuid=False):
        ...
        """
        if device == None:
            raise FilesystemException("device must not be null")
        if mountpoint == None:
            raise FilesystemException("mountpoin must not be null")
        if type == None:
            raise FilesystemException("type must not be null")
        uuid = None
        if useUuid:
        ...
        if fstab == None:
            ...
            if fsck == None:
                fsck = "0"
            self._appendFs("%s\t%s\t%s\t%s\t%s %s" % (device, mountpoint, type, options, dump, fsck))
        else:
            if useUuid:
    ...
    def formatDisk(self, device=None, type=None):
        ...
        if type not in Filesystems.SUPPORTED_FILESYSTEMS:
            raise FilesystemException("fs type %s is not supported. Supported types are %s" % (type, Filesystems.SUPPORTED_FILESYSTEMS))
        ...
        if out.rc == 0:
            ...
            raise FilesystemException("error while formation %s. STDOUT: %s STDERR: %s CMD: %s" % (device, out.stdout, out.stderr, mkfs))


    def deleteFsByUuid(self, uuid):
        fs = self.getByUUUID(uuid)
        if fs == None:
            return True
        fstabFile = os.path.join('/','etc','fstab')
        newFstab = []
        ...
        writefileFromArray(fstabFile, newFstab)
        self.refresh()
        f2 = self.getByUUUID(uuid)
        if f2 == None:
            return True
        elif fs.mountpoint != f2.mountpoint:
            return True
        return '%s - %s' % (fs.mountpoint , f2.mountpoint)
