...
class PickleFile(object):
    ...
    def io_name(self):
        return self.io_prefix + unicode(self.offset)

    def cpu_name(self):
        return self.cpu_prefix + unicode(self.offset)
...
class FileQueue(list):
    ...
    def __init__(self, max_size, chunk_size, process_count, offset, file_lambda):
        ...
        for chunk1 in itertools.count(0, chunk_size):
            if (chunk1 - max_size) >= 0:
                break
            if (chunk1 / chunk_size) % process_count != offset:
                continue
...
class ParallelData(object):
    ...
    def process(cls, datasource, datasource_type, **attrs):
        ...
        if (len(ps.datasource) - ps.result_len) > ps.offset:
            ps.recache()

        if ps.result_len == 0:
            os.system("rm -f %s" % ps.cache_filename)
