...
#				 29/05/2015 - When ATD has analyzed a file recently, the veridict of
#							  of the file is blaclisted on the ATD server.
#							  In this case, sometimes ATD doesn't report correctly
#							  the blacklisted value via the json string reporting a
#							  final veridict of 0, however in the ATD console the
#							  information showed is correct. 
#							  I have changed the script so if the OSVersion reports
#							  that the file is blacklisted I don't get the final 
#							  veredict and I force the veredict to be 5
#				 30/05/2015 - The following new functionalities are included:
#								- The script is now able to upload a single file. Use
#								the same -path parameter to do it pointing to the 
#								final file_name.
#								- The script support send a file to quarantine when
#								a file with the same name exits on the quarantine 
#								folder
#								- The script now works on Windows as well as Linux
#								systems (IMPORTANT - No tested)
#-------------------------------------------------------------------------------
...
#     666 -> Zip file protected with password, so the system can not analyze it
#**************************************************************************************************************************************************
...
atddir.py -u admin -p admin -atd 192.168.0.202 -path /path/malware -vm 20 -q c:\quarantine -monitor

Analyze the folder /path/malware in monitor mode (so it never ends and scan for new files) if the file is supicious (>=3) it will be sent to quarantine

4.-)

atddir.py -u admin -p admin -atd 192.168.0.202 -path /path/malware -vm 20 -q c:\quarantine -monitor -delta 86400
...
Even if the file filename.pdf has been already analyzed, so it is cache locally the -force paramete forces the rea-analisys of the file    
    		      '''      
...
def get_filename_path(filetosubmit):
	'''
	Description: Internal procedure to get a clean filename and a path when -path parameter points to a final file
				 This procedure supports Windows and 
	Input:       path to file
	Output:      tupe with the clean filename and the path
	'''
...
def get_list_and_hash(path_to_folder):
	...
	# Debug if the path points to a final file the return just the file
	...
	if os.path.isfile(path_to_folder):
		...
		try:
			...
			print 'Error opening file: %s or calculating hash - continuing the process'%path_to_file
...
def upload_to_atd(thread_id, q, atd_ip, atd_user, atd_pass, vm_id, q_path):
	...
	# ***************************************************************************************************************
	blacklist = False # Linea introducida de depuracion
	...
	while True:
		...
		while True:
			...
			if error_control == 1:
				...
				try:
					...
					elif data['Summary']['OSversion'] == 'Blacklist': # Debug linea introducida de depuracion
						blacklist = True # Debug linea introducida de depuracion
				except Exception as er:
				...
				elif blacklist: # Debug linea introducida de depuracion
					severity = '5' # Debug linea introducida de depuracion
					description = 'File already on blacklist - use the appropiated profile for a full scan ' # Debug line de depuracion
				# *****************************************************************************************************************
				break
			# error_control = 29
			time.sleep(30)
		...
		text_to_print = '\n[+] File: %s\n [-] Hash value: %s\n [-] Analisys result value: %s\n [-] Description: %s'%(path_to_folder + os.sep + file_name, file_hash, severity.encode('ascii','ignore'), description.encode('ascii','ignore'))
		text_to_log = '%s | %s | %s | %s | New annalisys perform - %s\n'%(datetime.strftime(datetime.now(), '%Y/%m/%d %H:%M:%S'), path_to_folder + os.sep + file_name, file_hash, severity.encode('ascii','ignore'), description.encode('ascii','ignore'))
		# ***************************************************************************************************************************
...
def quarantine_file(actual_path, new_path, file_name, severity):
	...
	if int(severity) > 2 and new_path != None:
		...
		# Check if file exits before to rename it
