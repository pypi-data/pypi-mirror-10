...
'''
@python_2_unicode_compatible
class MailMassNotification(models.Model):
    BACKENDS = (
    ...
    backend = models.CharField(choices=BACKENDS, db_index=True, max_length=50)
    start_hour = models.PositiveSmallIntegerField(default=0)
    end_hour = models.PositiveSmallIntegerField(default=23)
    is_enabled = models.BooleanField(default=True)

    """
    prowl: api_key,  boxcar2: token,  push_over: user,  parse_com: device_id
    mail: email_address
    sms: phone_number
    tts: phone_number
    """
    ...
    @staticmethod
    def _get_values(field, backend, **kwargs):
        now_hour = datetime.datetime.now().hour
        kwargs.update({
            'start_hour__lte': now_hour,
            'end_hour__gte': now_hour,
            'backend': backend,
            'is_enabled': True,
        })
        if kwargs.pop('force', False) is True:
            del kwargs['start_hour__lte'], kwargs['start_hour__gte']
        return list(set(MailMassNotification.objects.values_list(
            field, flat=True).filter(**kwargs)))

    def notify(self, slug, user_id, **kwargs):
        from dbmail import db_sender

        for backend in self.BACKENDS:
            for recipient in self._get_values(user_id, backend[0]):
                kwargs['backend'] = backend[0]
                db_sender(slug, recipient, **kwargs)
'''
