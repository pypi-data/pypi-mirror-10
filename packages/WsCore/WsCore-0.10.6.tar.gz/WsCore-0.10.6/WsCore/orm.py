...
class Model(dict):
    ...
    def insert(self):
        ...
        args = tuple(args)
        with MySQLHelper(db_host=self.__conn__['db_host'], db_user=self.__conn__['db_user'],
                         db_pwd=self.__conn__['db_pwd'], db_name=self.__conn__['db_name']) as sql:
    ...
    def execute(cls, sql_str, where=None, args=None):
        ...
        sql_str = sql_str.replace('?', '%s')
        with MySQLHelper(db_host=cls.__conn__['db_host'], db_user=cls.__conn__['db_user'],
                         db_pwd=cls.__conn__['db_pwd'], db_name=cls.__conn__['db_name']) as sql:
            return sql.execute(sql_str, args)
