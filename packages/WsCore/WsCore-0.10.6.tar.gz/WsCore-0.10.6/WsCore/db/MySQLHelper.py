...
class MySQLHelper(object):
    def __init__(self, db_host, db_user, db_pwd, db_name):
        self._host = db_host
        self._host = db_user
        self._host = db_pwd
        self._host = db_name
        self.last_id = 0
        self.conn = MySQLdb.connect(host=self._host, user=self._host, passwd=self._host, db=self._host, charset='utf8')
        if self.conn:
