...
$("#buscar-modal").on("click",function(){
	...
	$.ajax({
 	   	...
 	   	data : {
 	   		...
 	   		'clave':clave,
 	   		'nombre':nombre,
 	   		'tipo_cambio':$("#id_tipo_cambio_d").val(),
 	   	},
 	   	success: function(data){
 	   		if (data.articulos.length > 0)
 	   		{
	 	   		...
	 	   		data.articulos.forEach(function(articulo) {
	 	   			...
	 	   			{
	 	   				precio_convertido = parseFloat(articulo[5]).toFixed(2);
	 	   			};
					$('#articles_table').find('tbody').append( "<tr> <td> "+articulo[2]+" </td> <td id='nombre'>"+  articulo[1]+"</td> <td>"+articulo[4]+"</td><td>"+parseFloat(articulo[3]).toFixed(2)+" <small> "+articulo[6]+"</small> </td> <td> "+precio_convertido+"</td> <td> <button class='btn btn-default add'><i class='glyphicon glyphicon-plus'></i><input type='hidden' value='"+articulo[0]+"'> </a> </td> </tr>" );	
