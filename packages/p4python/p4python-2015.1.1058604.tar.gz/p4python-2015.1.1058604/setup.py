...
MIN_SSL_VERSION=101
MIN_SSL_RELEASE='k' # 1.0.1k required if built with SSL
...
def get_api_dir():
    if '--apidir' in sys.argv:
        ...
        if not p4_api_dir:
            ...
            print( "Loaded API into {}".format(p4_api_dir) )
    ...
    if match:
        ...
        if version >= MIN_SSL_VERSION:
            ...
            if release >= MIN_SSL_RELEASE:
                ...
                print("***************************************",file=sys.stderr)
                print("Minimum SSL release required is 1.0.1{}".format(MIN_SSL_RELEASE),file=sys.stderr)
                print("Building without SSL",file=sys.stderr)
                print("***************************************",file=sys.stderr)
        else:
            print("***************************************",file=sys.stderr)
