...
(function($) {
    ...
    CropDuster = {
        ...
        getUploadUrl: function(prefix, cropdusterUrl) {
            var params = {
                'image': 'image',
                'id': 'id',
                'thumbs': 'thumbs'
            ...
            return cropdusterUrl;
        },

        // open upload window
        show: function(prefix, cropdusterUrl) {
            var windowName = String(prefix).replace(/\-/g,"____").split(".").join("___");
            cropdusterUrl = this.getUploadUrl(prefix, cropdusterUrl);
            window.open(cropdusterUrl, windowName, 'height=650,width=960,resizable=yes,scrollbars=yes').focus();
        },

        showIframe: function(prefix, cropdusterUrl) {
            cropdusterUrl = this.getUploadUrl(prefix, cropdusterUrl);
            var $iframe = $('<iframe src="' + cropdusterUrl + '" width="850" height="500" allowtransparency="true"></iframe>');
            $('#id_' + prefix).parent().append($iframe);
        },
        ...
        registerInput: function(input) {
            ...
            if (false) {
                ...
                var $targetInput = $customField.closest('.form-row,.row,.grp-row').find('.cropduster-data-field').eq(0);
                var fieldName = $targetInput.attr('name');
                var inputData = $targetInput.data();
                var cropdusterUrl = $customField.data('cropdusterUrl');
                var arg = (cropdusterUrl.indexOf('?') >= 0) ? '&' : '?';
                if (inputData.uploadTo) {
                    cropdusterUrl += arg + 'upload_to=' + encodeURI(inputData.uploadTo);
                }
                CropDuster.showIframe(fieldName, cropdusterUrl);
            }
