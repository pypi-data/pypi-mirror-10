...
class DistributedTypeRegistry(object):
    ...
    def compute_hash(self, key):
        ...
        if isinstance(key, str):
            ...
            data = self.compute_hash(key.__dict__)
        elif isinstance(key, (bool, int, long, float,)):
            data = str(dill.dumps(key))
        elif key is None:
            data = "__None__"
        elif isinstance(key, (tuple, list)):
            data = "".join(self.compute_hash(item) for item in key)
        elif isinstance(key, set):
            data = "".join(self.compute_hash(item) for item in sorted(key))
        elif isinstance(key, dict):
            data = "".join(self.compute_hash(item) for item in key.items())
        else:
...
class _CachedFunction(object):
    ...
    def _put(self, name, hash_code_args, result, extra=None):
        item = self._store(name, result, hash_code_args, extra)
        self._cache[hash_code_args] = item
        return item

    def _store(self, name, what, hashcode_args, extra):
    ...
    def cached_call(self, args, kw):
        ...
        try:
            ...
            hash_code = self.tr.compute_hash(args_for_hash)
        except RuntimeError:
        ...
        if name == "":
            name = None
        item = self._put(name, hash_code, result, extra)
        return result, item
