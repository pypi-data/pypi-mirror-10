...
@click.option('--with-import/--without-import', help='Add import statement', default=True)
def sqlalchemy(dsn, table, limit=None, where=None, order_by=None, with_import=True):
    """
    Provide DSN and Table name for fixture generation
    """
    try:
        click.echo(generate(*sqlalchemy_data(table, dsn, limit, where, order_by), with_import=with_import))
    except NoSuchTable:
