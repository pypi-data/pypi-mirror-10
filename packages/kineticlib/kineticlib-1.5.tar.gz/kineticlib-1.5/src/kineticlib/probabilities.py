...
from math import exp, cosh, atan, sinh, tan
...
def ssh_vt_p10(T: float, sigma: float, mass: float, eps: float, hvc: float) -> float:
    """Returns the probability of a VT transition using the SSH model
    **M(1) + P -> M(0) + P**, where the vibrational spectrum of the molecule **M** is harmonic
...
def ssh_vv_p0110(T: float, sigma: float, mass: float, hvc: float) -> float:
    """Returns the probability of a VV transition using the SSH model
    **M(0) + M(1) -> M(1) + M(0)**, where **M** is a homonuclear molecule
    ...
    """
    return 3.8787 * constants.k * T * (constants.h / (sigma * hvc))**2 / mass


def vt_probability_fho(g: float, T: float, mass: float, beta: float, osc_mass: float,
    ...
    **M(i) + P -> M(i + delta) + P**, where **M** is a homonuclear molecule
    ...
    osc_mass : float
        The oscillator reduced mass of the molecule undergoing the transition
    ve_before : float
        the vibrational energy of the molecule before the transition
    ve_after : float
        the vibrational energy of the molecule after the transition
    ...
    if vel < 0.0:
        return 0.0
    # phi = 2.0 * atan(((2.0 * molecule_diss / mass) ** 0.5) / vel) / constants.pi

    # eps = (cosh((1.0 + phi) * constants.pi * omega / (beta * vel))
    #        / (sinh(2.0 * constants.pi * omega / (beta * vel)))) ** 2
    eps = .5 / (sinh(constants.pi * omega / (beta * vel)))**2
    eps *= (constants.pi ** 2) * svt(delta) * omega * mass**2 / (osc_mass * (beta ** 2) * constants.h)
    return ns * (eps ** s) * exp(- 2.0 * (ns ** (1.0 / s)) * eps / (s + 1)) / (factorial(s) ** 2)


def vt_prob_g_only_fho(g: float, T: float, mass: float, beta: float, osc_mass: float,
                       ve_before: float, ve_after: float, i: int, delta: int, this_svt: float=0.33) -> float:
    """Returns the velocity-dependent part of the probability of a VT transition using
    the FHO (Forced harmonic oscillator) model **M(i) + P -> M(i + delta) + P**, where **M** is a homonuclear molecule
    ...
    if delta == 1:
        ...
        # phi = 2.0 * atan(((2.0 * depth / mass) ** 0.5) / vel) / constants.pi

        # eps = (cosh((1.0 + phi) * constants.pi * omega / (beta * vel))
        #        / (sinh(2.0 * constants.pi * omega / (beta * vel)))) ** 2
        eps = .5 / (sinh(constants.pi * omega / (beta * vel)))**2
        eps *= 2. * this_svt * (constants.pi ** 2) * omega * mass**2 / (osc_mass * (beta ** 2) * constants.h)
...
def vv_prob_g_only_fho(g: float, T: float, mass: float, beta: float, ve_before1: float, ve_after1: float,
                       ve_before2: float, ve_after2: float,
                       i: int, k: int, i_delta: int, this_svv: float=0.04) -> float:
