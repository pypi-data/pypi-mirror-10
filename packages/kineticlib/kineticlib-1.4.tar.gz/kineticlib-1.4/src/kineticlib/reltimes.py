...
from .particles import Particle, Molecule, MoleculeQuasi, MoleculeMultiT, MoleculeOneT
import numpy as np
...
def vt_rel_time_def(T: float, T1: float, idata: np.ndarray, molecule: MoleculeQuasi, n: float,
                    model: str='VSS') -> float:
    """Calculates the VT relaxation time using the strict definition
    ...
    ------
    KineticlibError
        If ``molecule`` is not of type ``MoleculeMultiT`` or ``MoleculeOneT``
    """
    res = 0.
    ve_dimless = molecule.vibr / (constants.k * T)

    if isinstance(molecule, MoleculeMultiT):
        for i in range(molecule.num_vibr_levels(T, T1)):
            res += (ve_dimless[i + 1] - ve_dimless[i])**2 * molecule.vibr_exp(T, T1, i + 1)\
                   * cs.vt_integral_fho(T, 0, idata, molecule, i + 1, -1, model=model, nokt=True)

            # v -> v + 1
            res += (ve_dimless[i + 1] - ve_dimless[i])**2 * molecule.vibr_exp(T, T1, i)\
                   * cs.vt_integral_fho(T, 0, idata, molecule, i, 1, model=model, nokt=True)
        res *= (T * constants.k)**0.5
        return 1.0 / (4.0 * constants.k * n * res / (molecule.Z_vibr(T, T1)
                                                     * molecule.mass * molecule.E_vibr_dT1(T, T1)))
    elif isinstance(molecule, MoleculeOneT):
        for i in range(molecule.num_vibr):
            res += (ve_dimless[i + 1] - ve_dimless[i])**2 * molecule.vibr_exp(T, i + 1)\
                   * cs.vt_integral_fho(T, 0, idata, molecule, i + 1, -1, model=model, nokt=True)

            # v -> v + 1
            res += (ve_dimless[i + 1] - ve_dimless[i])**2 * molecule.vibr_exp(T, i)\
                   * cs.vt_integral_fho(T, 0, idata, molecule, i, 1, model=model, nokt=True)
        res *= (T * constants.k)**0.5
        return 1.0 / (4.0 * constants.k * n * res / (molecule.Z_vibr(T)
                                                     * molecule.mass * molecule.E_vibr_dT(T)))
    else:
        raise KineticlibError('Molecule is not of type MoleculeMultiT or MoleculeOneT')
...
#     """Calculates the high-temperature correction to the VT relaxation time (Park's correction), *raw* version
#
#     Parameters
#     ----------
#     T : float
#         the temperature of the mixture
#     mass : float
#         the mass of the molecule for which the VT relaxation time is being calculated
#     n : float
#         the numeric density of the mixture
#
#     Returns
