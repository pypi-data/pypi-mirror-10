...
def ssh_vv_p0110(T: float, sigma: float, mass: float, hvc: float) -> float:
    ...
    **M(1) + M(0) -> M(0) + M(1)**, where **M** is a homonuclear molecule
...
def vt_probability_fho(g: float, T: float, mass: float, beta: float, ve_before: float, ve_after: float,
    ...
    **M(i) + P -> M(i + delta) + P**, where **M** is a homonuclear molecule
    ...
    eps = 1. / (sinh(1.0 * constants.pi * omega / (beta * vel)))**2
    eps *= (constants.pi ** 2) * svt(delta) * omega * mass / ((beta ** 2) * constants.h)
    return ns * (eps ** s) * exp(- 2.0 * (ns ** (1.0 / s)) * eps / (s + 1)) / (factorial(s) ** 2)


def vt_prob_g_only_fho(g: float, T: float, mass: float, beta: float,
    ...
    if delta == 1:
        ...
        #        / (sinh(2.0 * constants.pi * omega / (beta * vel)))) ** 2
        eps = 1. / (sinh(1.0 * constants.pi * omega / (beta * vel)))**2
        eps *= this_svt * (constants.pi ** 2) * omega * mass / ((beta ** 2) * constants.h)
