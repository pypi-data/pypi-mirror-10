...
#
# def ssh_vv_p0110(T: float, sigma: float, mass: float, eps: float, hvc: float) -> float:
#     """Returns the probability of a VV transition using the SSH model
#     **M1(1) + M2(0) -> M1(0) + M2(1)**, where **M1** and **M2** are homonuclear molecules
#     with harmonic vibrational spectra
#
#     Parameters
#     ----------
#     T : float
...
#     alpha = 17.5 / sigma
#     omega = 2 * constants.pi * hvc / constants.h
#     p0110 = 0.0625 * 4 * constants.k * T * (alpha / omega)**2
#     return p0110


def vt_probability_fho(g: float, T: float, mass: float, morse_repulsion: float, ve_before: float, ve_after: float,
    ...
    **M(i) + P -> M(i + delta) + P**, where **M** is a homonuclear molecule
    ...
    mass : float
        the collision-reduced mass :math:`m_{cd}`
    morse_repulsion : float
        the repulsion power parameter of the Morse potential of the molecule undergoing the transition
    ve_before : float
        the vibrational energy of the molecule before the transition
    ve_after : float
        the vibrational energy of the molecule after the transition
    ...
    morse_repulsion = 4 * 10**10
    eps = 1. / (sinh(1.0 * constants.pi * omega / (morse_repulsion * vel)))**2
    eps *= (constants.pi ** 2) * svt(delta) * omega * mass / ((morse_repulsion ** 2) * constants.h)
    return ns * (eps ** s) * exp(- 2.0 * (ns ** (1.0 / s)) * eps / (s + 1)) / (factorial(s) ** 2)


def vt_prob_g_only_fho(g: float, T: float, mass: float, morse_repulsion: float,
    ...
    if delta == 1:
        ...
        #        / (sinh(2.0 * constants.pi * omega / (beta * vel)))) ** 2
        eps = 1. / (sinh(1.0 * constants.pi * omega / (morse_repulsion * vel)))**2
        eps *= this_svt * (constants.pi ** 2) * omega * mass / ((morse_repulsion ** 2) * constants.h)
