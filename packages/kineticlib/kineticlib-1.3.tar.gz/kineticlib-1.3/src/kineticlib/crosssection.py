...
def crosssection_vt_fho(g: float, T: float, idata: np.ndarray, molecule: Molecule, i: int, delta: int,
    ...
    if delta != 0:
        if model == 'GSS':
            return (constants.physical_constants['Angstrom star'][0] ** 2) *\
                   raw_crosssection_vt_gss_fho(g, T, idata[0], idata[16], idata[17], idata[18], idata[19],
                                               molecule.vibr[i], molecule.vibr[i + delta], i, delta)
        elif model == 'VSS':
            return (constants.physical_constants['Angstrom star'][0] ** 2) *\
                   raw_crosssection_vt_vss_fho(g, T, idata[0], idata[9], idata[10], molecule.vibr[i],
                                               molecule.vibr[i + delta], i, delta)
        else:
            return raw_crosssection_vt_rigid_sphere_fho(g, T, idata[0], idata[0], molecule.vibr[i],
                                                        molecule.vibr[i + delta], i, delta)
    else:
...
def vt_integral_fho(T: float, deg: int, idata: np.ndarray, molecule: Molecule, i: int, delta: int,
    ...
    """
    if model == 'GSS':
        return raw_vt_integral_gss_fho(T, deg, idata[0], idata[16], idata[17], idata[18], idata[19],
                                       molecule.vibr[i], molecule.vibr[i + delta], i, delta, nokt)
    elif model == 'VSS':
        return raw_vt_integral_vss_fho(T, deg, idata[0], idata[9], idata[10], molecule.vibr[i],
                                       molecule.vibr[i + delta], i, delta, nokt)
    else:
        return raw_vt_integral_rigid_sphere_fho(T, deg, idata[1], idata[0], molecule.vibr[i],
                                                molecule.vibr[i + delta], i, delta, nokt)
...
def raw_crosssection_vt_gss_fho(g: float, T: float, mass: float, gssc1: float, gssc2: float,
    ...
    float
        The VT crosssection
    """
    return prob.vt_probability_fho(g, T, mass, 0., ve_before, ve_after, i, delta)\
           * raw_crosssection_elastic_gss(g, T, gssc1, gssc2, gsso1, gsso2)


def raw_crosssection_vt_vss_fho(g: float, T: float, mass: float, vssc: float, vsso: float,
                                ve_before: float, ve_after: float, i: int, delta: int) -> float:
...
def raw_crosssection_vt_rigid_sphere_fho(g: float, T: float, sigma: float, mass: float,
                                         ve_before: float, ve_after: float, i: int, delta: int) -> float:
...
def raw_vt_integral_gss_fho(T: float, deg: int, mass: float, gssc1: float, gssc2: float,
    ...
    if delta != 0:
        ...
        svt = prob.svt(delta)
        f1 = lambda g: prob.vt_prob_g_only_fho(g, T, mass, 0., ve_before, ve_after,
                                               i, delta, svt)\
                    * elastic_omega_g_only_vss(g, deg, -gsso1)
        f2 = lambda g: prob.vt_prob_g_only_fho(g, T, mass, 0., ve_before, ve_after,
                                               i, delta, svt)\
...
def raw_vt_integral_vss_fho(T: float, deg: int, mass: float, vssc: float, vsso: float,
    ...
    if delta != 0:
        ...
        svt = prob.svt(delta)
        f = lambda g: prob.vt_prob_g_only_fho(g, T, mass, 0., ve_before, ve_after,
                                              i, delta, svt)\
...
def raw_vt_integral_rigid_sphere_fho(T: float, deg: int, sigma: float, mass: float,
