...
def crosssection_vt_fho(g: float, T: float, idata: np.ndarray, molecule: Molecule, i: int, delta: int,
    ...
    if delta != 0:
        if model == 'GSS':
            return (constants.physical_constants['Angstrom star'][0] ** 2) *\
                   raw_crosssection_vt_gss_fho(g, T, idata[0], 1., idata[16], idata[17], idata[18], idata[19],
                                               molecule.vibr[i], molecule.vibr[i + delta], i, delta)
        elif model == 'VSS':
            return (constants.physical_constants['Angstrom star'][0] ** 2) *\
                   raw_crosssection_vt_vss_fho(g, T, idata[0], 1., idata[9], idata[10], molecule.vibr[i],
                                               molecule.vibr[i + delta], i, delta)
        else:
            return raw_crosssection_vt_rigid_sphere_fho(g, T, idata[1], idata[0], 1., molecule.vibr[i],
                                                        molecule.vibr[i + delta], i, delta)
    else:
...
def crosssection_vv_fho(g: float, T: float, idata: np.ndarray, molecule1: Molecule, molecule2: Molecule, i: int,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**
    ...
    if i_delta != 0:
        if model == 'GSS':
            return (constants.physical_constants['Angstrom star'][0] ** 2) *\
                   raw_crosssection_vv_gss_fho(g, T, idata[0], 1., idata[16], idata[17], idata[18], idata[19],
                                               molecule1.vibr[i], molecule1.vibr[i + i_delta],
                                               molecule2.vibr[k], molecule2.vibr[k - i_delta], i, k, i_delta)
        elif model == 'VSS':
            return (constants.physical_constants['Angstrom star'][0] ** 2) *\
                   raw_crosssection_vv_vss_fho(g, T, idata[0], 1., idata[9], idata[10],
                                               molecule1.vibr[i], molecule1.vibr[i + i_delta],
                                               molecule2.vibr[k], molecule2.vibr[k - i_delta], i, k, i_delta)
        else:
            return raw_crosssection_vv_rigid_sphere_fho(g, T, idata[1], idata[0], 1.,
                                                        molecule1.vibr[i], molecule1.vibr[i + i_delta],
                                                        molecule2.vibr[k], molecule2.vibr[k - i_delta], i, k, i_delta)
    else:
...
def vt_integral_fho(T: float, deg: int, idata: np.ndarray, molecule: Molecule, i: int, delta: int,
    ...
    """
    if model == 'GSS':
        return raw_vt_integral_gss_fho(T, deg, idata[0], 1., idata[16], idata[17], idata[18], idata[19],
                                       molecule.vibr[i], molecule.vibr[i + delta], i, delta, nokt)
    elif model == 'VSS':
        return raw_vt_integral_vss_fho(T, deg, idata[0], 1., idata[9], idata[10], molecule.vibr[i],
                                       molecule.vibr[i + delta], i, delta, nokt)
    else:
        return raw_vt_integral_rigid_sphere_fho(T, deg, idata[1], idata[0], 1., molecule.vibr[i],
                                                molecule.vibr[i + delta], i, delta, nokt)
...
def vv_integral_fho(T: float, deg: int, idata: np.ndarray, molecule1: Molecule, molecule2: Molecule, i: int,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**
    ...
    """
    if model == 'GSS':
        return raw_vv_integral_gss_fho(T, deg, idata[0], 1., idata[16], idata[17], idata[18], idata[19],
                                       molecule1.vibr[i], molecule1.vibr[i + i_delta],
                                       molecule2.vibr[k], molecule2.vibr[k - i_delta], i, k, i_delta, nokt)
    elif model == 'VSS':
        return raw_vv_integral_vss_fho(T, deg, idata[0], 1., idata[9], idata[10],
                                       molecule1.vibr[i], molecule1.vibr[i + i_delta],
                                       molecule2.vibr[k], molecule2.vibr[k - i_delta], i, k, i_delta, nokt)
    else:
        return raw_vv_integral_vss_fho(T, deg, idata[0], 1., idata[9], idata[10],
...
def vv_collisions_fho(T: float, T1_1: float, T1_2: float, idata: np.ndarray,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**,
    ...
    ve1_dimless = molecule1.vibr / (constants.k * T)
    ve2_dimless = molecule2.vibr / (constants.k * T)
    for k in range(molecule_num_vibr2):
        for i in range(molecule_num_vibr1):
            cs = vv_integral_fho(T, 0, idata, molecule1, molecule2, i, k + 1, 1, model, nokt=True)
            result[0] += ((ve1_dimless[i] -
                           ve1_dimless[i + 1]) ** 2) * vibr_exp1[i]\
                                                              * vibr_exp2[k + 1] * cs
            result[1] += ((ve2_dimless[k + 1] -
                           ve2_dimless[k]) ** 2) * vibr_exp1[i] \
                                                          * vibr_exp2[k + 1] * cs
            result[2] += ((ve1_dimless[i + 1] - ve1_dimless[i])
                          * (ve2_dimless[k] - ve2_dimless[k + 1])) * vibr_exp1[i]\
                                                                                     * vibr_exp2[k + 1]\
                                                                                     * cs
            result[3] += (ve1_dimless[i + 1] - ve1_dimless[i]) * vibr_exp1[i]\
                                                                                 * vibr_exp2[k + 1] * cs
            result[4] += (ve2_dimless[k] -
                          ve2_dimless[k + 1]) * vibr_exp1[i] \
                                                       * vibr_exp2[k + 1] * cs
            result[5] -= (ve1_dimless[i + 1] -
                          ve1_dimless[i]) * vibr_exp1[i] \
                                                   * vibr_exp2[k + 1] * cs
            result[6] -= (ve2_dimless[k] -
                          ve2_dimless[k + 1]) * vibr_exp1[i] \
                                                       * vibr_exp2[k + 1] * cs
            result[7] += (ve1_dimless[i + 1] -
                          ve1_dimless[i]) * vibr_exp1[i] \
                                                       * vibr_exp2[k + 1] * cs
            result[8] += (ve2_dimless[k] -
                          ve2_dimless[k + 1]) * vibr_exp1[i] \
                                                       * vibr_exp2[k + 1] * cs
            ...
            cs = vv_integral_fho(T, 0, idata, molecule1, molecule2, i + 1, k, -1, model, nokt=True)
            result[0] += ((ve1_dimless[i] - ve1_dimless[i + 1]) ** 2)\
                * vibr_exp1[i + 1] * vibr_exp2[k] * cs
            result[1] += ((ve2_dimless[k] - ve2_dimless[k + 1]) ** 2)\
                * vibr_exp1[i + 1] * vibr_exp2[k] * cs
            result[2] += ((ve1_dimless[i] - ve1_dimless[i + 1])
                          * (ve2_dimless[k + 1] -
                             ve2_dimless[k]))\
                          * vibr_exp1[i + 1] * vibr_exp2[k] * cs
            result[3] -= (ve1_dimless[i] -
                          ve1_dimless[i + 1]) * vibr_exp1[i + 1] \
                                                       * vibr_exp2[k] * cs
            result[4] -= (ve2_dimless[k + 1] -
                          ve2_dimless[k]) * vibr_exp1[i + 1] \
                                                   * vibr_exp2[k] * cs
            result[5] += (ve1_dimless[i] -
                          ve1_dimless[i + 1]) * vibr_exp1[i + 1] \
                                                       * vibr_exp2[k] * cs
            result[6] += (ve2_dimless[k + 1] -
                          ve2_dimless[k]) * vibr_exp1[i + 1] \
                                                   * vibr_exp2[k] * cs
            result[7] += (ve1_dimless[i] -
                          ve1_dimless[i + 1]) * vibr_exp1[i + 1] \
                                                       * vibr_exp2[k] * cs
            result[8] += (ve2_dimless[k + 1] -
                          ve2_dimless[k]) * vibr_exp1[i + 1] \
                                                   * vibr_exp2[k] * cs
...
def raw_crosssection_vt_gss_fho(g: float, T: float, mass: float, beta: float, gssc1: float, gssc2: float,
                                ...
                                ve_before: float, ve_after: float, i: int, delta: int) -> float:
    ...
    mass : float
        the collision-reduced mass :math:`m_{cd}`
    beta : float
        the Morse potential exponential :math:`\\beta` parameter
    gssc1 : float
        the elastic crosssection GSS potential :math:`C_{1}` parameter
    gssc2 : float
        the elastic crosssection GSS potential :math:`C_{2}` parameter
    ...
    float
        The VT crosssection
    """
    return prob.vt_probability_fho(g, T, mass, beta, ve_before, ve_after, i, delta)\
...
def raw_vt_integral_gss_fho(T: float, deg: int, mass: float, beta: float, gssc1: float, gssc2: float,
    ...
    if delta != 0:
        ...
        if ve_after <= ve_before:
            ...
            min_g = ((ve_after - ve_before) / (constants.k * T))**0.5
        ...
        return mult * (gssc1 * ((constants.k * T) ** gsso1) * integrate.quad(f1, min_g, np.inf)[0] +
                       gssc2 * ((constants.k * T) ** gsso2) * integrate.quad(f2, min_g, np.inf)[0])
    else:
        raise KineticlibError('VT transition with no change in vibrational level specified')


def raw_vt_integral_vss_fho(T: float, deg: int, mass: float, beta: float, vssc: float, vsso: float,
                            ve_before: float, ve_after: float, i: int,
    ...
    if delta != 0:
        ...
        return mult * vssc * (T ** (-vsso)) * integrate.quad(f, min_g, np.inf)[0]
    else:
        raise KineticlibError('VT transition with no change in vibrational level specified')
...
def raw_vt_integral_rigid_sphere_fho(T: float, deg: int, sigma: float, mass: float, beta: float,
    ...
    if delta != 0:
        ...
        return mult * (sigma ** 2) * integrate.quad(f, min_g, np.inf)[0]
    else:
        raise KineticlibError('VT transition with no change in vibrational level specified')
...
def raw_vv_integral_gss_fho(T: float, deg: int, mass: float, beta: float, gssc1: float, gssc2: float,
    ...
    **M1(i) + M2(k) -> M1(i + i_delta) + M2(k - i_delta)**, *raw* version
    ...
    if i_delta != 0:
        ...
        delta_energy = ve_before1 + ve_before2 - ve_after1 - ve_after2
        if delta_energy >= 0:
            min_g = 0.
        else:
            min_g = (-delta_energy)**0.5
