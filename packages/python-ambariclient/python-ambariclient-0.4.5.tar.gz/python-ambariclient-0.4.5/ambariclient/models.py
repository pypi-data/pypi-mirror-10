    ...
    def create_many(self, *args):
        ...
        if len(args) > 1:
            if self.client.version < (2,0,0):
                for arg in args:
                    added.append(self.create(**arg))
            else:
                response = self.client.post(self.url, args)
                # need to see what the response looks like here
                # add them to self._models and added
        else:
            added.append(self.create(**args[0]))
...
class ClusterHost(Host):
    ...
    def create(self, *args, **kwargs):
        if self.client.version < (2,0,0):
            ...
            return self.load(self.client.post(self.parent.url, kwargs))
