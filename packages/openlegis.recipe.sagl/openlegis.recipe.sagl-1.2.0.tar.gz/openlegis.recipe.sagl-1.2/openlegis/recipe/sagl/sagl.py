...
def main(app, parser):
    ...
    if db:
        ...
        if not created:
            ...
            sagl_sql = pkg_resources.resource_filename('openlegis.sagl', 'instalacao/db_schema.sql')
            ...
            sagl_data = pkg_resources.resource_filename('openlegis.sagl', 'instalacao/db_initial_data.sql')
            process = Popen("mysql %s -u%s -p%s -h %s" % (mysql_db, mysql_user, mysql_pass, mysql_host), stdout=PIPE, stdin=PIPE, shell=True)
            process.communicate('source ' + sagl_data)[0]
