...
def get_h_data(code, start=None, end=None, autype='qfq',
    ...
    if len(qs)>1:
        for d in range(1, len(qs)):
            qtd = qs[d]
            ct._write_console()
            df = _parse_fq_data(_get_index_url(index, code, qtd), index,
                                retry_count, pause)
    ...
    if index is False:
        tdqt = du.year_qua(end)
        if (tdqt[0] == qt[0]) & (tdqt[1] == qt[1]):
            rate = float(data.head(1)['factor'])
        else:
            rate = _get_lastest_ft(code, retry_count, pause)
    if index:
    ...
    if autype == 'hfq':
        ...
        if autype == 'qfq':
            ...
            frow = df.head(1)
#             preClose = float(get_realtime_quotes(code)['price'])
...
def _get_lastest_ft(code, retry_count, pause):
    qt = du.get_quarts('2010-01-01', du.today())
    for d in qt:
        df = _parse_fq_data(_get_index_url(False, code, d), False,
                                retry_count, pause)
        if len(df)>0:
            df = df.sort('date', ascending=False)
            return float(df.head(1)['factor'])
