...
def decode_named(input: io.BufferedIOBase, errors: str='strict'):
    ...
    .. note::
        ...
        :meth:`input.read` must perform any buffering that may be necessary to
        the underlying I/O; if it returns less data than was requested, that
        will be interpreted as EOF.  :class:`io.BufferedIOBase` and its
        subclasses satisfy this requirement when in non-interactive mode, but
        may raise :exc:`BlockingIOError` if in non-blocking mode.  If you want
        to use non-blocking I/O here, look into event-driven frameworks; many
        provide non-blocking file and socket implementations with buffer-like
        behavior.

    """
    logger.debug("Decoding named tag...")
    tag_id = ByteTag._decode_payload(input, errors)
    if tag_id == ids.TAG_End:  # TAG_End has no name and no payload
        ...
        return (None, result)
    name = StringTag._decode_payload(input, errors)
...
class TagMixin(metaclass=abc.ABCMeta):
    ...
    @abc.abstractmethod
    def _encode_payload(self, output: io.BufferedIOBase,
        ...
        Writes to output and returns bytes written; see :meth:`encode_named`
        for some caveats related to this.
    ...
    def encode_named(self, name: str, output: io.BufferedIOBase,
        ...
        """
        total_length = ByteTag(self.tag_id)._encode_payload(output)
        total_length += StringTag(name)._encode_payload(output, errors)
        total_length += self._encode_payload(output, errors)
    ...
    @classmethod
    @abc.abstractmethod
    def _decode_payload(cls, input: io.BufferedIOBase, errors: str='strict'):
        ...
        Reads from input; see :meth:`~TagMixin._decode_payload` for some
...
def _decode_payload_closure(formatstr, length):
    def _decode_payload(cls, input: io.BufferedIOBase,
                       errors: str='strict') -> int:
        ...
        return result
    _decode_payload.__doc__ = _decode_payload.__doc__.format(length)
    return _decode_payload


def _encode_payload_closure(formatstr):
    def _encode_payload(self, output: io.BufferedIOBase, errors='strict'):
...
class ByteTag(TagMixin, int):
    ...
    def __init__(self, value):
        ...
        try:
            ...
            raise OverflowError("{} is too far from zero to encode."
                                .format(self)) from exc
    ...
    _encode_payload = _encode_payload_closure(">b")
    ...
    _decode_payload = _decode_payload_closure(">b", 1)
    _decode_payload = classmethod(_decode_payload)
...
class ShortTag(TagMixin, int):
    ...
    _encode_payload = _encode_payload_closure(">h")
    ...
    _decode_payload = _decode_payload_closure(">h", 2)
    _decode_payload = classmethod(_decode_payload)
...
class IntTag(TagMixin, int):
    ...
    _encode_payload = _encode_payload_closure(">i")
    ...
    _decode_payload = _decode_payload_closure(">i", 4)
    _decode_payload = classmethod(_decode_payload)
...
class LongTag(TagMixin, int):
    ...
    _encode_payload = _encode_payload_closure(">q")
    ...
    _decode_payload = _decode_payload_closure(">q", 8)
    _decode_payload = classmethod(_decode_payload)
...
class FloatTag(TagMixin, float):
    ...
    _encode_payload = _encode_payload_closure(">f")
    ...
    _decode_payload = _decode_payload_closure(">f", 4)
    _decode_payload = classmethod(_decode_payload)
...
class DoubleTag(TagMixin, float):
    ...
    _encode_payload = _encode_payload_closure(">d")
    ...
    _decode_payload = _decode_payload_closure(">d", 8)
    _decode_payload = classmethod(_decode_payload)
...
class ByteArrayTag(TagMixin, bytes):
    ...
    def _encode_payload(self, output: io.BufferedIOBase,
        ...
        total_length = IntTag(len(self))._encode_payload(output, errors)
    ...
    def _decode_payload(cls, input: io.BufferedIOBase, errors: str='strict'):
        ...
        array_len = IntTag._decode_payload(input, errors)
...
class StringTag(TagMixin, str):
    ...
    def _encode_payload(self, output: io.BufferedIOBase,
        ...
        total_length = ShortTag(len(raw))._encode_payload(output, errors)
        total_length += len(raw)
        output.write(raw)
    ...
    def _decode_payload(cls, input: io.BufferedIOBase, errors: str='strict'):
        ...
        length = ShortTag._decode_payload(input, errors)
        raw = input.read(length)
        if len(raw) < length:
            raise exceptions.IncompleteSequenceError("Expected {} bytes,"
                                                     " got {}"
...
class ListTag(TagMixin, list):
    ...
    def _encode_payload(self, output: io.BufferedIOBase,
        ...
        total_length = self.content_id._encode_payload(output, errors)
        total_length += IntTag(len(self))._encode_payload(output, errors)
        for tag in self:
            if tag.tag_id != self.content_id:
                raise TypeError("{} has id {}, not {}."
                                .format(repr(tag), tag.tag_id,
                                        self.content_id))
            total_length += tag._encode_payload(output, errors)
    ...
    def _decode_payload(cls, input: io.BufferedIOBase, errors: str='strict'):
        ...
        content_id = ByteTag._decode_payload(input, errors)
        length = IntTag._decode_payload(input, errors)
...
class CompoundTag(TagMixin, dict):
    ...
    def _decode_payload(cls, input: io.BufferedIOBase, errors: str='strict'):
        ...
        result = cls()
        sentinel = EndTag()
        new_name, new_tag = decode_named(input, errors)
...
class IntArrayTag(TagMixin, list):
    ...
    def _encode_payload(self, output, errors='strict'):
        ...
        total_length = length._encode_payload(output, errors)
        for tag in cooked:
            total_length += tag._encode_payload(output, errors)
    ...
    def _decode_payload(cls, input: io.BufferedIOBase, errors: str='strict'):
        ...
        length = IntTag._decode_payload(input, errors)
        for _ in range(length):
            item = IntTag._decode_payload(input, errors)
            result.append(item)
...
def decode_payload(input, tag_id: int, errors: str='strict') -> TagMixin:
    ...
    :meth:`~TagMixin._decode_payload` method.
    ...
    return klass._decode_payload(input, errors)
