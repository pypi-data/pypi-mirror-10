...
class TupleMultiField(MutableField, MultiField):
    ...
    def from_python(self, values: (object, ...)) -> (tags.TagMixin, ...):
        ...
        return tuple(None if cooked_value is None else func(cooked_value)
                     for func, cooked_value in
...
class _sentinel:
    """Sentinel value for :class:`NBTObjectField`.

    When passed as the obj_class argument, refers to the class in which the
    field is declared.  If the field is not declared inside a subclass of
    NBTObject, undefined behavior occurs.

    """
    def __repr__(self):
        return '{}.{}'.format(__name__, 'SELF_REFERENCE')


SELF_REFERENCE = _sentinel()
...
class NBTObjectField(MutableField, SingleField):
    ...
    If the default value is :obj:`None`, a brand-new instance of the object
    ...
    def attach(self, owner: NBT_META, name: str):
        super().attach(owner, name)
        if self.obj_class is SELF_REFERENCE:
            self.obj_class = owner
