...
class Migration(DataMigration):
    """ The forwards method for this migration has been modified as a workaround for a known issue related
        to upgrading to Django 1.6x - using get_or_create fails with the following error:
    ...
    def forwards(self, orm):
        kwargs = {'model':'account', 'app_label':'assetcloud'}
        model = orm['contenttypes.ContentType']
        try:
            ct = model.objects.get(**kwargs)
        except model.DoesNotExist:
            ct = model(**kwargs)
            ct.save()

        kwargs = {'content_type':ct, 'codename':'edit_assets'}
        model = orm['auth.permission']
        try:
            obj = model.objects.get(**kwargs)
        except model.DoesNotExist:
            obj = model(name=u'Can add and change assets', **kwargs)
            obj.save()

    def backwards(self, orm):
        ct = orm['contenttypes.ContentType'].objects.get(
            model='account', app_label='assetcloud')
        perm = orm['auth.permission'].objects.get(
            content_type=ct, codename='edit_assets')
        perm.delete()
    ...
    models = {
        ...
        'assetcloud.asset': {
            ...
            'account': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'assets'", 'null': 'True', 'to': "orm['assetcloud.Account']"}),
    ...
    complete_apps = ['contenttypes', 'auth', 'assetcloud']
