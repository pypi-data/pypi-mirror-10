...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        User = orm['auth.User']
        UserProfile = orm['assetcloud.UserProfile']
        selenium = User.objects.get(username='selenium-admin', pk=2)
        profile = UserProfile.objects.get(user=selenium)
        now = datetime.datetime.now()
        profile.accepted_terms_and_conditions = now
        profile.accepted_aup = now
        profile.save()
