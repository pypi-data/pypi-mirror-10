...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        """
        Create Uploads based on added_by and added data in Assets.

        Split assets into uploads heuristically - group assets that were
        uploaded at a similar time into the same upload.
        """
        # Note: Remember to use orm['appname.ModelName'] rather than "from appname.models..."
        
        # Maximum difference in added time to consider consecutive assets
        # part of the same download
        max_difference = datetime.timedelta(minutes=10)
        
        current_upload = None
        for asset in orm['assetcloud.Asset'].objects.all().order_by('added'):
            if current_upload:
                need_new_upload = asset.added_by != current_upload.added_by or \
                    abs(asset.added - current_upload.added) > max_difference
            else:
                need_new_upload = True

            if need_new_upload:
                current_upload = orm['assetcloud.Upload'](added_by=asset.added_by)
                current_upload.save()
                # Need to set added after creation otherwise auto_now_add will cause it to be set to current date
                current_upload.added=asset.added
                current_upload.save()

            asset.upload = current_upload
            asset.save()


    def backwards(self, orm):
        for asset in orm['assetcloud.Asset'].objects.all().select_related('upload'):
            asset.added_by = asset.upload.added_by
            asset.upload = None
            asset.save()

        orm['assetcloud.Upload'].objects.all().delete()
    ...
    models = {
        ...
        'assetcloud.asset': {
            ...
            'upload': ('django.db.models.fields.related.ForeignKey', [], {'blank': 'True', 'related_name': "'assets'", 'null': 'True', 'to': "orm['assetcloud.Upload']"})
        },
