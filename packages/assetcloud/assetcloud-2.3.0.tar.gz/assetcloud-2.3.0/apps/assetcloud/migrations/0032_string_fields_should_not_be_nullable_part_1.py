...
class Migration(SchemaMigration):
    ...
    def forwards(self, orm):
        # We work around the following error (with PostgreSQL) by putting the
        # DML and DDL in seperate migrations, so they are run in seperate
        # transactions.
        # FATAL ERROR - The following SQL query failed: ALTER TABLE "assetcloud_account" ALTER COLUMN "header_background" TYPE varchar(7), ALTER COLUMN "header_background" SET NOT NULL, ALTER COLUMN "header_background" DROP DEFAULT;
        # The error was: cannot ALTER TABLE "assetcloud_account" because it has pending trigger events
        if not db.dry_run:
            orm['assetcloud.Account'].objects.filter(header_background=None).update(header_background='')
            orm['assetcloud.Account'].objects.filter(welcome_text=None).update(welcome_text='')
            orm['assetcloud.Account'].objects.filter(header_links_active=None).update(header_links_active='')
            orm['assetcloud.Account'].objects.filter(highlight_colour=None).update(highlight_colour='')
            orm['assetcloud.Account'].objects.filter(header_links=None).update(header_links='')
