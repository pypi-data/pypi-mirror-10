...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Storage is used with 2 different meanings in this method:
        # ... here it means 'Django file storage engine' ...
        asset_file_storage = get_asset_storage()

        # ... and here it means 'storage used in bytes'
        account_storages = collections.defaultdict(lambda: 0)
        for asset in orm['assetcloud.Asset'].objects.all():
            # patch file storage because it doesn't get set up properly in
            # the model classes in the South orm dict - without the next line
            # the attempt to get asset.file.size would fail because the local
            # filesystem would be consulted even when ASSET_FILE_STORAGE was
            # set to use S3BotoStorage
            asset.file.storage = asset_file_storage

            asset.file_size = asset.file.size
            asset.save()
            account_storages[asset.account] = account_storages[asset.account] + asset.file_size

        for (account, storage) in account_storages.iteritems():
    ...
    models = {
        ...
        'assetcloud.asset': {
            ...
            'file': ('assetcloud.models.QuotaValidatingFileField', [], {'max_length': '100', 'size_field_name': "'file_size'"}),
