...
class ThriftRemoteConnection(RemoteConnection):
    ...
    def execute(self, command, params):
        """
        Overrides execute() method by sending commands through the Thrift interface.
...
def unwrap_response_value(value, contained_values):
    """
    Unwraps response values received from the Thrift client.
    Response values sent by the Thrift server are packaged in the form of a Thrift ResponseValue.
    Complex response values containing Thrift containers are transformed into a flat tree on the server side. In order
    to preserve the integrity of the response values from server to client, the flattened trees are reconstructed here.
    """
    if value is None:
        return None
        ...
        for k, v in value.map_values.iteritems():
            contained_value = _get_contained_value(contained_values, v)
        ...
        for list_item in value.list_values:
            contained_value = _get_contained_value(contained_values, list_item)
...
def _get_contained_value(contained_values, id):
