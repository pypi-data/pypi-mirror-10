...
class StaticAgent(murano_object.MuranoObject):
    def initialize(self, _context, env, input_queue, response_queue):
        self._enabled = False
        self._responce_queue = response_queue
        ...
        self._environment = env #self._get_environment(_context)
        self._enabled = True
        if input_queue != None:
            self._queue = input_queue
        else:
            self._queue = str('e%s-h%s' % (
               self._environment.object_id, '123')).lower()
    ...
    def _prepareMessage(self, template, msg_id):
    ...
    def _send(self, template, wait_results, timeout, _context):
        ...
        msg = self._prepareMessage(template, msg_id)
        with common.create_rmq_client() as client:
