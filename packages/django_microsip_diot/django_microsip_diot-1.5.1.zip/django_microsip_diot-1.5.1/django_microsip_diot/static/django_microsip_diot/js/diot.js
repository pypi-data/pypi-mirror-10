...
function get_totales(){
	...
	var $row = $(this);
	total_importes += parseFloat($row.find("#importe").text().replace("$ ",""));
	total_ivas += parseFloat($row.find("#iva").text().replace("$ ",""));
	});
	$("#id_total_importes").text("$ "+total_importes.toFixed(2));
	$("#id_total_ivas").text("$ "+total_ivas.toFixed(2));
}
get_totales();
...
$("#crear_txt").on("click",function(){
	...
	checked_repos.each(function(){
		...
		var importe = parseFloat($row.find("#importe").text().replace("$ ",""));
		var iva = parseFloat($row.find("#iva").text().replace("$ ",""));
		...
		var pagado = parseFloat($row.find("#pagado").text().replace("$ ",""));
		...
		if ($row.attr("class") != "diot_xml")
		{
			$.ajax({
				...
				data:{
					...
					'iva_pagado':iva,
					'iva':parseFloat($row.find("#iva").text().replace("$ ","")),
...
function calc_exento(btn){
	...
	var importe = parseFloat($tr.find("#importe").text().replace("$ ",""));
	var iva = parseFloat($tr.find("#iva").text().replace("$ ",""));
	var iva_retenido = parseFloat($tr.find("#iva_retenido").val());
	var descuento = parseFloat($tr.find("#descuento").val());
	var pagado = parseFloat($tr.find("#pagado").text().replace("$ ",""));
...
$("#modificar_xml").on("click",function(){
	...
	var importe = parseFloat($id_xml.parent().parent().find("#importe").text().replace("$ ",""));
	var pagado_final;
...
$("#export_excel").on("click",function(){
	...
	$("#tabla_repositorios").find("tbody tr:visible").each(function(){
		...
		var pagado = $(this).find("#pagado").text().replace("$ ","").trim();
		var importe = $(this).find("#importe").text().replace("$ ","").trim();
		var iva = $(this).find("#iva").text().replace("$ ","").trim();
...
// ELIMINAR LOS REPOS VARIOS A LA VEZ
