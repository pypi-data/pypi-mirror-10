...
class Sobi(object):
    def __init__(self):
        ...
        self.hubs_url = 'https://app.socialbicycles.com/api/hubs/%s.json'
        self.bikes_url = 'https://app.socialbicycles.com/api/bikes/%s.json'
    ...
    def get_data(self, page=1):
        ...
        response = requests.get(this_routes_url, auth=self.auth)
    ...
    def lookup_bike_name(self, bike_id):
        ...
        if not bike_match:
            this_bike_url = self.bikes_url % (bike_id)
            response = requests.get(this_bike_url, auth=self.auth)
            if response.status_code == 200:
    ...
    def lookup_hub_address(self, hub_id):
        ...
        if not hub_match:
            this_hub_url = self.hubs_url % (hub_id)
            response = requests.get(this_hub_url, auth=self.auth)
            if response.status_code == 200:
...
sobi = Sobi()
sobi.username = 'ryan@quandyfactory.com'
sobi.password = 'ThePasswordShouldBeStrong'
sobi.path = '/home/ryan/projects/sobi/data'
