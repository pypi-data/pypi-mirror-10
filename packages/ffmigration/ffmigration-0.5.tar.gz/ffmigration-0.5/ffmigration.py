...
class DatabaseDriver(object):
    ...
    def __init__(self, options, schema):
        self.connection = self.connect_db(options, schema)
    ...
    def connect_db(self, options, schema):
...
class PgSqlDriver(DatabaseDriver):
    ...
    def connect_db(self, options, schema):
        ...
        conn = ["%s=%s" % (k, v) for k,v in conn_params.items()]

        try:

            return psycopg2.connect(" ".join(conn),
...
def main(argv):
    ...
    parser.add_option("-d", "--driver",
                    action="store", dest="driver", default="mysql",
                    metavar="DRIVER", choices=DRIVERS.keys(),
                    help="Database driver to use. choices : %s" % ", ".join(DRIVERS.keys()))
