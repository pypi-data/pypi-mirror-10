...
from .dig_craters import impactor


class ImpactorComponent(Component):
    _name = 'Impactor'
    ...
    def __init__(self, grid, min_radius=5e-3, min_ejecta_thickness=.1e-6):
        with tempfile.NamedTemporaryFile('w+') as fp:
            fp.write('min_radius:\n%e\n' % min_radius)
            fp.write('min_ejecta_thickness:\n%e\n' % min_ejecta_thickness)
            fp.write('record_impacts:\n%d\n' % 1)
            fp.seek(0)

            self._impactor = impactor(grid, fp.name)
        self._grid = grid

    def update(self):
        self._impactor.excavate_a_crater_furbish(self._grid)
