...
class USBDeviceHandle(object):
    ...
    def getBOSDescriptor(self):
        return BOSDescriptor(self.__handle)
...
class USBEndpoint(object):
    ...
    def getSuperspeedEndpointCompanion(self):
        return self.__context._getSuperspeedEndpointCompanion(self.__endpoint)

class SuperspeedEndpointCompanion(object):
    __companion = None
    __libusb_free_ss_endpoint_companion_descriptor = \
        libusb1.libusb_free_ss_endpoint_companion_descriptor
    def __init__(self, context, endpoint):
        if not isinstance(endpoint, libusb1.libusb_endpoint_descriptor):
            raise TypeError('Unexpected descriptor type.')
        companion = libusb1.libusb_ss_endpoint_companion_descriptor_p()
        result = libusb1.libusb_get_ss_endpoint_companion_descriptor(context,
            endpoint, byref(companion))
        if result:
            raise libusb1.USBError(result)
        self.__companion = companion

    def __del__(self):
        companion = self.__companion
        if companion is not None:
            self.__companion = None
            self.__libusb_free_ss_endpoint_companion_descriptor(companion)

    def getMaxBurst(self):
        return self.__companion.contents.bMaxBurst

    def getAttributes(self):
        return self.__companion.contents.bmAttributes

    def getBytesPerInterval(self):
        return self.__companion.contents.wBytesPerInterval

class BOSDescriptor(object):
    __bos = None
    __libusb_free_bos_descriptor = libusb1.libusb_free_bos_descriptor
    def __init__(self, handle):
        bos = libusb1.libusb_bos_descriptor_p()
        result = libusb1.libusb_get_bos_descriptor(handle, byref(bos))
        if result:
            raise libusb1.USBError(result)
        self.__bos = bos
    ...
    def __del__(self):
        ...
        if bos is not None:
            self.__bos = None
            self.__libusb_free_bos_descriptor(bos)

    def getNumDeviceCaps(self):
        return self.__bos.contents.bNumDeviceCaps

    __len__ = getNumDeviceCaps

    def __iter__(self):
        dev_capability = self.__bos.contents.dev_capability
        for capability_num in xrange(self.getNumDeviceCaps()):
            yield getBOSCapabilityClass(
                dev_capability[capability_num].contents)

    def __getitem__(self, index):
        if not isinstance(index, int):
            raise TypeError('interface parameter must be an integer')
        if not (0 <= index < self.getNumDeviceCaps()):
            raise IndexError('No such capability: %r' % (index, ))
        return getBOSCapabilityClass(
            self.__bos.contents.dev_capability[index].contents)

def getBOSCapabilityClass():
    pass
    # XXX: keep strong ref on BOSDescriptor
...
class USBContext(object):
    ...
    @_validContext
    def _getSuperspeedEndpointCompanion(self, endpoint):
        return SuperspeedEndpointCompanion(self.__context_p, endpoint)
