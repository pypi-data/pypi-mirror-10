...
class TemplateContent(object):
    ...
    def as_string(self):
        ...
        """
        return self.environment.as_string(self.template, self.template_context)

    def as_stream(self):
        """
        Return the rendered template as a stream
        """
        return self.environment.as_stream(self.template, self.template_context)
...
class BaseTemplateEnvironment(object):
    """\
    An abstract template rendering environment, not tied to any specific
    templating system
    """
    #: Registry of template environments
    __registry__ = {}
    ...
    @classmethod
    def of(cls, app):
        return cls.__registry__.get((cls, app))
    ...
    def init_app(self, app):
        self.__registry__[(self.__class__, app)] = self
    ...
    def as_string(self, template=None, data=None, _in_decorator=False):
        ...
        ns = self.get_context()
        ns.update(data)
        return self._render(template, ns, stream=False)
    ...
    def as_stream(self, template=None, data=None, _in_decorator=False):
        ...
        ns = self.get_context()
        ns.update(data)
        return self._render(template, ns, stream=True)
    ...
    def as_response(self, template=None, data=None, _in_decorator=False,
        ...
        return Response(TemplateContent(self, template, data, None,
                                        stream=_stream),
                        **response_kwargs)
...
def get_template_environment(app=None, cls=BaseTemplateEnvironment):
    app = app or context.app
    return cls.of(app)


class RenderWithApp(object):

    def __init__(self, cls, app=None):
        self.cls = cls
        self.app = app

    def __call__(self, *args, **kwargs):
        return get_template_environment(self.app, self.cls)(*args, **kwargs)

    def __getattr__(self, attr):
        return getattr(get_template_environment(self.app, self.cls), attr)
