...
class VirtualBox(Machinery):
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.vnc = kwargs.pop('vnc')
        self.vnc_port = kwargs.pop('vnc_port')
        self.vnc_pass = kwargs.pop('vnc_pass')
    ...
    def start_vm(self, visible=False, vnc=False):
        type_ = 'gui' if visible else 'headless'
        if not self.vnc:
            return self._call('startvm', self.name, type_=type_)
        else:
            return self._call('startvm', self.name, type_=type_, vnc=True,
                              vncport=self.vnc_port, vncpass=self.vnc_pass)
