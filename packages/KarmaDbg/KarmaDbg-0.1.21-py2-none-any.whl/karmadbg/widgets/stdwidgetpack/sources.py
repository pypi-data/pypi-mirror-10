...
class SourceManager(QObject):
    ...
    def onTargetStopped(self):
        ...
        fileName, line = yield ( self.uimanager.debugClient.callServerAsync( getSourceLine ) )
        if fileName == "":
            return

        if not os.path.exists(fileName):
            fileName = yield( self.uimanager.debugClient.callServerAsync( getSourceFromServer ) )
            if fileName == "":
                return
...
def getSourceFromServer():
    import pykd
    try:
        ip = pykd.getFrame().ip
        return  pykd.getSourceFileFromSrcSrv(ip)
    except pykd.DbgException:
        return ""
