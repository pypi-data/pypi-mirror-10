...
class CmdConsoleWidget(QDockWidget):
    ...
    def onInputComplete(self, inputStr):
        self.uimanager.debugClient.inputComplete(inputStr)

    @async
    def onAutoComplete(self, startAutoComplete):

        self.console.stopInput()
        autoComplete = yield( self.uimanager.debugClient.getAutoCompleteAsync(startAutoComplete))

        if len(autoComplete) == 0:
            self.console.requireInput()
        if len(autoComplete): 
            self.console.setEditLine( self.console.getEditLine() + autoComplete[0][0])
            self.console.requireInput()
        elif len(autoComplete) > 1:
            self.console.writeToLog(startAutoComplete+"\n")
            self.console.writeToLog("   ".join( [ac[1] for ac in autoComplete] ) )
            self.uimanager.debugClient.inputComplete("")
