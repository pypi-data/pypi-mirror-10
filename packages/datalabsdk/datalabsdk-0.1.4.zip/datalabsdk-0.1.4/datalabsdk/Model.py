...
class ClassificationModel(Model):
    ...
    def get_performance_metric(self,input_path):
        ...
        return {
            'TPR' : float(TPN)+1 / PN+1,
            'SPC' : float(TNN)+1 / NN+1,
            'PPV' : float(TPN)+1 / (TPN+FPN)+1,
            'NPV' : float(TNN)+1 /(TNN+FNN)+1,
            'FPR' : float(FPN)+1  / NN+1,
            'FNR' : float(FNN)+1 / (TPN+FNN)+1,
            'FDR' : float(FPN)+1 / (TPN+FPN)+1,
            'ACC' : (TPN+TNN)+1 / float(N)+1,
            'F1': (2.0*TPN)+1 / ((2.0*TPN)+FPN+FNN)+1,
        }
