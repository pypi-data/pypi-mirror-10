...
from geminisqlite.config import read_gemini_config
import time
from geminisqlite import gemini_annotate
from random import randint


class GeminiLoader(object):
    """
    Object for creating and populating a geminisqlite
    database and auxillary data files.
    """
    def __init__(self, args, buffer_size=100):
        self.args = args

        # create the geminisqlite database
        self._create_db()
    ...
    def store_resources(self):
        """Create table of annotation resources used in this geminisqlite database.
        """
        database.insert_resources(self.c, annotations.get_resources( self.args ))

    def store_version(self):
        """Create table documenting which geminisqlite version was used for this db.
        """
    ...
    def populate_from_vcf(self):
        ...
        vars_inserted = 0
        self.skipped = 0
        log_file = open("%s%s" % (self.args.log_loc, str(os.getpid())), "w")
        ...
        log_file.write("%s;%.2f\n" % (len(self.var_buffer), time.time() - starttime))  
        log_file.close()
    ...
    def _get_anno_version(self):
        ...
        if self.args.anno_type == "snpEff":
            try:
                ...
                "http://geminisqlite.readthedocs.org/en/latest/content/functional_annotation.html#stepwise-installation-and-usage-of-snpeff\n")
                sys.exit(error)
    ...
    def _get_vep_csq(self, reader):
        ...
        error = "\nERROR: Check geminisqlite docs for the recommended VCF annotation with VEP"\
                "\nhttp://geminisqlite.readthedocs.org/en/latest/content/functional_annotation.html#stepwise-installation-and-usage-of-vep"
        sys.exit(error)

    def _create_db(self):
        """
        private method to open a new DB
        and create the geminisqlite schema.
        """
        ...
        # create the geminisqlite database tables for the new DB
    ...
    def _prepare_variation(self, var):
        ...
        ############################################################
        # collect annotations from geminisqlite's custom annotation files
    ...
    def _prepare_samples(self):
        ...
        for sample in self.samples:
            ...
            if sample in self.ped_hash:
                ...
                sample_list += [None, None, str(randint(1,2)), str(randint(1,2))]
...
def load(parser, args):
    ...
    # create a new geminisqlite loader and populate
    # the geminisqlite db and files from the VCF
    ...
    loading_time = mid - starttime
    indexing_time = ind_time - mid
    total_time = end_time - starttime
    
    log = open('/home/bgossele/gemsq_logs/%s' % os.getpid(), 'a')
    log.write("%s;%s;%s\n" % (loading_time, indexing_time, total_time))
