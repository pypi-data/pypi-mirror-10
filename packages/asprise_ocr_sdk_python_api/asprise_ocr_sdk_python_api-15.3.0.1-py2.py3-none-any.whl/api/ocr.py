...
class Ocr(object):
    ...
    def recognize(self, img_files, page_index, start_x, start_y, width, height, recognize_type, output_format, **kwargs):
        ...
        global OCR_OUTPUT_FORMAT_PDF, OCR_OUTPUT_FORMAT_PLAINTEXT, OCR_OUTPUT_FORMAT_XML, OCR_PAGES_ALL
        global OCR_RECOGNIZE_TYPE_ALL, OCR_RECOGNIZE_TYPE_BARCODE, OCR_RECOGNIZE_TYPE_BARCODE
        global PROP_PDF_OUTPUT_FILE, PROP_OUTPUT_SEPARATE_WORDS
        ...
        if recognize_type not in [OCR_RECOGNIZE_TYPE_ALL, OCR_RECOGNIZE_TYPE_BARCODE, OCR_RECOGNIZE_TYPE_TEXT]:
            raise ValueError("Invalid recognize type: " + recognize_type)
        ...
        properties_string = _CONFIG_PROP_SEPARATOR.join(['%s%s%s' % (
            key, _CONFIG_PROP_KEY_VALUE_SEPARATOR,
            str(value).lower() if value is True or value is False else value) for (key, value) in kwargs.items()])
    ...
    @staticmethod
    def is_windows():
        return sys.platform.lower().find("win") >= 0
    ...
    @staticmethod
    def _is_debug():
        return 'AD' in os.environ.keys() and os.environ['AD']
