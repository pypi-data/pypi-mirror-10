...
class SimpleInstrument(Instrument):
    ...
    def get_projection_operator(self, sampling, scene, verbose=True):
        ...
        nu = self.filter.nu
        return self._get_projection_operator_from_rotation(rotation, scene, nu,
                                                           verbose=verbose)

    def _get_projection_operator_from_rotation(self, rotation, scene, nu=None,
                                               verbose=True):
        if nu is None:
            nu = self.filter.nu
        ...
        theta, phi, vals = self._peak_angles(scene, nu)
    ...
    def _peak_angles(self, scene, nu):
        """
...
class QubicInstrument(SimpleInstrument):
    ...
    def _peak_angles(self, scene, nu):
        ...
        theta, phi = self._peak_angles_kmax(scene, self.synthbeam.kmax, nu)
    ...
    def _peak_angles_kmax(self, scene, kmax, nu):
        """
        Return the spherical coordinates (theta, phi) of the beam peaks,
        in radians.
    ...
    def get_response_A(self, x, y, area, nu):
        """
    ...
    def get_response_B(self, theta, phi, spectral_irradiance, nu, bandwidth):
        """
        ...
        nu : float
            The frequency for which the response is computed [Hz].
        bandwidth : float
            The filter bandwidth for which the response is computed [Hz].
        ...
        source_E = np.sqrt(spectral_irradiance * bandwidth *
                           self.primary_beam(theta, phi) *
    ...
    def get_response(self, theta, phi, spectral_irradiance=1, x=None, y=None,
                     area=1, nu=None, bandwidth=None):
        """
        Return the complex electric field [W^(1/2)] on specified locations
        of the focal plane created by sources of specified spectral irradiance
        [W/m^2/Hz].
        ...
        if bandwidth is None:
            bandwidth = 1 if self.filter.bandwidth == 0 \
                          else self.filter.bandwidth
        A = self.get_response_A(x, y, area, nu)
        B = self.get_response_B(theta, phi, spectral_irradiance, nu, bandwidth)
        E = np.dot(A, B.reshape((B.shape[0], -1))).reshape(
            A.shape[:-1] + B.shape[1:])
        return E

    def _get_synthbeam(self, scene, x, y, area, nu, bandwidth, theta_max=45):
        """
        Return the monochromatic synthetic beam for a specified location
        on the focal plane, integrated over a given area and bandwidth.
        ...
        >>> sb = inst._get_synthbeam(scene, xpos, ypos, 1, nu, bandwidth)

        The power per unit surface collected on the point (xpos, ypos) of
        the focal plane, given a sky in W/m²/Hz is:
        ...
        for s in split(npix, ngroup):
            ...
            sb = self.get_response(theta[index_], phi[index_], x=x, y=y,
                                   area=area, nu=nu, bandwidth=bandwidth)
            out[..., index_] = abs2(sb, dtype=self.synthbeam.dtype)
        return out

    def get_synthbeam(self, scene, idetector, theta_max=45):
        """
        Return the monochromatic synthetic beam for a specified detector.
        ...
        scene : QubicScene
            The scene.
        idetector : integer
            Detector number for which the synthetic beam is calculated.
        theta_max : float, optional
            The maximum zenithal angle above which the synthetic beam is
            assumed to be zero, in degrees.

        """
        x = self.detector.center[idetector, 0]
        y = self.detector.center[idetector, 1]
        a = self.detector.area
        nu = self.filter.nu
        bandwidth = self.filter.bandwidth
        return self._get_synthbeam(scene, x, y, a, nu, bandwidth, theta_max)
