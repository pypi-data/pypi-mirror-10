...
from base import _getTitle, _evaluate
...
class Scene:
	...
	def _createWindow (self):
		...
		self.window = glutCreateWindow (_getTitle (self.name))
		...
		glLight (GL_LIGHT3, GL_DIFFUSE, (1, 1, 0.5))
		glEnable (GL_LIGHT3)
	...
	def _display (self):
		...
		glPushMatrix ()
		self.display (self._graphics.world)
		glPopMatrix ()
...
class _Thing:
	...
	def __init__ (self, size = 1, axis = (0, 0, 1), angle = 0, center = (0, 0, 0), joint = (0, 0, 0), pivot = (0, 0, 1), color = (1, 1, 1)):
		self.size = size
		self.axis = axis
		self.angle = angle
		self.center = center
	...
	def __call__ (self, angle, parts = lambda: None):
		glPushMatrix ()
		glColor (*self.color)
		glTranslate (* (tAdd (self.center, self.joint)))  # Put joint at correct position
		glRotate (_evaluate (angle), *self.pivot)  # Rotate over angle around self.pivot to move
