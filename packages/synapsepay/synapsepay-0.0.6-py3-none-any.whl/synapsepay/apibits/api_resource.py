class APIResource(object):
    ...
    def api_attribute_names(cls):
        return list(cls._api_attributes.keys())
    ...
    def determine_api_attribute_value(self, name, raw_value):
        if isinstance(self.__class__._api_attributes[name], dict) and "constructor" in list(self.__class__._api_attributes[name].keys()):
    ...
    def api_subclass(cls, name):
        if name in list(cls.subclass_dict.keys()):
