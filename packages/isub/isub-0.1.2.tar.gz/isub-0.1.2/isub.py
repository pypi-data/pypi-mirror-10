...
class JobObject(object):
    ...
    def write_script(self, script_path, log_path, redirecting=False):
        ...
        f.write(PBS_FORMAT % (self.opt.name,
                              log_path,
                              self.opt.resource_list))
        if not self.opt.mail_options is None:
            f.write('#PBS -m ' + self.opt.mail_options + '\n')
        if not self.opt.user_list is None:
...
class Interface(object):
    ...
    def parse_input(self):
        ...
        if len(args) == 0:
            self.exit_on_error(
                    'No argument\n' + 
                    'Type `%s help` to see what you can do with this' %
                    sys.argv[0])
    ...
    def parse_job(self, args):
        ...
        argparser.add_argument('-l', '--resource_list', default='nodes=1:taqy')
