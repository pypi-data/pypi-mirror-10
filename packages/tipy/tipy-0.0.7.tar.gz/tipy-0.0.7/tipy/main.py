...
from os import path, environ, abspath
...
def main():
    ...
    if mode == 'cmdl':
        ...
        pr = Profile()
        pr.enable()
        runctx('run_profile(profileString, profileLoops)', {'profileString': profileString, 'run_profile': run_profile, 'profileLoops': profileLoops}, locals())
        pr.disable()
        s = StringIO()
