...
class FileFinder():
    def __init__(self, data_path, logger=None, audit=None):
        ...
        self._audit = audit or app.audit.NullAudit()
    ...
    def find(self, path, optional=False):
        if len(path) == 0:
        ...
        self._logger.info("The loader is finding %s.", path)

        result = self._find(path, optional)
    ...
    def _find(self, path, optional=False):
        ...
        return self._extract(parts_taken, remaining_parts, json, optional)
    ...
    def _extract(self, path, parts, obj, optional, skip_restricted=False):
        self._logger.debug("Processing %s.", parts)
        ...
        if len(parts) == 1:
            ...
            return self._extract(path + parts, [], current, optional)

        return self._extract(path + [name], parts[1:], current, optional)
