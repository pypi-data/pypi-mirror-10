import app
import json
from app.exceptions import AuthenticationException, FormatterException
...
class TestTrust(LoggedTests):
    ...
    def test_no_credentials(self):
        ...
        mock = FinderMock()
        result = app.Trust(mock).process(
            "/", "complete", app.Credentials.get_empty()
        )
    ...
    def _process_list_using_response_type(self, response_type):
        ...
        return app.Trust(FinderStub()).process(
            "/", response_type, app.Credentials.get_empty()
        )
