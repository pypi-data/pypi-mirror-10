...
from portfolio.component import (mailer, storage)
...
def post_admin(model, **kwargs):
    ...
    def wrapper(view):
        template_dir = kwargs["template_dir"] if "template_dir" \
                                                 in kwargs else "Portfolio/PostAdmin"
        template_page = template_dir + "/%s.html"
        PostModel = model.PostModel

        Portfolio.set_context__(COMPONENT_POST_ADMIN=True)
        ...
        class PostAdmin(object):
            ...
            def categories(self):
                self.set_meta__(title="Post Categories")
            ...
            def types(self):
                self.set_meta__(title="Post Types")
                if request.method == "POST":
                    try:
                        id = request.form.get("id", None)
                        action = request.form.get("action")
            ...
            def tags(self):
                self.set_meta__(title="Post Tags")
                if request.method == "POST":
                    id = request.form.get("id", None)
                    action = request.form.get("action")
                    name = request.form.get("name")
                    ...
                    try:
                        if not id:
                            ...
                            self.flash_success__("New Tag '%s' added" % name)
                        else:
                            post_tag = PostModel.Tag.get(id)
                            if post_tag:
                                if action == "delete":
                                    post_tag.delete()
                                    self.flash_success__("Tag '%s' deleted successfully!" % post_tag.name)
                                else:
                                    post_tag.update(name=name, slug=slug)
                                    self.flash_success__("Tag '%s' updated successfully!" % post_tag.name)
                    except Exception as ex:
                        if ajax:
                            return jsonify({
                                "error": True,
                    ...
                    if id:
                        ...
                        if image:
                            if action == "delete":
                                ...
                                if obj:
                                    obj.delete()
                                self.flash_success__("Image deleted successfully!")
                            else:
                                image.update(description=description)
                                self.flash_success__("Image updated successfully!")
                    else:
                    ...
                    if request.files.get("file"):
                        ...
                        if obj:
                            ...
                            if ajax:
                                ...
                                self.flash_success__("Image '%s' uploaded successfully!" % upload_object.name)

                    else:
                        self.flash_error__("Error: Upload object file is invalid or doesn't exist")
                except Exception as e:
                    self.flash_error__("Error: %s" % e.message)
                return redirect(url_for("PostAdmin:images"))
