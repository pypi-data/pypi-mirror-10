...
class WebDriverTestCase(TestCase):
    ...
    def setUp(self):
        ...
        self.driver = WebDriverFactory.get(self.Log, self._testMethodName)

    def tearDown(self):
        """
        The teardown hook.  This will be called after each individual test.
        """
        self.Log.info("Test Runner: Attempting to teardown.")
        WebDriverFinalizer.finalize(self.driver, self.has_failed(), self.Log)
    ...
    def run(self, result=None):
        ...
        try:
            ...
            # self.Log = test_log(self._testMethodName)
            self.Log = Logger(self._testMethodName)

            self.Log.info("Test Runner: Running test setup: {0}".format(str(self)))
            self.setUp()

            try:
                self.Log.info("Test Runner: Running test: {0}".format(str(self)))
                test_method()

            except:
                self.Log.info("Test Runner: Adding failure to result for {0}".format(str(self)))
                result.addFailure(self, sys.exc_info())
                self.Log.info("Test Runner: Recording failure info for teardown for {0}".format(str(self)))
            ...
            try:
                self.Log.info("Test Runner: Tearing down test: {0}".format(str(self)))
                self.tearDown()
            ...
            self.Log.error(
                "OMG! See - https://external.skyscanner.co.uk/jira/browse/TAS-296 {} {}".format(type(e), str(e)))
            self.Log.error(traceback.format_exc())
        finally:
