...
class WebDriverFinalizer:
    ...
    @staticmethod
    def finalize(driver, test_failed, logger):
        """
        ...
        if desired_execution_environment == 'grid':
            ...
            WebDriverFinalizer.finalise_skygrid_driver(driver, test_failed, logger)
            return

        elif desired_execution_environment in ['saucelabs', 'sauce']:
            WebDriverFinalizer.finalise_saucelabs_driver(driver, test_failed, logger)
            return
    ...
    @staticmethod
    def finalise_saucelabs_driver(driver, test_failed, logger):
        logger.info("Finalizing driver for Saucelabs")
        try:
            job_id = driver.session_id
            logger.debug("Quitting driver instance.")
            driver.quit()
            ...
            WebDriverFinalizer.wait_until_sauce_job_completes(client, job_id, logger)

            logger.debug("Setting job public.")
            client.set_job_public(job_id, True)

            if test_failed:
                logger.debug("Setting job pass status to False.")
                ...
                logger.debug("Setting job error text.")
                client.set_error(job_id, error)
                logger.info('SauceLabs report: https://saucelabs.com/jobs/' + str(job_id))

            else:
                logger.debug("Setting job pass status to True.")
    ...
    @staticmethod
    def finalise_skygrid_driver(driver, test_failed, logger):
