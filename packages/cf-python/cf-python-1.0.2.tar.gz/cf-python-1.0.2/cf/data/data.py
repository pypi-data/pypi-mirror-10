...
#    def chunk22(self, shape, chunksize=None):
#        '''
#
#Partition the data array
#
...
#        d = {}
#
#        for partition_o, partition_s in izip(other.partitions.matrix.flat,
#                                             self.partitions.matrix.flat):
#
...
#        for partition in self.partitions.matrix.flat:
#            
#            n_chunks = int(broadcast_size*factor + 0.5)
#            
#            if n_chunks <= 1:
#                continue
#            
...
#        print  repr(data0), repr(other) # dch
...
#            data.varray
        ...
        # Whether or not at least two, but not all, axes are to be
        # collapsed.
...
'''
    ...
    def mask(self, value):
        raise AttributeError(
            "Can't set the 'mask' attribute directly, use the 'setmask' method")
    #--- End: def
    @mask.deleter
    def mask(self):
        raise AttributeError(
"Can't delete the 'mask' attribute directly, use the 'setmask()' method")
    #--- End: def
...
'''
        ...
        old = [data_axes.index(axis) for axis in cyclic_axes]
...
'''
        if not self.Units.isreftime:
            raise ValueError(
                "Can't override the calender of non-reference-time units: %r" %
                self.Units)
...
#        d = self
    ...
    def setitem(self, *args, **kwargs):
        raise NotImplementedError(
"cf.Data.setitem is dead. Use cf.Data.__setitem__ or cf.Data.where instead.")

    def setmask(self, *args, **kwargs):
        raise NotImplementedError(
"cf.Data.setmask is dead. Use cf.Data.__setitem__ or cf.Data.where instead.")
...
Return a dictionary of Data objects, which are the m dimensional sections of
this n dimensional Data object, where m <= n. The keys of the dictionary are
the indicies of the sections in the original Data object. The m dimensions that
are not sliced are marked with None as a placeholder making it possible to
reconstruct the original data object. The corresponding values are the
resulting sections of type cf.Data.

:Parameters:

    axes : *optional*
        This is should be a tuple or a list of the m indices of the m axes that
        define the sections of the Data object. If axes is None (the default)
        all axes are selected.

    stop : int, optional
        Stop after this number of sections and return. If stop is None all
        sections are taken.
