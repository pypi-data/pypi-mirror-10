...
(or URLs if DAP access is enabled) and Met Office (UK) PP format files
from disk and return their contents as a :ref:`field list
<fs_field_list>`, i.e. an ordered collection of fields stored in a
`cf.FieldList` object:
...
The `cf.read` function always returns a field list as opposed to a
field. If a single field as a `cf.Field` object is required, then the
returned field list may be indexed:
