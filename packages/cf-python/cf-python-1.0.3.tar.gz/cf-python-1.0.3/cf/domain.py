...
'''
        role = ('d', 'a')
        for pointer in ref.coords.copy():
            key = self.item(pointer, role=role, exact=True, key=True)
            if key is not None:
                ref.change_coord(pointer, key)            
    #--- End: def
...
(self.__class__.__name__, identity))
        ...
        for ref0 in self.r.itervalues():
            ...
            if not found_match:
                if traceback:
                    print(
                        "%s: Missing coordinate reference: %r" %
...
'''        
        ...
        dim = self.item(role='d', axes=axis, ndim=1, strict_axes=True)
        if dim is not None:
            # Get the name from the dimension coordinate
            return dim.name('domain%%%s' % axis) 

        aux = self.item(role='a', axes=axis, ndim=1, strict_axes=True)
        if aux is not None:
       ...
       #axis = self.axis(arg, role=('d', 'a', 'm'))
       #if axis is None:
       #    raise ValueError("No axis is identified by %r" % arg)    
       #
       #if axis in self.d:
       #    # Get the name from the dimension coordinate
       ...
       #aux = self.item(role='a', axes=axis, ndim=1)
       #if aux is not None:
...
'''
    ...
    def items(self, items=None, role=None, axes=None, ndim=None, exact=False,
              inverse=False, match_and=True, strict_axes=False):
        '''
...
#                            x = domain_axes.get(key, ())
#                            if ndim == len(x) and axes.intersection(x):
#                                axes_out[key] = value
##                    elif subset_axes:
##                        # --------------------------------------------
...
##                        for key, value in pool.iteritems():
##                            x = domain_axes.get(key, ())
##                            if ndim == len(x) and axes.issuperset(x):
##                                axes_out[key] = value
#                    else:
#                        # --------------------------------------------
#                        # Select items which span exactly the given
...
#                else:
                ...
                if not strict_axes:
                    ...
                    # --------------------------------------------
                    for key, value in pool.iteritems():
                        if axes.intersection(domain_axes.get(key, ())):
                            axes_out[key] = value
#                    elif subset_axes:
#                        # --------------------------------------------
 #                       # Select items which span exactly the given
 ...
 #                       # --------------------------------------------
 #                       for key, value in pool.iteritems():
 #                           if axes.issuperset(domain_axes.get(key, ())):
#                                axes_out[key] = value
                else:
                    # --------------------------------------------
                    # Select items which span exactly the given
                    ...
                    # --------------------------------------------
                    for key, value in pool.iteritems():
                        if axes == set(domain_axes.get(key, ())):
                            axes_out[key] = value
                #--- End: if
            #--- End: if
...
'''
    ...
    def insert_measure(self, msr, key=None, axes=None, copy=True, replace=True):
...
:Parameters:
    ...
    replace : bool, optional
        ...
        same identifier is replaced with *msr*.
...
>>>
        ...
        msr = self._insert_item(msr, key, 'cell measure', axes=axes, copy=copy)
    ...
    def insert_ref(self, ref, key=None, copy=True, replace=False):
        '''
...
'''
        ...
        for key, item in d.items(items, **kwargs).iteritems():
            ...
            if x in 'da':
                d._replace_refs_coord_identifier(key)
                
            del d._map[key]
      
            d._axes.pop(key, None)
...
'''
    ...
    def axis(self, axis=None, size=None, **kwargs):
        '''
...
'''
        axes = self.axes(axis, size=size, **kwargs)
        if not axes:
            return
...
'''
        ...
        # ------------------------------------------------------------
#        key_map = {}
##        for self_keys, other_keys in izip(sorted(self.d),  sorted(self.a),  sorted(self.m),
##                                          sorted(other.d), sorted(other.a), sorted(other.m)):
#        for self_keys, other_keys in izip((sorted(self.d),  sorted(self.a),  sorted(self.m)),
#                                          (sorted(other.d), sorted(other.a), sorted(other.m))):
