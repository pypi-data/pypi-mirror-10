...
'''       
        ...
        if n_groups == 1:
            ...
            # There are multiple groups of records, distibguished by
        ...
        # ------------------------------------------------------------
        self.lbtim_ia, ib = divmod(int_hdr[lbtim], 100)
        ...
        if header_um_version > 0 and len(um_version) <= 3:
        ...
        for recs, nz, nt in zip(groups, groups_nz, groups_nt):
            ...
            if axiscode is not None:             
                ...
                LBLEV = int_hdr[lblev]
                if LBVC in (2, 9) or LBLEV in (7777, 8888): # CHECK!
                    self.LBLEV = LBLEV
                    c = self.model_level_number_coordinate(aux=(not c)) 
            #--- End: if
            ...
            cf_properties['lbproc']      = LBPROC
            cf_properties['stash_code']  = stash
...
'''
    ...
    def atmosphere_hybrid_height_coordinate(self, axiscode):
        '''Can't calculate eta (because we don't know the height of
        the top level), so we'll create dimension coordinate for
        model_level_number instead.
...
''' 
        ...
        if c is not None:
            if aux:
                self.domain.insert_aux(c, key=[_axis['z']], copy=True)
            else:
...
'''
        array = tuple([rec.real_hdr.item(blev,) for rec in self.z_recs])
...
'''    
    ...
    um = [UMField(var, f.format, f.byte_ordering, f.word_size, um_version,
                  set_standard_name, history=history)
          for var in f.vars]
