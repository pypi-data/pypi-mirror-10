...
class LocalStarter(Starter):
    ...
    def start(self, command, fileset={}):
        ...
        def spawn_process(command, path=None):
            ...
            d = defer.maybeDeferred(reactor.spawnProcess, protocol, command[0], command, env=None, path=path)
            def started(_):
    ...
    def start(self, command_args, fileset={}):
        ...
        def on_connected(connection):
            ...
            def command_started(proc):
                return _SSHProcess(proc, connection)
...
class _SSHProcess(object):
    ...
    resources.
    """
    def __init__(self, sshprocess, sshconnection):
...
class EC2Starter(Starter):
    ...
    def __init__(self, provider, provider_keyid, provider_key, image_id, size_id, public_key_file, private_key_file, tmp_dir, **provider_kwargs):
    ...
    def start(self, command, fileset={}):
        ...
        def public_ips_received(ips):
            ...
            ip = ips[0]
            sshstarter = SSHStarter(ip, username="root", private_keys=[self.private_key], tmp_dir=self.tmp_dir)
