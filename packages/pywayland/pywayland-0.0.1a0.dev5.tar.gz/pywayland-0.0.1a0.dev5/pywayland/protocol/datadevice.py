...
class DataDevice(Interface):
    ...
    global :class:`~pywayland.protocol.datadevicemanager.DataDeviceManager`
    singleton.
...
def start_drag(self, source, origin, icon, serial):
    ...
    :func:`Surface.attach() <pywayland.protocol.surface.Surface.attach>`
    request can move the relative position. Attach requests must be confirmed
    with :func:`Surface.commit() <pywayland.protocol.surface.Surface.commit>`
    as usual. The icon surface is given the role of a drag-and-drop icon. If
    the icon surface already has another role, it raises a protocol error.

    The current and pending input regions of the icon
    :class:`~pywayland.protocol.surface.Surface` are cleared, and
    :func:`Surface.set_input_region()
    <pywayland.protocol.surface.Surface.set_input_region>` is ignored until the
    :class:`~pywayland.protocol.surface.Surface` is no longer used as the icon
    surface. When the use as an icon ends, the current and pending input
...
def data_offer(self, id):
    ...
    :class:`~pywayland.protocol.dataoffer.DataOffer` object, which will
...
def leave(self):
    ...
    :class:`~pywayland.protocol.dataoffer.DataOffer` introduced at enter time
    at this point.
    """
    self._post_event(2)
...
def selection(self, id):
    ...
    :class:`~pywayland.protocol.dataoffer.DataOffer` for the selection for this
