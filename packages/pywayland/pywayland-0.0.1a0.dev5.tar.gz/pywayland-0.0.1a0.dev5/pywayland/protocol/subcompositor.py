...
class Subcompositor(Interface):
    ...
    :class:`~pywayland.protocol.surface.Surface`, that has sub-surfaces
    ...
    :class:`~pywayland.protocol.surface.Surface` objects is to be considered as
    a single window, and it should also behave as such.
    ...
    :class:`~pywayland.protocol.surface.Surface` objects. This should allow the
    compositor to pass YUV video buffer processing to dedicated overlay
...
def destroy(self):
    ...
    :class:`~pywayland.protocol.subsurface.Subsurface` objects included.
    """
    self._marshal(0)
    self._destroy()
...
def get_subsurface(self, surface, parent):
    ...
    :class:`~pywayland.protocol.surface.Surface` into a sub-surface.

    The to-be sub-surface must not already have another role, and it must not
    have an existing :class:`~pywayland.protocol.subsurface.Subsurface` object.
