...
class Buffer(Interface):
    ...
    :class:`~pywayland.protocol.surface.Surface`. Buffers are created through
    factory interfaces such as :class:`~pywayland.protocol.drm.Drm`,
    :class:`~pywayland.protocol.shm.Shm` or similar. It has a width and a
    height and can be attached to a
    :class:`~pywayland.protocol.surface.Surface`, but the mechanism by which a
...
def destroy(self):
    ...
    <pywayland.protocol.surface.Surface.attach>`.
    """
    self._marshal(0)
    self._destroy()
...
def release(self):
    ...
    <pywayland.protocol.surface.Surface.commit>` that attaches this
    ...
    :class:`~pywayland.protocol.surface.Surface` contents, e.g. as a GL
    texture. This is an important optimization for GL(ES) compositors with
    :class:`~pywayland.protocol.shm.Shm` clients.
    """
    self._post_event(0)
