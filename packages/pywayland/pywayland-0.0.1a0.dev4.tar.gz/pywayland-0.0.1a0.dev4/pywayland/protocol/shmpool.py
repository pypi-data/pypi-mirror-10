...
class ShmPool(Interface):
    ...
    can allocate shared memory :class:`~pywayland.protocol.buffer.Buffer`
    objects. All objects created through the same pool share the same
    underlying mapped memory. Reusing the mapped memory avoids the
    setup/teardown overhead and is useful when interactively resizing a surface
...
def create_buffer(self, offset, width, height, stride, format):
    ...
    :func:`Shm.format() <pywayland.protocol.shm.Shm.format>` event.
