...
Reference = namedtuple('Reference', ['label', 'docname', 'text', 'internal'])
...
class Blog(object):
    ...
    def add_reference(self, label, docname, text, internal=None):

        if internal is None:
            internal = 'html' in self.builder

        Blog.references[label] = Reference(label=label, docname=docname,
            text=text, internal=internal)
...
class Post(object):
    ...
    def __init__(self, ablog, docname, info):
        ...
        ablog.add_reference(label, docname, title, internal=True)
