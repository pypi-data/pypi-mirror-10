...
주    의 : 1. range_address는 튜플타입 
             (좌측상단셀 행번호,좌측상단셀 열번호,우측하단셀 행번호,우측하단셀 열번호)
           2. range_address를 지정하지 않으면 A1셀부터 UsedRange까지를 반환한다.
    '''
#개발자주의사항 : #xlwings모듈의 Workbook, Range객체는 Excel데이터 입출력에 있어 매우 편리하나, 
                #(ex. 좌측상단의 셀주소만 알면 Range 전체를 입력가능)
...
#개발자주의사항 : #xlwings모듈의 Workbook, Range객체는 Excel데이터 입출력에 있어 매우 편리하나, 
    ...
    if data.values.shape[0] > 1048576 or data.values.shape[1] > 16384:
        print("data의 크기가 Excel 시트의 범위를 넘습니다. 분할이 필요합니다!")
        return 0
