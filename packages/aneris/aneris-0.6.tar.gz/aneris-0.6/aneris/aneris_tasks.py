...
from aneris.aneris_db_if import aneris_db_if
			...
			if(self.m_tasks[task_index]["is_a_project"] == True):
				...
				if(task_index >= self.m_task_cursor):
					...
					if(project_completion_day > self.m_tasks[task_index]["deadline"]):
						window.addstr(y, 4, "{0:3d} h by {1} : exp. {2} : {3} :  ~ LATE! ".format( \
										...
										self.m_tasks[task_index]["text"])[:width-5], style)
					else:
				...
				if(task_index >= self.m_task_cursor):
					...
					if(self.m_tasks_visible == True):
						...
						window.addstr(y, 5, "{0:3d} h remaining : {1}".format(self.m_tasks[task_index]["hours_work"], self.m_tasks[task_index]["text"])[:width-6], style)
						y += 1
