...
#define YAYI_DEBUG_MESSAGE_STREAM(s)\
  "File :\t\t"  __FILE__  "\n" \
  "Line :\t\t" << __LINE__ << "\n" \
  "Message :\t" << s
...
#define YAYI_THROW(s)\
  {yayi::errors::yayi_error_stream() << YAYI_DEBUG_MESSAGE_STREAM(s) << std::endl; \
...
#define YAYI_ASSERT(cond, s)\
  ...
  #define DEBUG_INFO(s)           {errors::yayi_error_stream() << YAYI_DEBUG_MESSAGE_STREAM(s) << std::endl;}
#else
