...
class TsDbClient(object):
    ...
    def get_hlocv(self, name, start=None, end=None, field=None):
        ...
        try:
            ...
            rows = curs.fetchall()
            if field is None:
                def parse(v): return [np.nan if x is None else x for x in v]
                crv = HlocvCurve.__new__(HlocvCurve)
                crv.knot_cls = HlocvKnot
                crv.data = [HlocvKnot(Date.parse(r[0]),
                                      parse(r[1:])) for r in rows]
            else:
                def parse(x): return np.nan if x is None else x
                idx = 1 + HLOCV_FIELDS.index(field)
                crv = Curve.__new__(Curve)
                crv.knot_cls = Knot
                crv.data = [Knot(Date.parse(r[0]),
                                 parse(r[idx])) for r in rows]
            crv.data.sort()
        finally:
            curs.close()
