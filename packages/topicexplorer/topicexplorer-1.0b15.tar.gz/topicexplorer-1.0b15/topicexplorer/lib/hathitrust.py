...
def solr_query(query, format="json", sleep_time=1):
    solr = "http://chinkapin.pti.indiana.edu:9994/solr/meta/select/?q={0}".format(format)
    if format == 'json':
...
def get_metadata(folder):
    ...
    data = [(id.strip(), metadata(id.strip())) for id in ids 
        if not id.endswith('.log') and not id.endswith('.err')]
    data = dict(data)
