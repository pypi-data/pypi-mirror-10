...
class Runner(object):
    def __init__(self, basedir='~/.ohh'):
        ...
        self._read_config()
        self._background_runner = BackgroundRunner(self._pid_file)
    ...
    def _post(self, path, data=None):
        ...
        token = self.config.get('ohh', 'token', None)
        if token:
    ...
    def start(self, start_daemon=True):
        ...
        if start_daemon and not self.is_running():
    ...
    def show_info(self):
        ...
        timediff = self._timediff(status['start_time'])
        print("You have been {0} for {1}.".format(status['status'], timediff))
