...
REMOTE_QUERY_FIELDS = unicode(
  "nextPageToken,items(id,md5Checksum,title,version,"
  "mimeType,quotaBytesUsed,fileSize,modifiedDate,"
  "userPermission(role),labels(restricted),"
  "parents(id,isRoot))")
...
class driveInfo(object):
  ...
  def remove(self, info, rmtree=False, ig=False):
    ...
    # don't start removing things until we've worked out what children
    # various folder/directory resources have (see fixup) because this
    # method will want to muck with those relationships
  ...
  def fixup(self):
    for info in self._id_dict.values():
      if info.parent_isroot is False:
        parent_info = self.parent(info)
        assert(parent_info)
        parent_info.children_ids.append(info.id)
  ...
  def fixup(self):
    ...
    for id in list(self._id_dict.keys()):
      ...
      if info.path:
        ...
        # an orphan (see calc_path). We won't need this
...
class resourceDataBase(object):
  ...
  def __init__(self, rootdir=None, makecopy=False):
    ...
    self.MOD = 1
    if rootdir:
      shelf_name = rootdir + '/' + CLIENTSHELFFILE
      try:
...
def adjust_child_paths(drive, info):
  ...
  for child in drive.children(info):
    drive.remove(child)
    child.path = info.path + '/' + child.name
    drive.add(child)
    if child.is_folder:
  ...
  while not ended:
    ...
    try:
      resp = list_files(
        ...
        maxResults=100,
        q='trashed=false',
        fields=REMOTE_QUERY_FIELDS).execute(num_retries=GDAPINUMRETRIES)
...
def check_for_identicals(gs_data):
  ...
  for i, threshold, threshold_str in zip(
      list(range(4)),
      [0x400 << 20, 0x400 << 10, 0x400, 0x1],
      ['Gbytes', 'Kbytes', 'Mbytes', 'bytes']):
    if total >= threshold:
      printf(
        "there are %d set(s) of duplicated files, using %s %s of space\n",
        len(dups),
        "%.2f" % float(total / threshold) if i < 3 else "%d" % total,
        threshold_str)
      break
...
def rm_local_resource(gs_data, info):
  ...
  if prog_args.dry_run:
    gs_data.trans.local_rm(info.path)
    return True
...
def rm_remote_resource(gs_data, info):
  ...
  if prog_args.dry_run:
    if not info.no_delete:
      gs_data.trans.remote_rm(info.path)
    return True
...
def dump_trans():
  ...
  filename = prog_args.root_dir + '/' + CLIENTTRANSFILE
...
def wipe_shelf_data():
  shelf_name = prog_args.root_dir + '/' + CLIENTSHELFFILE
  try:
...
def wipe_client_secrets():
  clientsecrets = prog_args.root_dir + '/' + CLIENTSECRETSFILE
  datafile = prog_args.root_dir + '/' + CLIENTDATAFILE
  try:
...
def auth(http):
  ...
  secretsfile = prog_args.root_dir + '/' + CLIENTSECRETSFILE
  datafile = prog_args.root_dir + '/' + CLIENTDATAFILE
...
def check_client_secrets():
  ...
  secretsfile = prog_args.root_dir + '/' + CLIENTSECRETSFILE
  storagedir = prog_args.root_dir + '/' + CLIENTSTORAGEDIR
