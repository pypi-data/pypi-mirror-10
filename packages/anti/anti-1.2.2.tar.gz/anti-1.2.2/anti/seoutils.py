...
from .utils import get_or_create, GetHash, check_page, check_absolute_page, get_normal_url, transaction, remove_last_slash
...
class SeoParser():
    ...
    def GetPageTic(self, days = False):
    ...
    def GetYaca(self):
        import urllib
        yurl = 'http://bar-navig.yandex.ru/u?ver=2&show=32&url=%s' % self.site
        f = urllib.urlopen(yurl)
        st = f.read()
    ...
    def check_www(self):
        ...
        try:
            reqst = requests.get(url, verify=False)
            if len(reqst.history) > 0:
                if remove_last_slash(reqst.url) == www_url and 302 not in [item.status_code for item in reqst.history]:
                    return 'www.'
        ...
        try:
            reqst = requests.get(www_url, verify=False)
            if len(reqst.history) > 0:
                if remove_last_slash(reqst.url) == url and 302 not in [item.status_code for item in reqst.history]:
                    return True
        except:
            pass
    ...
    def check_yandex_www(self):
        ...
        data = obj.get_yandex_cache_pos('host:%s' % urllib.quote_plus(self.site))
        if len(data) > 0:
            return True
        data = obj.get_yandex_cache_pos('host:www.%s' % urllib.quote_plus(self.site))
        if len(data) > 0:
            return 'www.'
        return False
    ...
    def check_yandex_pages(self):
        ...
        link = link % (urllib.quote_plus(self.site), urllib.quote_plus(self.site))
        obj = openYandex(self.KEY_ANTIGATE)
    ...
    def check_last_update(self, days = 7):
        link = u'http://yandex.ru/yandsearch?text=host:%s | host:www.%s&site=&rstr=&within=0&numdoc=50&lr=213&how=tm' % (urllib.quote_plus(self.site), urllib.quote_plus(self.site))
        obj = openYandex(self.KEY_ANTIGATE)
    ...
    def check_google_pages(self, days = 7):
        #!TODO: доделать кэш
        link = 'https://www.google.ru/search?num=100&q=site:%s/' % urllib.quote_plus(self.site)
    ...
    def check_google_pages_clear(self, days = 7):
        #!TODO: доделать кэш
        link = 'https://www.google.ru/search?num=100&q=site:%s/&' % urllib.quote_plus(self.site)
        obj = openGoogle(self.KEY_ANTIGATE)
        soup = obj.get_soup(link)
    ...
    def get_counter(self):
        rsst = requests.get('http://counter.yadro.ru/values?site=%s' % (self.site), verify = False)
        res = re.search('(LI_month_vis = )(\d+);',rsst.text)
        if res:
            return int(res.group(2))
        else:
            rsst = requests.get('http://counter.yadro.ru/values?site=www.%s' % (self.site), verify = False)
            res = re.search('(LI_month_vis = )(\d+);',rsst.text)
            if res:
                return int(res.group(2))
        return 0
