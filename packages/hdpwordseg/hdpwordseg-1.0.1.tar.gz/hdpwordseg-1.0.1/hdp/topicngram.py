        ...
        for z in xrange(self._topicdist.get_maxtopics()):
            ...
            for y in xrange(1, len(self._text) + 1):
                ...
                for x in self.unigrams(y):
                    key = self.genkey(self._idx, z, x, y)
            ...
            for y in xrange(1, len(self._text) + 1):
                ...
                for x in self.unigrams(y):
                    if x > 0:
                        choices = []
                        for k in self.unigrams(x):
                            key = self.genkey(self._idx, z, k, x, y)
                            p = math.log(self._totaldist[key])
                            (p1, k1) = tbl[z][x][k]
                        ...
                        key = super(TopicBigram, self).genkey(self._idx, z, x, y)
                        p = math.log(self._totaldist[key])
                        tbl[z][y][x] = (p, None)
        return tbl
...
class TopicTrigram(TopicBigram):
    ...
    def genkey(self, idx, z, k2, k1, x, y):
        t0 = self.get_text(x, y)
                    ...
                    if x > 1:
                        ...
                        for k1 in self.unigrams(x):
                            ...
                            for k2 in self.unigrams(k1):
                                ...
                                key = self.genkey(self._idx, z, k2, k1, x, y)
                                p = math.log(self._totaldist[key])
                                (p1, _) = tbl[z][x][k1]
                                (p2, _) = tbl[z][k1][k2]
                        ...
                        choices = []
                        for k in self.unigrams(x):
                            key = super(TopicTrigram, self).genkey(self._idx, z, k, x, y)
                            p = math.log(self._totaldist[key])
                            (p1, _) = tbl[z][x][k]
...
class TopicWordseg(Wordseg):
    ...
    def acctotal(self):
        for z in self._unidist.prems():
            for t0 in self._unidist.consqs(z):
                key = (z, t0)
                self._totaldist.acc(key, self._unidist.pdf(t0, z))
        for ctx in self._bidist.prems():
            z = ctx.z
            (t1,) = ctx.ts
            for t0 in self._bidist.consqs(ctx):
                key = (z, t1, t0)
                self._totaldist.acc(key, self._bidist.pdf(t0, ctx))
