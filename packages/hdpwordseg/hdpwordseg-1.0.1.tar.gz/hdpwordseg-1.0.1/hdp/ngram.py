            ...
            for x in self.unigrams(y):
                if x > 0:
                    ...
                    for k in self.unigrams(x):
                        key = self.genkey(self._idx, k, x, y)
                    ...
                    key = self.genkey(self._idx, None, x, y)
                    p = math.log(self._totaldist[key])
                    tbl[y][x] = (p, None)
        return tbl
                ...
                if x > 1:
                    ...
                    for k1 in self.unigrams(x):
                        ...
                        for k2 in self.unigrams(k1):
                            ...
                            key = self.genkey(self._idx, k2, k1, x, y)
                            p = math.log(self._totaldist[key])
                            (p1, _) = tbl[x][k1]
                            (p2, _) = tbl[k1][k2]
                    ...
                    for k in self.bigrams(x, y):
                        key = super(Trigram, self).genkey(self._idx, k, x, y)
                        p = math.log(self._totaldist[key])
                        (p1, _) = tbl[x][k]
                    ...
                    key = super(Bigram, self).genkey(self._idx, x, y)
                    p = math.log(self._totaldist[key])
                    tbl[y][x] = (p, (None, None))
        return tbl
...
class Wordseg(object):
    ...
    def __init__(
        ...
        if disk is not None:
            ...
            disk_tri = None
            disk_total = None
            totaldist = IdxTable(tempfile.mktemp() + '_total', defval=1e-6)
    ...
    def acctotal(self):
        for ctx in self._unidist.prems():
            for t0 in self._unidist.consqs(ctx):
                self._totaldist.acc(t0, self._unidist.pdf(t0))
        for t1 in self._bidist.prems():
            for t0 in self._bidist.consqs(t1):
                key = (t1, t0)
                self._totaldist.acc(key, self._bidist.pdf(t0, t1))
        for ctx in self._tridist.prems():
            (t1, t2) = ctx.ts
            for t0 in self._tridist.consqs(ctx):
                key = (t0, t1, t2)
                self._totaldist.acc(key, self._tridist.pdf(t0, ctx))

    def normtotal(self):
        if type(self._totaldist) is dict:
            for k in self._totaldist:
                self._totaldist[k] /= self._maxiters
        else:
            self._totaldist.normalize(float(self._maxiters))

    def run_oneloop(self, x, step):
        if self._verbose:
            sys.stderr.write('Iter %d: ' % (x + 1))
