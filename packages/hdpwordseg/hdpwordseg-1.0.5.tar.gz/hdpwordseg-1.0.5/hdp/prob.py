...
class IdxTable:
    ...
    def __init__(self, disk, defval=0.0):
        self._disk = disk
        self._fname_tbl = disk + '.idx'
        self._fname_set1 = disk + '.set1'
        self._fname_set2 = disk + '.set2'
        self._fname_set3 = disk + '.set3'
        self._tbl = shelve.open(
            self._fname_tbl, protocol=pickle.HIGHEST_PROTOCOL
        )
        self._set1 = shelve.open(
            self._fname_set1, protocol=pickle.HIGHEST_PROTOCOL
        )
        self._set2 = shelve.open(
            self._fname_set2, protocol=pickle.HIGHEST_PROTOCOL
        )
        self._set3 = shelve.open(
            self._fname_set3, protocol=pickle.HIGHEST_PROTOCOL
        )
        self._defval = defval
    ...
    def close(self):
        ...
        self._set3.close()
        os.remove(self._fname_tbl)
        os.remove(self._fname_set1)
        os.remove(self._fname_set2)
        os.remove(self._fname_set3)
    ...
    def add_unikey(self, key):
        k = self.index(key)
        if k not in self._set1:
            self._set1[k] = True

    def add_bikey(self, key):
        k = self.index(key)
        if k not in self._set2:
            self._set2[k] = True

    def add_trikey(self, key):
        k = self.index(key)
        if k not in self._set3:
            self._set3[k] = True

    def unikeys(self):
        for k in self._set1.keys():
            yield cPickle.loads(k)

    def bikeys(self):
        for k in self._set2.keys():
            yield cPickle.loads(k)

    def trikeys(self):
        for k in self._set3.keys():
            yield cPickle.loads(k)
