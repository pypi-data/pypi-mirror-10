...
def builddefaults(args):
    ...
    defaults['slicesmp'] = 'on' if args.slicesmp else 'off'
...
def configdict(cfgfname, args):
    ...
    if cfgfname is not None:
        ...
        cfgdict['slicesmp'] = config.getboolean('Experiment', 'slicesmp')
        ...
        cfgdict['slicesmp'] = (defaults['slicesmp'] == 'on')
...
parser.add_argument(
    '-L', '--noslicesmp', dest='slicesmp', action='store_false',
    help='Disable the slice sampling [default=enabled]',
    default=True
)
parser.add_argument(
...
def main():
    ...
    if cfgdict['topics'] in [0, 1, None]:
        wordseg = Wordseg(
            ...
            slicesmp=cfgdict['slicesmp'], disk=disk
        )
    else:
        unidist = None
        topicdist = None
        ...
        wordseg = TopicWordseg(
            ...
            maxtopics=cfgdict['topics'], unidist=unidist, 
            slicesmp=cfgdict['slicesmp'], topicdist=topicdist
        )
