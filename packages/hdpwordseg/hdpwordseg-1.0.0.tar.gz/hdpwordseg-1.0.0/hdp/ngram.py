            ...
            for x in self.unigrams(y):
                ...
                if x > 0:
                    ...
                    for k in self.unigrams(x):
                        ...
                        key = (self._idx, k, x, y)
                        prob = self.pdf_bi(t0, t1)
                        choices.append((prob, key))
                    for (p, k) in nlargest(self._beamsize, choices):
                        self.addtotal(k, p)
                else:
                    key = (self._idx, None, x, y)
                    self.addtotal(key, self.pdf(x, y))
        self._n += 1.0
            ...
            for x in self.unigrams(y):
                ...
                if x > 1:
                    ...
                    for (k2, k1) in self.trigrams(x, y):
                        ...
                        key = (self._idx, k2, k1, x, y)
                        prob = self.pdf_tri(t0, t1, t2)
                        choices.append((prob, key))
                    for (p, k) in nlargest(self._beamsize, choices):
                        self.addtotal(k, p)
                elif x > 0:
                    for k1 in self.bigrams(x, y):
                        t1 = self.get_text(k1, x)
                        key = (self._idx, None, k1, x, y)
                        self.addtotal(key, self.pdf_bi(t0, t1))
                else:
                    key = (self._idx, None, None, x, y)
                    self.addtotal(key, self.pdf_uni(t0))
        self._n += 1.0
