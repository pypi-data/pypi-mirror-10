...
class Unigram(ProbWordLattice):
    ...
    def __init__(
        ...
        super(Unigram, self).__init__(
            ...
            totaldist=totaldist, idx=idx, slicesmp=slicesmp
        )
...
class Wordseg(object):
    ...
    def __init__(
        ...
        if unidist is None:
            if not isinstance(strdist, TopicStrDist):
                strpdf = lambda s, p: strdist.pdf(s)
            else:
                strpdf = lambda s, p: strdist.pdf(s, p)
            unidist = CRP(alpha_uni, theta_uni, disk=disk_uni, basepdf=strpdf)
        if bidist is None:
            bidist = CRP(alpha_bi, theta_bi, disk=disk_bi, basepdf=unidist.pdf)
        if tridist is None:
            tridist = CRP(alpha_tri, theta_tri, disk=disk_tri, basepdf=bidist.pdf)
        ...
        self._lattices = []
        self._itercnt = 0
        self._poisreg = Poisson(slicesmp) if self._poisson else None
        self._verbose = False
        self._debug = False
        self._cnt = 0
        self._temp = maxtemp
    ...
    def run(self):
        if self._debug:
            ...
            sys.stderr.flush()
        denom = self._maxiters if self._burnin is None \
                else (self._maxiters - self._burnin)
        step = self._maxtemp / (denom - 1.0)
        n = 0.0
    ...
    def run_oneloop(self, x, step):
        if self._verbose:
            sys.stderr.write('%6d: [t=%f] ' % (x + 1, self._temp))
    ...
    def save_samples(self, i):
        ...
        if samples is None or len(samples) == 0:
            print m.repr().encode('utf-8')
            L = len(m)
            print [x for x in m.unigrams(L)]
            print [m.pdf(x, L) for x in m.unigrams(L)]
            raw_input()
        prob = m.pdf_samples(samples) if len(samples) > 0 \
               else float('-inf')
    ...
    def gibbs(self, i, temp):
        ...
        m.learn(m.getsent())
        if samples_new is None or len(samples_new) == 0:
            print 'gibbs old = %s' % samples_old
            print 'gibbs new = %s' % samples_new
            raw_input()
    ...
    def mh(self, i, temp):
        ...
        if samples_new is None or len(samples_new) == 0:
            print 'mh old = %s' % samples_old
            print 'mh new = %s' % samples_new
            raw_input()
        if p_new > 0.0:
            acc = p_old / p_new
        else: acc = None
    ...
    def sample_hparams(self):
        if self._hparamsmp:
            self._tridist.estimate_hparams()
            self._bidist.estimate_hparams()
            self._unidist.estimate_hparams()
        if self._poisson:
