...
def urls(url):
    ...
    urls = {"dir": url_root, "filename": url_filename, "fileid": fileid, 'domain': url_host}
    return urls
...
def rebuild_img(content):
    return regex.replace("<img[\w\W]*?src=['\"](.*?\.jpg)['\"][\w\W]*?>", '<img src="\g<1>" class="img-responsive">', content)
