...
def distance_content(source, clear=False, article=False, duplicate=False):
    ...
    def clear_tags(content, tag):
        ...
        :return:
        """
        pattern = re.compile(r'(##a [\w\W]*?##/a##)|(##{0}##)'.format(tag) + u'|(┠)', re.I)
    ...
    def recover(content, tag):
        ...
        :return:
        """
        pattern = re.compile(r'##{0}##'.format(tag), re.I)
        return re.sub(pattern, r'<\g<tag>>', content)
    ...
    x = [len(i) for i in z]
    maxX = max(x)
    body = lines[x.index(maxX)]

    index = 0
    if maxX > 100 and index == 0:
        index = maxX
        # print(maxX)
