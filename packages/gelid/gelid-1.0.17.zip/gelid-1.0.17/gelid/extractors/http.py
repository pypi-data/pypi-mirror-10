...
class Request(object):
    ...
    """
    def set_response(self, url, timeout=request_timeout):

        headers = dict()
        self.url = url
        if url and str(url).startswith('http'):
            html, headers = Http.get_unicode_with_header(url, count=self.count, data=self.data, headers=self.headers, timeout=self.timeout)
            self._body = html
