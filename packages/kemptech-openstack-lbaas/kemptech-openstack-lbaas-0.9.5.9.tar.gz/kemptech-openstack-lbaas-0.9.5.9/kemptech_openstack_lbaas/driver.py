...
class PoolManager(ManagerBase):
    ...
    def create(self, context, pool):
        ...
        prep_pool = self.api_helper.prepare_pool(context, pool)
        f = open('/tmp/shanesucks', 'w')
        f.write(repr(prep_pool))
        f.close()
        try:
            self.client.create_virtual_service(prep_pool)
...
class HealthMonitorManager(ManagerBase):
    ...
    def create(self, context, health_monitor):
        ...
        if "http" in health_monitor['type']:
            params = self.api_helper.prepare_health_monitor(context, health_monitor)
...
class MapNeutronToKemp(object):
    ...
    def prepare_pool(self, context, pool):
            ...
            for key in dir(model):
                if not key.startswith('__') and not callable(getattr(model, key)):
                    try:
                        ...
                        continue
                    if key == "lb_algorithm":
                        vs_params['schedule'] = kemp_consts.LB_METHODS.get(value)
        ...
        if pool.sessionpersistence is not None:
            for session_persist in lb_constants.SUPPORTED_SP_TYPES:
                if session_persist == pool.sessionpersistence.type:
                    ...
                    if persistence == kemp_consts.PERS_ACT_COOKIE:
                        cookie = pool.sessionpersistence.cookie_name
                        vs_params['cookie'] = cookie
        return vs_params

    def prepare_member(self, context, member):
        rs_params = {}
    ...
    def prepare_health_monitor(self, context, health_monitor):
        ...
        prep_hm = self.prepare_pool(context, health_monitor.pool)
        return prep_hm
