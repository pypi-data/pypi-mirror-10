    ...
    var DjangoFormset = Class.extend({
        ...
        spliceInto: function($form, index, isNewAddition) {
            ...
            if ($form.djangoFormsetPrefix() == this.prefix) {
                ...
                this.$inline.parent().closest('.djnesting-stacked').each(function() {
                    var $parentForm = self.$inline.closest('.nested-inline-form');
                    var parentPkField = $(this).data('fieldNames').pk;
                    var $parentPk = $parentForm.djangoFormField(parentPkField);
                    parentPkVal = $parentPk.val();
                });

                if (!isInitial && parentPkVal && $form.attr('data-is-initial') == 'true') {
                    isInitial = true;
                    $form.data('isInitial', true);
                }
