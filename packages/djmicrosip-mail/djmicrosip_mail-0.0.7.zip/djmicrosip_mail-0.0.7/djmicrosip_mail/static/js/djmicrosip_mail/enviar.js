$(document).ready(function() {
	...
	function enviar_sms(){
		...
		$.ajax({
			url:'/mail/enviar_mail/', 
			type : 'get', 
			data:{'mensaje':mensajes[(numero_mensaje-1)][1],
				'email':mensajes[(numero_mensaje-1)][0],
			...
			success: function(data){ 
				...
				console.log(data.resultado+'--'+data.mail)
				if (data.resultado!='Mensaje enviado') {
					if (data.resultado == 'Apikey incorrecta' || data.resultado == 'Apikey no definido') {
						incorrectos.push(data.mail+' = Llave errónea')
					}
					else{
						incorrectos.push(data.mail+' = '+data.resultado);
					};
