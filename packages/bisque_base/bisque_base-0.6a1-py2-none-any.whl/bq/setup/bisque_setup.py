...
PSERVE_DEFAULT = config_path('server.ini.default')
...
def initialize_database(params, DBURL=None):
    ...
    if not db_initialized and getanswer(
        ...
        """) == "Y":
        if call (['gearbox','setup-app', '-c', config_path('site.cfg')]) != 0:
...
def install_modules(params):
    ...
    if not os.path.exists ('modules'):
        print "No mdoule directory found..  need to create a module"
        os.makedirs ('modules')
        return params
...
def install_engine_defaults(params):
    ...
    #if not os.path.exists(config_path('who.ini')):
    #    shutil.copy(config_path('who.engine.default'), config_path('who.ini'))
    ...
    if getanswer ('Do you want to create/edit new server configuations', 'N',
                  "Use an editor to edit the server section (see http://biodev.ece.ucsb.edu/projects/bisquik/wiki/Installation/ParsingSiteCfg )") == 'Y':
        setup_server_cfg(params)
    return params
...
def setup_server_cfg (params):
    ...
    if server_params['backend'] == 'pserve':
        params = setup_pserve(params, server_params)
    return params
...
#######################################################
#
def setup_pserve(params, server_params):
    if getanswer("Install pyramid (application server and configs)", 'Y',
                 "pserve is the default backend server") != 'Y':
        return params
    ...
    for server, sv in servers.items():
        cfg = config_path ("%s_pserve.cfg" % server)
        if os.path.exists (cfg) and os.path.exists(SITE_CFG):
            cfg_time  = os.stat(cfg).st_mtime
            site_time = os.stat (SITE_CFG).st_mtime
            if cfg_time - site_time > 60:
                if getanswer ("%s looks newer than %s.. modify" % (cfg, SITE_CFG), "N",
                              "%s may have special modifications" %cfg) == "N":
                    continue
        install_cfg (cfg, section="*", default_cfg=PSERVE_DEFAULT)

        pserve_vars = sv.get ('pserve', {})
        bisque_vars = sv.get ('bisque', {})
        ...
        if 'host' not in pserve_vars:
            pserve_vars['host'] = fullurl[1].split(':')[0]
        if 'port' not in pserve_vars:
            pserve_vars['port'] = str(fullurl.port)

        update_site_cfg(cfg=cfg, bisque_vars=svars)
        update_site_cfg(cfg=cfg, section='server:main',bisque_vars = pserve_vars )
...
def bisque_installer(options, args):
    ...
    if not os.path.exists('config'):
        if getanswer ("New installation: install new configuration", "Y",
                      "No config directory was found or specified") == 'Y':
            call (['pcreate', '-s', 'bisque_config', 'config'])
    if not os.path.exists('config'):
        print "Cannot find config.. cannot continue"
        sys.exit()
...
class setup(object):
    ...
    def run(self):
        ''
        r = bisque_setup( self.options, self.args )
        sys.exit(r)
