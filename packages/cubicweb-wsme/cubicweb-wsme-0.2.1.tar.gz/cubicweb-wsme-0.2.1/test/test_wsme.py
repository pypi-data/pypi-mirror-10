...
from cubicweb.devtools import testlib, httptest
import cubicweb
...
class WSMETest(testlib.CubicWebTC):
    @classmethod
...
class WSMECRUDTest(httptest.CubicWebServerTC):
    @classmethod
    def init_config(cls, config):
...
class WSMEQueryTest(httptest.CubicWebServerTC):
    @classmethod
    def init_config(cls, config):
    ...
    def test_reverse_inlined(self):
        ...
        res = self.web_request(
            ...
            body=('{"name": "test", "workflow_of": [{"eid": %s}], '
                  '"<state_of": [{"name": "draft"}]}') % etype_eid,
            headers={"Accept": "application/json",
                     "Content-Type": "application/json"})
