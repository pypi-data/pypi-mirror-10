...
class Base(six.with_metaclass(BaseMeta, wsme.types.Base)):
    ...
    def to_entity(self, cnx, extra_relations=()):
        """Apply the data to a new or existing entity (depending on the 'eid'
        value).

        :param cnx: A cw connection
        :param extra_relations: A list of relations (rtype, role, inlined, value)
        :return: The updated entity or a new one
        """
        eid = self.eid or None
        values = {}
        relation_values = []
        for attr in self._wsme_attributes:
            if not isinstance(attr, wsattr):
            ...
            if attr.isfinal:
                values[attr.rtype] = value
            elif attr.role == 'subject' and attr.inlined:
                values[attr.rtype] = value.to_entity(cnx)
            else:
                relation_values.append((attr, value))

        for rtype, role, inlined, value in extra_relations:
        ...
        if eid:
            ...
            entity = cnx.create_entity(self.__etype__, **values)

        for attr, value in relation_values:
            extra_relations = ()
            if attr.role == 'object' and attr.inlined:
                extra_relations = [(attr.rtype, 'subject', True, entity)]
            if value is not None:
                if wsme.types.isarray(attr.datatype):
                    targets = [
                        item.to_entity(cnx, extra_relations)
                        for item in value
                    ]
                else:
                    targets = [
                        value.to_entity(cnx, extra_relations)
                    ]
            else:
                targets = None

            if targets and extra_relations:
                continue

            d = {'x': entity.eid}
            if attr.role == 'subject':
                relation = 'X %s Y' % attr.rtype
            else:
                relation = 'Y %s X' % attr.rtype
            if targets is None:
                entity._cw.execute(
