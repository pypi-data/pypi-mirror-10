...
class SSHRunner(object):
    ...
    def run(self):
        if self.workspace:
            self.create_workspace(self._remote_workspace)
    ...
    def args(self):
        return self._ssh_args() + self._command_args()
    ...
    def create_workspace(self, dest):
        with self.spawn('mkdir', ['-p', dest]):
            pass
    ...
    def _remote_path(self):
        return '~/.ssh-run/{}/'.format(os.path.basename(os.getcwd()))
