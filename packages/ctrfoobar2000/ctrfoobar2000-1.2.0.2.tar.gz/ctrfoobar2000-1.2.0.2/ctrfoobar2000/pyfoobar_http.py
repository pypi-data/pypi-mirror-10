...
class foobar(object):
    ...
    def addFolder(self, folder):
        ...
        #http://192.168.10.10:8888/default/?cmd=Browse&param1=M%3A%5CWEST%5CROCK%5CChad%20Kroeger%20-%20Featuring%20%282014%29%5C&param2=EnqueueDir
        #http://192.168.10.10:8888/default/?cmd=Browse&param1=M%3A%5CWEST%5CROCK%5CChad%20Kroeger%20-%20Featuring%20%282014%29&param2=EnqueueDir
        if not folder[-1] == '\\':
            folder = folder + '\\'
        folder = str(folder).replace(':', '%3A')
        folder = str(folder).replace('\\', '%5C')
        folder = str(folder).replace(' ', '%20')
        folder = str(folder).replace('(', '%28')
        folder = str(folder).replace(')', '%29')
        data = {'cmd':'Browse', 'param1':folder, 'param2':'EnqueueDir'}
        url = self.setURL(data)
        #print "URL =", url
        return c_handle.play(url)
