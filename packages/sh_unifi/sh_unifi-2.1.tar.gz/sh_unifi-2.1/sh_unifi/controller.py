...
class Controller:
	...
	def get_wlan_groups(self):
		"""Return a list of user groups with its rate limiting settings."""

		return self._read(self.api_url + 'list/wlangroup')
	...
	def add_wlan(self, ssid, wlangroup, usergroup, password = False):
		params = dict()
		
		defaults = '{"name":"","security":"open","x_passphrase":"","wep_idx":"1","x_wep":"","enabled":true,"radius_ip_1":"","radius_port_1":"1812","x_radius_secret_1":"","is_guest":true,"vlan_enabled":false,"vlan":"","hide_ssid":false,"wpa_mode":"auto","wpa_enc":"ccmp","usergroup_id":"","wlangroup_id":""}'
		
		params = json.loads(defaults)
		
		params['name'] = ssid
		
		params['wlangroup_id'] = wlangroup
		params['usergroup_id'] = usergroup
		...
		return self._read(self.api_url +'add/wlanconf', json.dumps(params))
	...
	def add_site(self, site_id, site_name):
		params = dict()
		 
		params['cmd'] = 'add-site'
		params['name'] = site_id
		params['desc'] = site_name
		 
		return self._read(self.api_url +'cmd/sitemgr', json.dumps(params))
		 
	def set_site_auto_upgrade(self, value):
		params = dict()
		
		params['auto_upgrade'] = value
		
		return self._read(self.api_url +'set/setting/mgmt', json.dumps(params))
		
	def set_site_timezone(self, timezone):
		params = dict()
		
		params['key'] = 'locale'
		params['timezone'] = timezone
		
		return self._read(self.api_url +'set/setting/locale', json.dumps(params))
		
	def set_site_guest_access(self):
		data = '{"portal_enabled":true,"auth":"custom","x_password":"","redirect_enabled":false,"redirect_url":"","custom_ip":"93.191.132.61","expire":"480","voucher_enabled":false,"payment_enabled":false,"gateway":"paypal","x_paypal_username":"","x_paypal_password":"","x_paypal_signature":"","paypal_use_sandbox":false,"x_stripe_api_key":"","x_quickpay_merchantid":"","x_quickpay_md5secret":"","x_authorize_loginid":"","x_authorize_transactionkey":"","authorize_use_sandbox":false,"x_merchantwarrior_merchantuuid":"","x_merchantwarrior_apikey":"","x_merchantwarrior_apipassphrase":"","merchantwarrior_use_sandbox":false,"portal_customized":false,"portal_use_hostname":true,"portal_hostname":"portal.socialhotspotmanager.com","restricted_subnet_1":"192.168.0.0/16","restricted_subnet_2":"172.16.0.0/12","restricted_subnet_3":"10.0.0.0/8","restricted_subnet_4":null,"allowed_subnet_1":null}'
		
		return self._read(self.api_url +'set/setting/guest_access', data)
		
	def get_networks(self):
		return self._read(self.api_url +'list/networkconf')
		
	def set_guest_network(self):
		network_id = [network for network in self.get_networks() if network['name'] == 'LAN'][0]['_id']
		
		params = {
			'attr_hidden_id': "LAN",
			'attr_no_delete': True,
			'dhcpd_enabled': True,
			'dhcpd_start': "172.16.128.2",
			'dhcpd_stop': "172.16.159.254",
			'enabled': True,
			'ip_subnet': "172.16.128.1/19",
			'is_nat': True,
			'name': "Guest LAN",
			'networkgroup': "LAN",
			'purpose': "corporate",
		...
		return self._read(self.api_url +'upd/networkconf/'+ network_id, json.dumps(params))