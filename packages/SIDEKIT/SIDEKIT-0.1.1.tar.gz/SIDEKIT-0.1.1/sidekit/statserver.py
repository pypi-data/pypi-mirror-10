...
def fa_model_loop(index, batch_start, r, Phi, stat0, stat1, E_h, E_hh):
    """
    """
    for idx in index:
        invLambda = np.linalg.inv(np.eye(r) + \
                    (Phi.T * stat0[idx + batch_start, :]).dot(Phi))
        Aux = Phi.T.dot(stat1[idx + batch_start, :])
        E_h[idx] = Aux.dot(invLambda)
        E_hh[idx] = invLambda + np.outer(E_h[idx], E_h[idx])


def fa_distribution_loop(idx, _A, stat0, batch_start, batch_stop, E_hh):
    """
    """
    for c in idx:
...
if h5py_loaded:
    ...
    def load_statserver_hdf5(statserver, statserverFileName):
...
class StatServer:
    ...
    def __init__(self, statserverFileName='', statserverFileFormat='pickle'):
        ...
        # initialize by reading an existing StatServer
            ...
            if h5py_loaded:
                logging.warning('HDF5 does not save start and stop')
                load_statserver_hdf5(self, statserverFileName)
            else:
    ...
    def accumulate_stat(self, ubm, feature_server, segIndices=[]):
        ...
        """
        #assert os.path.exists(featureDir), 'featureDir does not exist'
        #assert ((featureFormat.upper() == 'SPRO4') | \
        #(featureFormat.upper() == 'HTK')), 'featureFormat must be SPRO4 or HTK'
        #assert isinstance(featureExtension, str), \
        #                                    'featureExtension must be a string'
    ...
    def whiten_stat1(self, mu, Sigma):
    ...
    def whiten_cholesky_stat1(self, mu, Sigma):
        ...
        if Sigma.ndim == 2:
            ...
            chol_invcov = np.linalg.cholesky(invSigma)
    ...
    def get_lda_matrix_stat1(self, rank):
        ...
        DiscriminationMatrix = np.dot(Sb, np.linalg.inv(Sw)).transpose()
        eigenValues, eigenVectors = np.linalg.eig(DiscriminationMatrix)
    ...
    def get_wccn_choleski_stat1(self):
        ...
        invW = np.linalg.inv(WCCN)
        W = np.linalg.cholesky(invW)
        return W
    ...
    def ivector_extraction_weight(self, ubm, W, Tnorm, delta=np.array([])):
        ...
        for iv in range(self.stat0.shape[0]):  # loop on i-vector
            ...
            # Estimate i-vector
            enroll_iv.stat1[iv, :] = np.linalg.solve(L, X[iv,: ])
    ...
    def estimate_spectral_norm_stat1(self, it=1, mode='efr'):
        ...
        for i in range(it):
            ...
            spectral_norm_mean.append(self.get_mean_stat1())
            
            if mode == 'efr':
                spectral_norm_cov.append(self.get_within_covariance_stat1())
            elif mode == 'sphNorm':
                spectral_norm_cov.append(self.get_within_covariance_stat1())

            # Center and whiten the statistics
            self.whiten_stat1(spectral_norm_mean[i], spectral_norm_cov[i])
        return spectral_norm_mean, spectral_norm_cov

    def spectral_norm_stat1(self, spectral_norm_mean, spectral_norm_cov):
    ...
    def _expectation(self, Phi, mean, Sigma, session_per_model, batch_size=100, numThread=1):
        ...
        if Sigma.ndim == 2:
            invSigma = np.linalg.inv(Sigma)
            chol_invcov = np.linalg.cholesky(invSigma)
            Phi_white = chol_invcov.T.dot(Phi)
        elif Sigma.ndim == 1:
            chol_invcov = 1/np.sqrt(Sigma)
            Phi_white = Phi * chol_invcov[:,None]
        ...
        for batch in range(batch_nb):
            ...
            jobs = []
            for i in los:
                p = multiprocessing.Process(target=fa_model_loop,
                            args=(i, batch_start, r, Phi_white, _stat0, 
                                  self.stat1, E_h, E_hh))
                jobs.append(p)
                p.start()
            for p in jobs:
            ...
            _r += np.sum(E_h * session_per_model[:,None], axis=0)
            _R += np.sum(E_hh * session_per_model[:,None, None], axis=0)

            # Update _C and _a after each batch
            if chol_invcov.ndim == 2:
                _C += E_h.T.dot(self.stat1[batch_start:batch_stop, :]).dot(np.linalg.inv(chol_invcov))
            elif chol_invcov.ndim == 1:
                _C += E_h.T.dot(self.stat1[batch_start:batch_stop, :]) /chol_invcov
            
            # Parallelized loop on the model id's
            # Split the list of segment to process for multi-threading
            los = np.array_split(np.arange(C), numThread)

            jobs = []
            for i in los:
                p = multiprocessing.Process(target=fa_distribution_loop,
                    args=(i, _A, self.stat0, batch_start, batch_stop, E_hh))                    
                jobs.append(p)
                p.start()
            for p in jobs:
                p.join()
    ...
    def _maximization(self, Phi, _A, _C, _R=None, Sigma_obs=None, session_number=None):
        ...
        for c in range(C):
            ...
            Phi[distrib_idx, :] = np.linalg.solve(_A[c], _C[:,distrib_idx]).T
    ...
    def estimate_within_class(self, itNb, U, mean, Sigma_obs, batch_size=100,
        ...
        for it in range(itNb):
            ...
            # E step
            #A, C, R = session_shifted_stat._expectation(U, batch_size, numThread)
    ...
    def estimate_hidden(self, mean, Sigma, V=None, U=None, D=None, numThread=1):
        ...
        if Sigma.ndim == 2:
            invSigma = np.linalg.inv(Sigma)
            chol_invcov = np.linalg.cholesky(invSigma)
            W_white = chol_invcov.T.dot(W)
        elif Sigma.ndim == 1:
            chol_invcov = 1/np.sqrt(Sigma)
            W_white = W * chol_invcov[:,None]
