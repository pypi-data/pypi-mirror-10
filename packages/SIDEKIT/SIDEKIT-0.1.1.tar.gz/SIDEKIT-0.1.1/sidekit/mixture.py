...
if h5py_loaded:
    ...
    def read_mixture_hdf5(mixture, mixtureFileName):
        ...
        with h5py.File(mixtureFileName, 'r') as f:
            mixture.w = f.get('/w').value
            mixture.w.resize(np.max(mixture.w.shape))
            mixture.mu = f.get('/mu').value
            mixture.invcov = f.get('/invcov').value
            mixture.cst = f.get('/cst').value
            mixture.det = f.get('/det').value


    def save_mixture_hdf5(mixture, mixtureFileName):
...
class Mixture:
    ...
    def __init__(self, mixtureFileName='', mixtureFileFormat='pickle',
                 name='empty'):
        ...
        if mixtureFileName == '':
            ...
            if h5py_loaded:
                read_mixture_hdf5(self, mixtureFileName)
            else:
