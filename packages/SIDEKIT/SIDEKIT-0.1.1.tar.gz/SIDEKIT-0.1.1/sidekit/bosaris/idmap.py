...
if h5py_loaded:
    def save_idmap_hdf5(idmap, outpuFileName):
        """ Save IdMap in HDF5 format

        :param idmap: IdMap object to save
        ...
        space_left = h5py.h5s.create_simple(idmap.leftids.shape)
        dset_left = h5py.h5d.create(fid, set_leftids, filetype, space_left)
        dset_left.write(h5py.h5s.ALL, h5py.h5s.ALL, idmap.leftids)

        space_right = h5py.h5s.create_simple(idmap.rightids.shape)
        dset_right = h5py.h5d.create(fid, set_rightids, filetype, space_right)
        dset_right.write(h5py.h5s.ALL, h5py.h5s.ALL, idmap.rightids)
    ...
    def read_idmap_hdf5(idmap, inputFileName):
        ...
        idmap.leftids = np.empty(set_leftids.shape[0], dtype=set_leftids.dtype)
        set_leftids.read(h5py.h5s.ALL, h5py.h5s.ALL, idmap.leftids)

        set_rightids = h5py.h5d.open(fid, "/right_ids")
        idmap.rightids = np.empty(set_rightids.shape[0],
                                  dtype=set_rightids.dtype)
        set_rightids.read(h5py.h5s.ALL, h5py.h5s.ALL, idmap.rightids)
        fid.close()
...
class IdMap:
    ...
    def __init__(self, idmapFileName='', idmapFileFormat='pickle'):
        ...
        if idmapFileName == '':
            ...
            if h5py_loaded:
                read_idmap_hdf5(self, idmapFileName)
            else:
    ...
    def save(self, outputFileName):
        ...
        if extension == 'p':
            ...
            if h5py_loaded:
                logging.warning('HDF5 format, only saves leftids and rightids')
                save_idmap_hdf5(self, outputFileName)
            else:
                raise Exception('h5py is not installed, chose another' + 
                        ' format to load your IdMap')
        elif extension == 'txt':
    ...
    def read(self, inputFileName):
        ...
        if extension == 'p':
            ...
            if h5py_loaded:
                read_idmap_hdf5(self, inputFileName)
            else:
                raise Exception('Wrong output format, must be pickle, hdf5 or txt')
        elif extension == 'txt':
