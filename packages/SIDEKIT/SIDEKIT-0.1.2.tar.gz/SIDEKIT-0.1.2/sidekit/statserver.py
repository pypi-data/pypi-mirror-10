...
def fa_model_loop_routine(index, batch_start, r, Phi_white, Phi, Sigma, 
    ...
    if Sigma.ndim == 2:
        ...
        eigenValues, eigenVectors, _ = scipy.linalg.lapack.dsyevd(A)
...
class StatServer:
    ...
    def get_lda_matrix_stat1(self, rank):
        ...
        eigenValues, eigenVectors, _ = scipy.linalg.lapack.dsyevd(DiscriminationMatrix)
    ...
    def get_nap_matrix_stat1(self, coRank):
        ...
        eigenValues, eigenVectors, _ = scipy.linalg.lapack.dsyevd(W)
    ...
    def _expectation(self, Phi, mean, Sigma, session_per_model, batch_size=100, numThread=1):
        ...
        #index_map = np.repeat(np.arange(C), r)
    ...
    def estimate_between_class(self, itNb, V, mean, Sigma_obs, batch_size=100, Ux=None, Dz=None,
        ...
        for it in range(itNb):
            ...
            #if not minDiv:
            print('minDiv = {}, it = {}'.format(minDiv, it))
            if  minDiv and it < itNb -1:
                print('toto iteration {}, do not apply minDiv'.format(it))
                _R = None
            else:
                print('expression = {}'.format((not minDiv) or (it == itNb -1)))
                print('iteration {}, apply minDiv'.format(it))
