...
def rocchdet(tar, non, dcfweights=np.array([]), pfa_min=5e-4, pfa_max=0.5, pmiss_min=5e-4, pmiss_max=0.5, dps=100):
...
class DetPlot:
    ...
    def plot_rocch_det(self, idx=0, style='color', plot_args=''):
        ...
        x, y, eer, mindcf = rocchdet(self.__tar__[idx], self.__non__[idx],
                                  np.array([]), pfa_min, pfa_max,
                                  pmiss_min, pmiss_max, dps)

        fig = mpl.plot(x, y,
                label='{}; (eer; minDCF) = ({:.03}; {:.03})'.format(
