...
def opt_weights(X, metric='return', max_leverage=1, rf_rate=0., reg=0., freq=252, no_cash=False, sd_factor=1., **kwargs):
    ...
    :reg: regularization parameter for volatility in sharpe
    ...
    if metric == 'return':
        ...
        objective = lambda b: -sharpe(np.log(np.maximum(np.dot(X - 1, b) + 1, 0.0001)),
                                      rf_rate=rf_rate, reg=reg, freq=freq, sd_factor=sd_factor)
...
def sharpe(r_log, rf_rate=0., reg=0., freq=252, sd_factor=1.):
