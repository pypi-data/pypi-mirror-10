...
class ContentV2Api(BaseApi):
    def upload(self, path='', uid='', file=None, filename='', account=None, instance=None):
        ...
        return self._post('/api/v2/content/upload', account=account, instance=instance, path=path, uid=uid, filename=filename, data=file)
    ...
    def upload_rev(self, path='', uid='', file=None, parent_rev='', account=None, instance=None):
        ...
        if not account: account = self.account_name
        if not instance: instance = self.instance_name
        return self._post('/api/v2/content/upload_rev', account=account, instance=instance, path=path, uid=uid, parent_rev=parent_rev, data=file)
...
class ContentApi(BaseApi):
    ...
    def items(self, path='', uid='', fields=[], start=0, limit=1000, sort='-modified', account=None, instance=None):
        """ 获取文件夹下所有文件和文件夹的元数据"""
        if not account: account = self.account_name
        if not instance: instance = self.instance_name

        return self._get('/api/v1/content/items', fields=','.join(fields), sort=sort, account=account, instance=instance, uid=uid, path=path, start=start, limit=limit)


    def lock(self, path='', uid='', account=None, instance=None):
        """ 将对象加锁"""
        if not account: account = self.account_name
        if not instance: instance = self.instance_name

        return self._get('/api/v1/content/lock', account=account, instance=instance, uid=uid, path=path)


    def unlock(self, path='', uid='',force=False,  account=None, instance=None):
        """ 将对象加锁"""
        if not account: account = self.account_name
        if not instance: instance = self.instance_name

        return self._get('/api/v1/content/unlock', account=account, instance=instance, uid=uid, path=path, force=json.dumps(force))
    ...
    def get_lock(self, path='', uid='',  account=None, instance=None):
        ...
        return self._get('/api/v1/content/get_lock', account=account, instance=instance, uid=uid, path=path)

    def search(self, path='', uid='', queries=[], fields=[], start=0, limit=1000, sort='-modified', account=None, instance=None):
        """ 获取文件夹下所有文件和文件夹的元数据"""
        if not account: account = self.account_name
        if not instance: instance = self.instance_name
 
        return self._get('/api/v1/content/search', queries=json.dumps(queries), fields=','.join(fields), sort=sort, account=account, instance=instance, uid=uid, path=path, start=start, limit=limit)
    ...
    def workflow_inputs(self, path='', uid='', account=None, instance=None):
        ...
        return self._get('/api/v1/content/workflow_inputs', account=account, instance=instance, uid=uid, path=path)

    def workitems(self, path='', uid='', pid='', stati='', account=None, instance=None):
        """删除版本"""
        if not account: account = self.account_name
        if not instance: instance = self.instance_name

        return self._get('/api/v1/content/workitems', pid=pid, stati=stati, account=account, instance=instance, uid=uid, path=path)
