...
class ViewerApi(BaseApi):
    ...
    def remove_cache(self, path, mimes, expire=None, account=None, instance=None):
        if not account: account = self.account_name
        if not instance: instance = self.instance_name
        return self._get('/api/v1/cache/remove', account=account, instance=instance, path=path, mimes=','.join(mimes), expire=expire)
