...
class EDOOAuthClient(OAuthClient):
    ...
    def request(self, method, uri, **opts):
        # 针对使用post方式上传大文件的修补
        if uri in ['/api/v2/content/upload', '/api/v2/content/upload_rev']:
            uri = Connection.build_url(self.site, path=uri)
            data = opts.pop('data')
            response = Request(method, uri, **opts).request(data)
        else:
            uri = Connection.build_url(self.site, path=uri)
...
class BaseClient(WoAPI):
    ...
    def refresh_token(self, refresh_token):
        ...
        self._access_token = access_token.refresh()
    ...
    def auth_with_password(self, username, password, account='', **opt):
        if not account:
            account = self.account_name
        self._access_token, token_info = self.client.password.get_token(username=username,
                                 password=password, account=account, **opt)
        if not self.token_code:
            raise ApiError(403 ,403, 'Authentication failed')

    def auth_with_borrow_token(self, access_token, **opt):
        self._access_token, token_info = self.client.borrow.get_token(access_token=access_token, **opt)
        if not self.token_code:
            raise ApiError(403 ,403, 'Authentication failed')
    ...
    def auth_with_token(self, access_token, refresh_token=''):
        ...
        self._access_token = AccessToken(self.client, token=access_token, refresh_token=refresh_token)
