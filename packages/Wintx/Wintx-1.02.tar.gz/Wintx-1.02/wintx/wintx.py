...
class Wintx(object):
  ...
  def connect(self):
    ...
    try:
      ...
      raise WintxDatabaseError('Was unable to connect to %s@%s:%s - %s' %
          (self.db_config['user'], self.db_config['fabric']['host'], self.db_config['fabric']['port'],
           str(e)))
    ...
    for key in query_dict:
      ...
      if( type(query_dict[key]) == type({}) ):
        for param in query_dict[key]:
          ...
          if( key=='timestamp' ):
            from_epoch = value - datetime.utcfromtimestamp(0)
            value = int(from_epoch.total_seconds())
  ...
  def queryWithin(self, polygon, query_dict, reverse_points=False, sort_column=None):
    ...
    records = self.__queryAllShards__(query)
    records = self.__finalizeRecords__(results, sort_column)
    return records
  ...
  def __determineShardGroup__(self, data_dict):
    ...
    server = self.connection._fabric.get_shard_server((table,), key, scope=fabric.SCOPE_LOCAL, mode=fabric.MODE_READWRITE)
    return server[1]
