"""
Calculate birth information based on a specified DOB.
"""
...
class Current:
    """
    Stores the current date in a class.
    ...
    Note: These variables can be modified to the end user's liking.

    Example (Finding out how old you were exactly a month ago):
    >>> import agecalc
    >>> agecalc.Current.mm = 5
    >>> dob = agecalc.AgeCalc(1, 2, 2000)
    >>> print dob.age
    """
    def __init__(self):
        pass
...
class AgeCalc:
    ...
    def age_weeks_days(self):
        ...
        aws = aw - awi
        aws *= 7
        awr = aws // 1  # Round float up if >=0.5, down if <0.5.
        awr += (aws % 1) / float(0.5)
        awr //= 1
        awr = int(awr)
        aw = int(aw)
