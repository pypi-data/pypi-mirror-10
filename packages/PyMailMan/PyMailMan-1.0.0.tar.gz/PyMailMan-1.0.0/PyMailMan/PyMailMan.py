...
class PyMailMan():
    ...
    def send(self, receivers, title, content, *attachment):
        ...
        if attachment:
            ...
            for attachName in attachment:
                attachName = attachName if os.path.isabs(attachName) else self.RltToAbs(attachName)
                filename = os.path.basename(attachName)
                if imghdr.what(attachName):
                    with open(attachName, 'rb') as f:
                        mime = MIMEBase('image', imghdr.what(attachName), filename=filename)
    ...
    def RltToAbs(self, path):
        return os.path.sep.join([os.getcwd(),path])