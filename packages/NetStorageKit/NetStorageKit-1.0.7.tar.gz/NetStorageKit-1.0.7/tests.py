...
def get_temp_file(contents=None, prefix=__name__):
    file = tempfile.NamedTemporaryFile(prefix=prefix, delete=False)
    if contents:
...
def test_unicode():
    ...
    file = get_temp_file(prefix='foo_bar')
    body = u"""
    <?xml version="1.0" encoding="ISO-8859-1"?>
    <stat directory="/dir/foo">
        <file type="dir" name="test é" mtime="1395977462"/>
    </stat>
    """
    mock_response = {'status': 200, 'content_type': 'text/xml;charset=iso-8859-1', 'body': body}
    request = ns.api.Request('test-key', '123', '12345', 'host', testing=mock_response)

    data, r = request.dir('/foo')
    assert data.stat.file.name == u'test é'
    assert r.status_code == 200
    assert r.headers['Content-Type'] == 'text/html'
