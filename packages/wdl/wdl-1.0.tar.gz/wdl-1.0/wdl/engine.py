...
class SymbolTable(list):
            ...
            if isinstance(node, Call):
                for input_name, expression in node.inputs.items():
                    ...
                    for index, x in enumerate(self):
                        if x[0] == input_name:
                            if isinstance(expression.ast, wdl_parser.Terminal) and expression.ast.str == 'identifier':
    ...
    def is_scatter_var(self, call, var):
        ...
        if var in call.inputs:
            ...
            if isinstance(expr, wdl_parser.Terminal) and expr.str == 'identifier':
        ...
        for entry in self:
            ...
            if name == var:
                ...
                if evaluate and isinstance(value, str) and value.startswith('%expr:'):
                    ...
                    tokens = wdl_parser.lex(expr_string, 'string')
                    parser_ctx = wdl_parser.ParserContext(tokens, wdl_parser.DefaultSyntaxErrorHandler(expr_string, 'string'))
                    expr = wdl_parser.parse_e(parser_ctx).ast()
