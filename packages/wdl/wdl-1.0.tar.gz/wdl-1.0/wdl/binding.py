...
def assign_ids(ast_root, id=0):
    ...
    elif isinstance(ast_root, wdl_parser.Terminal):
        ast_root.id = id

def parse(string):
    ast = wdl_parser.parse(string).ast()
...
def parse_runtime(ast):
    if not isinstance(ast, wdl_parser.Ast) or ast.name != 'Runtime':
...
def parse_declaration(ast):
    if not isinstance(ast, wdl_parser.Ast) or ast.name != 'Declaration':
...
def parse_call(ast, tasks):
    if not isinstance(ast, wdl_parser.Ast) or ast.name != 'Call':
...
def parse_command_variable(ast):
    if not isinstance(ast, wdl_parser.Ast) or ast.name != 'CommandParameter':
        raise BindingException('Expecting a "CommandParameter" AST')
    type_ast = ast.attr('type') if ast.attr('type') else wdl_parser.Terminal(wdl_parser.terminals['type'], 'type', 'string', 'fake', ast.attr('name').line, ast.attr('name').col)
...
def parse_output(ast):
    if not isinstance(ast, wdl_parser.Ast) or ast.name != 'Output':
...
def parse_type(ast):
    ...
    if not isinstance(ast, wdl_parser.Ast) or ast.name != 'Type':
        raise BindingException('Expecting an "Type" AST')
    name = ast.attr('name').source_string
    ...
    if isinstance(ast, wdl_parser.Terminal):
        if ast.str == 'integer':
            ...
            if symbol is None:
                return WdlUndefined()
            return symbol
    elif isinstance(ast, wdl_parser.Ast):
        ...
        if ast.name == 'MemberAccess':
            ...
            if isinstance(object, WdlUndefined):
                return object
            if not isinstance(member, wdl_parser.Terminal) or member.str != 'identifier':
