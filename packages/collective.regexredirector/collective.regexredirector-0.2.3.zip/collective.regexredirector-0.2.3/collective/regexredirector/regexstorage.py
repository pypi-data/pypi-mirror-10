...
class RegexRedirectionStorage(RedirectionStorage):
    ...
    def has_path(self, old_path, settings=None):
        """
        After watching on superclass, look on regex registry
        if the oldpath is redirected
        """
    ...
    def get(self, old_path, default=None):
        """
        After watching on superclass, look on regex registry if the oldpath is
        redirected and return the new url in paying attention on the regex
        pattern
        """
    ...
    def parse_to_array(self, regex_string):
        result = {}
        lines = regex_string.replace("\r", "").split("\n")
        for element in lines:
            search = re.search("'(.+)'='(.+)'", element)
            old = search.group(1)
            new = search.group(2)
            result[old] = new
        return result
