...
from appdynamics.config import BT_INFO_REQUEST_TIMEOUT_MS
...
class TransactionService(object):
    ...
    def _do_start_transaction(self, info_transport, reporting_transport, bt):
        ...
        response = info_transport.recv(timeout_ms=BT_INFO_REQUEST_TIMEOUT_MS)

        if response is None:
            self.logger.warning(
                'BT:%s (name=%r) timeout waiting for BTInfoResponse (timeout=%dms)',
                bt.request_id, bt.name, BT_INFO_REQUEST_TIMEOUT_MS)
