...
class Agent(object):
    ...
    def start_exit_call(self, bt, caller, backend, **kwargs):
        ...
        with self.snapshot_svc.bt_lock:
            snapshotting = bt.snapshotting

        if exit_call and snapshotting:
    ...
    def db_exit_call(self, caller, backend_id, operation, params=None):
        ...
        if backend:
            ...
            try:
                exit_call = self.start_exit_call(bt, caller, backend, operation=operation, params=params)
            except:
            ...
            try:
                ...
                if exit_call:
                    self.end_exit_call(bt, exit_call, exc_info)
        else:
            yield

    def end_transaction(self, exc_info, bt=None):
        ...
        if self.enabled and bt:
            ...
            # Logged errors mark the BT as error depending on the config.
            if error_config.logged_errors_should_cause_error_transaction:
                bt.has_errors = bool(bt.logged_errors)

            # Unignored http errors and exceptions always cause an error BT.
            # Note: exceptions are always added to the BT, regardless of whether or not they are ignored.
            if not bt.has_errors:
                bt.has_errors = bool(bt.http_errors or
                                     not all(error_config.is_exception_ignored(e) for e in bt.exceptions))
