...
HTTP_ERROR_CODES : str, optional
    A list of HTTP error code configuration dictionaries.  Each error code has
    the following attributes: description; lowerBound, upperBound, enabled.
...
def parse_http_error_codes(codes):
    """Parse a string of comma seperated error code tuples.

    Each tuple should be of the form (description, lowerBound, upperBound,
    enabled).

    """
    error_codes = []
    for code in re.findall(r'\(([^\)]+)\)', codes):
        description, lowerBound, upperBound, enabled = code.split(',')
        error_codes.append({
            'description': description.strip(),
            'lowerBound': int(lowerBound),
            'upperBound': int(upperBound),
            'enabled': on_off(enabled.strip()),
        })
    return error_codes
...
_CONFIG_OPTIONS_BY_SECTION = {
    'agent': {
        ...
        'node': ('NODE_NAME', None),
        'dir': ('DIR', None),
        'http_error_codes': ('HTTP_ERROR_CODES', parse_http_error_codes),
    },
