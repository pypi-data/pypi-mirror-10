...
class _Cluster(object):
    ...
    def run_job(self, _job, cluster, coro=None):
        ...
        try:
            ...
            if self._sched_jobs.pop(_job.uid, None) == _job:
                ...
                self.unsched_jobs += 1
                _job.node.busy -= 1
                _job.node.dispy_node.busy = _job.node.busy
            self._sched_event.set()
        except:
            ...
            if cluster.status:
                self.worker_Q.put((cluster.status,
                                   (DispyJob.Running, _job.node.dispy_node, _job.job)))
    ...
    def _schedule_jobs(self, coro=None):
        ...
        while not self.terminate:
            ...
            cluster = self._clusters[_job.compute_id]
            _job.node = node
            logger.debug('Scheduling job %s on %s (busy: %s / %s)',
    ...
    def submit_job(self, _job, coro=None):
        ...
        if cluster.status:
            self.worker_Q.put((cluster.status, (DispyJob.Created, None, None)))
    ...
    def submit(self, *args, **kwargs):
        ...
        def _submit_job(self, _job, coro=None):
            try:
                ...
                if self.status:
                    self._cluster.worker_Q.put((self.status, (DispyJob.Created, None, None)))
                if self.fault_recover_file:
                    shelf = shelve.open(self.fault_recover_file, flag='c')
                    state = {'id':_job.job.id, 'hash':_job.hash, 'compute_id':_job.compute_id,
                             'args':_job.args, 'kwargs':_job.kwargs,
