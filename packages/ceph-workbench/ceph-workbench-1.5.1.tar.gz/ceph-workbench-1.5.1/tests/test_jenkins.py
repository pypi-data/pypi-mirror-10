...
"""
    ...
    def test_prepare_qemu_img(self):
        j = jenkins.Jenkins.factory([
            ...
            '--jenkins-qemu-image', 'ubuntu-14.04-i386',
            '--libdir', 'data_files',
            '--datadir', 'data',
            '--jenkins-qemu-prepare',
        ])
        j.run()
        self.assertTrue(os.path.exists('data/ubuntu-14.04-i386'))
        self.assertTrue(os.path.exists('data/ubuntu-14.04-i386/.ssh/id_rsa'))
    ...
    def test_qemu_run_jenkins_slave(self):
        ...
        self.assertIn(via_master_cmdline, via_slave_cmdline)
