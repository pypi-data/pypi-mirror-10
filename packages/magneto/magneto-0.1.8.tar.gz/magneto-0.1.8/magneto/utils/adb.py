...
class ADBLogWatch(threading.Thread):
    ...
    def run(self):
        ...
        while not self.asked_to_stop:
            ...
            for pattern, (future, min_times) in self._watchers.items():
                Logger.debug('MATIAS - %s' % line)
                match = pattern.search(line)
                Logger.debug('MATIAS - %s' % match)
                if match:
                    # short_pattern = pattern.pattern[:20]+'...'
                    short_pattern = pattern.pattern
    ...
    def assert_done(self, timeout=15, stall=None):
        ...
        for pattern, _ in self._watchers.items():
            Logger.debug('MATIAS - %s' % pattern.pattern)
