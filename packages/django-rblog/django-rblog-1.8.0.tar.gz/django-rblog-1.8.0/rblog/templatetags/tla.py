...
def tla_list(context):
    ...
    ref = "/" + slugify(request.META.get('REQUEST_URI',
                                         request.META.get('PATH_INFO', '/')))
    url = 'http://www.text-link-ads.com/xml.php?inventory_key=%s&referer=%s' % (
        conf.KEY_TLA, ref)
    try:
...
{% if links %}
    {% for link in links %}
    <li>{{ link.before_text }}
        <a href="{{ link.url }}">{{ link.text }}</a> {{ link.after_text }}</li>
    {% endfor %}
{% endif %}
</ul>
{% endcache %}
