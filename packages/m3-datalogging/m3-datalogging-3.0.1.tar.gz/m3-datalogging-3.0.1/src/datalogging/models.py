...
try:
    # на момент коммита версия architect в REQUIREMENTS не позваляет установить
    # свои feature возможно потом можно будет
    # сделать свой класс от PartitionFeature
    # проблема роутинга решается заменой курсора
    DataLog.architect.operation.cursor = connections['datalogger'].cursor()
