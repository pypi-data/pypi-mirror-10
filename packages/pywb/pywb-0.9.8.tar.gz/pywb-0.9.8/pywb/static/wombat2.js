...
//        }
    ...
    function override_innerHTML(obj) {
        var setter = function(string) {
            ...
            if (write_doc.head && document.head) {
                ...
                for (var i = 0; i < elems.length; i++) {
                    ...
                    this.insertAdjacentHTML('beforeend', elems[i].outerHTML);
                    console.log("Adding: " + elems[i].outerHTML);
                }
            }
        ...
        var getter = function() {
            ...
            var end = outer.lastIndexOf("</");
            var start = outer.indexOf(">");
            if (end <= start) {
                return "";
            } else {
                return outer.slice(start + 1, end);
            }
        }

        var curr_src = obj.innerHTML;

        def_prop(obj, 'innerHTML', curr_src, setter, getter);
    }
