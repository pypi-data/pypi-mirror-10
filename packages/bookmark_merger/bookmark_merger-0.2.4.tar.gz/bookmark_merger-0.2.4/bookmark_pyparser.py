...
try:
    ...
    # This is present only for python 2.7+
except ImportError:
    # however, a dropin replacement exists on pypi
    try:
...
def bookmarkDict(parseresults):
    ...
    for pos,item in enumerate(parseresults): #treating parseresults as a list (order should be preserved)
        if type(item)==str:
...
def top_folders_dict(parseresults):
    itemlist=OrderedDict() # top folder?
        ...
        for key,item in bookdict2.items():
            if key in new:
                if item == new[key]: #remove duplicates (optimisation)
                    print_() 
                    print_('#'*10+' found duplicate of ',key)
                    pass
                elif type(item)==str:
