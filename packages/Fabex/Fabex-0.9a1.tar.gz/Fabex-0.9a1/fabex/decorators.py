...
_host_runs = defaultdict(set)

def runs_once_per_host(func):
    @wraps(func)
    def decorated(*args, **kwargs):
        if not hasattr(decorated, 'return_value') and env.host not in _host_runs[func]:
            _host_runs[func].add(env.host)
            decorated.return_value = func(*args, **kwargs)
        else:
            print("[{host}] Already executed '{command}' on this host".format(**env))
