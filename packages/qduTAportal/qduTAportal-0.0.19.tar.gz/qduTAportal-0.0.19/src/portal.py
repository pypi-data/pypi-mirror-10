...
class Portal(object):
    ...
    def __init__(self):
        ...
        # headers
        self.__headers = dict()
        # if check the account is already used, try one more time
        #   logout first, then login
        self.__account_is_used_count = 0
    ...
    def login(self, username, password, times=1):
        ...
        try :
            ...
            print("Times %s: Unable to Connect the Network. Please check it." % times)
            time.sleep(10)
        ...
        if 'Logged in successfully.' == login_message:
            ...
            if self.__account_is_used_count == 0:
                self.__account_is_used_count += 1
                self.logout()
                return self.login(username, password, times)
                # return 'Try Login with the situation that the account may beused.'
