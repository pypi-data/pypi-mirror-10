...
def generate_fastr(fasta_q_file, depth_file,\
    ...
    Input:
        ...
        depth_file:
                This depth file may be in SAM or BAM format or it may be
                a coverage file generated from the command
                "bedtools genomecov -ibam <bam file> -d"
    ...
    if depth_file_type == 'sam':
        ...
        output(message, args.verbosity, 0, log_file = args.log_file,\
               fatal = True)
    elif depth_file_type != 'bam' or depth_file_type != 'bed':
    ...
    for entry in entry_generator(fasta_q_file, fasta_q_file_type):
        ...
        if depth_file_type == 'bam':
            generator = read_bam(depth_file, sequenceID)
        elif depth_file_type == 'cov':
            generator = read_cov(depth_file, sequenceID)
        for basePosition, baseReadDepth in generator:
    ...
    def read_bam(in_file, sequence_id):
        '''yields the base position and read depth of every base in a sequence

        Input:

            in_file:
                   A BAM file to obtain sequence depth from.

            sequence_id:
                    The Sequence ID to obtain the read depth for.

        Output:
                A generator which yields tuples containing base position
                and base position: (base position, base depth).
        '''
        
        alignmentFile = pysam.Samfile(in_file, 'rb')
        for pileupColumn in alignmentFile.pileup(sequence_id):
            yield (pileupColumn.pos, pileupColumn.n)
        alignmentFile.close()

    def read_cov(in_file, sequence_id):
        '''yields the base position and read depth of every base in a sequence

        Input:

            in_file:
                   A bedtools coverage file to obtain sequence depth from.
        ...
        with open(in_file, 'rU') as in_handle:
            isOneBase = False
            if int(in_handle.read().split('\t')[1]) == 1:
                isOneBase = True
            in_handle.seek(0)
            for line in in_handle:
                lineSplit = line.split('\t')
                if lineSplit[0] == sequence_id:
                    if isOneBase:
                        lineSplit[1] = int(lineSplit[1]) - 1
                    yield (int(lineSplit[1]), int(lineSplit[2]))
...
if __name__ == '__main__':
    ...
    readDepthFile = parser.add_mutually_exclusive_group()
    readDepthFile.add_argument('--bsam',
                               type = file_type,
                               default = None,
                               help = 'SAM or BAM file containing mapping' +\
                               'data for given FASTA/Q file')
    readDepthFile.add_argument('--bedtools_d',
                               type = file_type,
                               default = None,
                               help = 'coverage generated using the command' +\
                               ' "bedtools genomecov -ibam <bam file> -d"')
