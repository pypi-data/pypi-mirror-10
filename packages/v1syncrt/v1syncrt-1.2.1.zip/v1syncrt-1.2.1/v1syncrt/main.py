...
from sync_tags import main as main_sync_tests
...
def main(args_list=sys.argv[1:]):
    ...
    commamd_help = {
        'add_regression': 'Adds all regression test taged to V1',
        'add_to_suite': 'Adds test to its corresponding Regression Suite',
        'sync_tests': 'Adds the test tags from Yarara to V1',
        'sync_rs_tags': 'If a Certain test scenario is associated to a suite in V1, adds that tag to the Scenario',
    }
    ...
    for command in command_list:
        parser.add_argument('--' + command, action='store_true', help=commamd_help[command], default=False)
    ...
    parser.add_argument('--update_results', required=False, help='Updates test results to V1', default=None)
    ...
    for command in command_list:
        if getattr(args, command):
            print command
            try:
                command_mains[command](common_args, v1m, v1config)
