...
class CWWorker(AnyEntity, UtilsHolder):
        ...
        with self.temp_cnx() as cnx:
            ...
            try:
                ...
                if not subprocess:# or cnx.vreg.config.mode == 'test':
                    ...
                    self._launch_task_in_subprocess(cnx, task)
    ...
    def _launch_task_in_subprocess(self, cnx, task):
        ...
        self.warning('Created Subprocess %s for %s', proc.eid, task.dc_title())
        cnx.commit()
        proc.synchronousstart()
        retcode = proc.return_code
        if retcode != 0:
            task.cw_adapt_to('IWorkflowable').fire_transition('task_fail')
            cnx.commit()
        self.info('Task %s output : %s %s', task.dc_title(),
                  retcode, proc.cw_adapt_to('IWorkflowable').state)
