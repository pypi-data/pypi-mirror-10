...
def euler(function,y0,t_mat,*args):
...
def rk2(function,y0,t_mat,*args):
...
def rk4(function,y0,t_mat,*args):
...
def rkwrapper(function,_self):
    def fun(t,y,*args):
        return function(y,t,_self,*args)
    return fun

def rk45(function,y0,t_mat,_self,*args):
...
class Simulation(object):
    ...
    def add(self,diffstr,initial_value=0,
        ...
        if name.count("'")<=1:
            ...
            for i in range(order-1):
                ...
                c=Component(ds,initial_value[i],min,max,plot,save)
            ...
            ds="%s'=%s" % (vname,rest)
            c=Component(ds,initial_value[order-1],min,max,plot,save)
            self.components.append(c)
            self.data_delay[c.name]={'t':[],'value':[]}
            cc.append(c)
    ...
    def repeat(self,t_min,t_max,num_iterations=1000,discrete=False,**kwargs):
    ...
    def run_fast(self,t_min=None,t_max=None,num_iterations=1000):
        if t_min is None:
        ...
        with RedirectStdStreams(stdout=devnull, stderr=devnull):
            result,extra=odeint(func,y0,t,(self,),full_output=True,printmessg=False)
    ...
    def run(self,t_min=None,t_max=None,num_iterations=1000,dt=None,discrete=False,xlabel='Time'):
        self.figures=[]
        ...
        if self.method=='map':
            ...
            result=rk2(func,y0,t,self)
        elif self.method=='rk4':
            result=rk4(func,y0,t,self)
        elif self.method=='rk45':
            result=rk45(func,y0,t)
        else:
        ...
        for c in self.components+self.assignments:
            ...
            if c.plot and not self.noplots:
                ...
                if c.data:
                    if c.data['plot']:
                        plot(c.data['t'],c.data['value'],color+'s')
